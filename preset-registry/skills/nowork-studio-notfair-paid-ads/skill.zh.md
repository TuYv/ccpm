...
从业务成果出发，而不是从平台请求出发。明确转化目标、转化价值或盈亏平衡阈值、目标指标、每日和每月预算、地理区域、落地页，以及跟踪是否已验证。如果某项输入不可用，请将由此得出的建议标记为草案，而不要用通用假设填补空缺。

## 工作分流

| 请求 | 使用 |
|---|---|
| 连接账户、确认访问权限或获取工作区上下文 | `/notfair:paid-ads-setup` 或 `/notfair:paid-ads-integrations` |
| 在进行更改前审计 Google 或 Meta | `/notfair:google-ads-audit` 或 `/notfair:meta-ads-audit` |
| 运营 Google 或 Meta 账户 | `/notfair:google-ads` 或 `/notfair:meta-ads` |
| 审计或运营 X Ads 或 LinkedIn Ads | `/notfair:paid-ads-x` 或 `/notfair:paid-ads-linkedin` |
| 规划新的跨渠道广告活动 | `/notfair:paid-ads-launch` |
| 查看每周/每月评分卡 | `/notfair:paid-ads-review` |
...
对于单一平台的请求，应直接分流，而不要重复专门工作流。在承诺读取或写入账户之前，先确认该平台自身经过验证的连接器可用。

## 操作原则

先给出决策：哪些有效、哪些无效，以及最有充分依据、影响最大的一项行动。确保建议可衡量且可逆。广告活动计划、创意简报或预算分配表的状态为 `ready_for_review`；只有在平台确认具体对象和设置后，其状态才会变为 `published`。