...
- ❌ **在 Main 上阻塞协程** - 切勿在主线程上使用 `runBlocking`
- ❌ **使用 GlobalScope** - 使用 `viewModelScope`/`lifecycleScope` 实现结构化并发
- ❌ **在 init 中收集 flow** - 使用 `repeatOnLifecycle` 或 `collectAsStateWithLifecycle`
- ❌ **暴露可变状态** - 暴露 `StateFlow` 而非 `MutableStateFlow`
- ❌ **不处理 flow 中的异常** - 始终使用 `catch` 操作符
- ❌ **对可空值使用 lateinit** - 使用 `lazy` 或带有 `?` 的可空类型
- ❌ **硬编码调度器** - 注入调度器以便于测试
- ❌ **不使用密封类** - 对于有限的状态集合，优先使用密封类
- ❌ **在 Composable 中执行副作用** - 使用 `LaunchedEffect`/`SideEffect`
