...
配置完成后，`/seo-analysis` 会自动从 CMS 拉取已发布的内容，并与 Google Search Console 数据进行交叉比对——找出不可见页面、内容缺口、陈旧文章以及缺失的 SEO 字段。

---

## 第 0 步 — 设置

阅读并遵循 `../shared/preamble.md`——该文件会定位 SEO 脚本目录。以下所有脚本调用都使用前言中的 `$SKILL_SCRIPTS`。
...
- `CMS_STATUS=0` → 已配置 CMS（`$CMS_TYPE` 是其名称）。
  ...
  等待用户回复。如果用户选择重新配置/切换，则继续执行第 2 步。
  如果用户选择测试或验证，则跳转到第 5 步（直接进行连接测试）。

- `CMS_STATUS=2` → 尚未配置任何 CMS。继续执行第 2 步。
...
> 2. **你的 WordPress 用户名**（用于登录的用户名）
> 3. **一个应用程序密码**——请在以下位置创建：
>    WordPress 后台 → 用户 → 个人资料 → 向下滚动到**应用程序密码**
...
> 2. **一个具有完整访问权限的 API 令牌**——请在以下位置创建：
>    Strapi 后台 → 设置 → 全局设置 → API 令牌 → 创建新的 API 令牌
>    → 类型：**完整访问权限** → 复制令牌
>
> 可选：
...
> “我需要以下三项信息来连接 Contentful：
>
> 1. **空间 ID** — 可在以下位置找到：Contentful → 设置 → 常规设置 → 空间 ID
> 2. **Content Delivery API 令牌** — 可在以下位置找到：
>    设置 → API 密钥 → [你的密钥] → Content Delivery API - 访问令牌
>    （如果不存在密钥，请在设置 → API 密钥 → 添加 API 密钥下创建一个）
...
**`PREFLIGHT_EXIT=2`** → 凭据在步骤之间从 `.env.local` 中被移除了。从步骤 3 重新开始。
...
然后提供以下选项：
> “运行 `/seo-analysis`，查看包含你的 CMS 内容的完整审计，
