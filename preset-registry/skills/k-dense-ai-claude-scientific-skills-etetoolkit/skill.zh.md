...
- 读取 Newick/Nexus，然后检查、注释、转换、设定根节点、剪枝并写出
  Newick 树
- 比较拓扑结构并计算系统发育距离
...
此 skill 针对 **ETE 4.4.0**，该版本于 2025 年 9 月 3 日发布，并已于
2026 年 7 月 23 日验证为当前 PyPI 版本。
...
- 包和导入：`ete4`，而不是 `ete3`
- 文件输入：传入已打开的文件对象；字符串用于 Newick 文本，不要依赖
...
无需凭据。NCBI 和 GTDB 工作流会下载公开的分类数据，可能占用大量磁盘空间；
首次更新前请参阅 [`references/taxonomy.md`](references/taxonomy.md)。
...
请有意识地选择解析器。解析器不匹配是导致 `NewickError`、内部标签丢失，或支持值被读取为名称的最常见原因。
...
`list(tree.search_nodes(name="A"))` 并验证数量。
...
RF 比较使用共享的叶标签，并要求名称具有意义，最好是唯一的。请明确决定在科学上应采用有根比较还是无根比较。
...
物种重叠调用是根据所提供的拓扑结构和命名函数得出的推断，并非独立的直系同源关系证据。请显式传入命名函数，并使用有根且完全二叉的基因树。对于严格的协调分析，请使用经过整理的物种树和
...
ETE 4 还提供了 `GTDBTaxa`，用于以基因组为中心的细菌和古菌分类学。不要混用 NCBI 数字 TaxID 和 GTDB 字符串标识符。
...
从此 skill 目录运行。以下命令通过 `uv run --with` 使用固定版本、隔离的 ETE 4
运行时。
...
对于每行一个分类单元名称的情况，使用 `--keep-file taxa.txt` 代替 `--keep ...`。
该脚本会拒绝含义不明确或缺失的请求名称，而不是静默生成不完整的树。
