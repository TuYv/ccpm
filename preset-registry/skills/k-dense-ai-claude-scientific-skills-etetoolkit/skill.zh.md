...
ETE 不会取代序列比对或系统发育推断软件。对于原始序列，请先使用 MAFFT
或其他比对工具，以及 IQ-TREE 2、FastTree 或其他推断工具；然后将生成的树加载到 ETE 中。

## 当前目标

此技能针对 **ETE 4.4.0**，该版本于 2025 年 9 月 3 日发布，并于 2026 年 7 月 23 日确认是当前的 PyPI 版本。

使用 `https://etetoolkit.github.io/ete/` 作为 ETE 4 文档。尽管 URL 中包含
`latest`，`etetoolkit.org/docs/latest` 页面实际上是旧版 ETE 3 文档。

不要将以下示例默默转换回 ETE 3：

- 包和导入：`ete4`，而不是 `ete3`
- 文件输入：传入已打开的文件对象；Newick 文本使用字符串，不要依赖 ETE 4.4.0 中保留的路径字符串启发式判断
...
请有意选择解析器。解析器不匹配是导致
`NewickError`、内部标签丢失或支持值被读取为名称的最常见原因。
...
RF 比较使用共享的叶标签，并要求名称具有意义，最好是唯一的。请明确决定采用有根比较还是
无根比较，并确认其是否符合科学分析的要求。
...
物种重叠调用是根据所提供的拓扑结构和命名函数得出的推断，并非独立的直系同源证据。
请显式传入命名函数，并使用有根且完全二叉的基因树。对于严格的协调分析，请使用经过整理的物种树，
并调用 `gene_tree.reconcile(species_tree)`。
...
ETE 4 还提供了 `GTDBTaxa`，用于以基因组为中心的细菌和古菌分类。不要混用 NCBI
数字 TaxID 和 GTDB 字符串标识符。
...
请从此 skill 目录运行。以下命令通过 `uv run --with` 使用固定版本、隔离的 ETE 4
运行时。
...
对于每行一个分类单元的文件，请使用 `--keep-file taxa.txt` 代替 `--keep ...`。
如果请求的名称存在歧义或缺失，脚本会拒绝执行，而不是静默生成不完整的树。
...
2. 在按名称查找或进行 RF 比较之前，检查是否存在空的和重复的叶名称。
3. 说明将树视为有根树还是无根树。
4. 仅在需要保持保留节点之间的成对距离不变时，才在剪枝时保留枝长。
5. 将任意的多分支解析视为显示或算法上的便利，而非进化证据。
6. 在可复现分析中记录 ETE 版本、解析器、生根方法、剪枝集合和分类数据库快照。
