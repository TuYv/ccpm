...
**检查可用性：** 在使用任何 Firecrawl 工具之前，请通过检查 `firecrawl_scrape` 或任何 Firecrawl 工具是否可用，验证 MCP 服务器是否已连接。如果工具不可用，请告知用户扩展尚未安装，并提供安装说明。

## 快速参考

| 命令 | 用途 |
|---------|---------|
| `/seo firecrawl crawl <url>` | 使用内容提取功能进行全站爬取 |
| `/seo firecrawl map <url>` | 发现网站结构（仅 URL，速度快） |
| `/seo firecrawl scrape <url>` | 使用 JS 渲染抓取单个页面 |
| `/seo firecrawl search <query> <url>` | 在已爬取的网站中搜索 |

## 命令

### crawl -- 全站爬取

从给定 URL 开始爬取整个网站。返回所有已发现页面的页面内容、元数据和链接。

**MCP 工具：** `firecrawl_crawl`

**参数：**
- `url`（必需）：开始爬取的 URL
- `limit`：要爬取的最大页面数（默认：100，最大：500）
...
**SEO 使用模式：**
1. **全面审计爬取**：爬取整个网站，提取所有页面供子代理分析
2. **针对特定部分的爬取**：使用 `includePaths` 仅审计 `/blog/*` 或 `/products/*`
3. **损坏链接检测**：使用 `["links"]` 格式进行爬取，检查所有 href 是否返回 404
4. **内容清单**：大规模提取所有页面的标题、元描述和 H1
...
- 1 个 credit = 爬取或抓取 1 个页面
- Map 操作成本更低（每发现一个 URL 消耗 0.5 个 credits）
- 在进行大规模爬取之前，始终告知用户预计的 credits 使用量

### map -- 网站结构发现

发现网站上的所有 URL，但不获取内容。速度快且节省 credits。

**MCP 工具：** `firecrawl_map`

**参数：**
- `url`（必需）：要映射的网站 URL
- `limit`：要发现的最大 URL 数量（默认：5000）
- `search`：用于筛选 URL 的可选搜索词

**SEO 使用模式：**
1. **站点地图比较**：映射网站，并将发现的 URL 与 XML 站点地图进行比较
2. **孤立页面检测**：检测存在于站点地图中但未从任何页面链接的 URL
3. **爬取预算分析**：分析可编入索引的页面总数与从首页链接的页面数
4. **URL 模式分析**：识别 URL 结构模式、重复项和参数膨胀
5. **预审计发现**：先运行 map，然后对关键部分进行定向爬取
...
`fetch_page.py` 更加彻底，因为它会执行 JS 并等待动态内容。
...
- `onlyMainContent`：移除导航栏/页脚/侧边栏（默认：true）
...
| 场景 | 使用 |
|-----|-----|
| 静态 HTML 页面 | `fetch_page.py`（无 API 费用） |
...
**SEO 使用场景：**
1. **内容差距验证**：在网站上搜索关键词，检查是否存在相关内容
2. **内部链接机会**：查找提及某个主题的页面，以便让它们相互链接
3. **重复内容检测**：搜索关键短语，查找近似重复内容
...
2. 与 XML sitemap（seo-sitemap）进行比较，查找孤立页面/缺失页面
3. 选择重点页面进行深度分析
4. 将抓取的内容提供给所有子代理（技术、内容、schema、geo）
5. 报告可抓取页面总数、URL 模式和抓取深度

### 与 seo-technical 集成
- 损坏链接检测：抓取所有内部链接，检查是否存在 404
- 重定向链映射：跟随所有重定向，标记超过 2 跳的重定向链
- 混合内容检测：检查 HTTPS 页面上的 HTTP 资源
- 规范链接验证：将规范 URL 与实际 URL 进行比较

### 与 seo-sitemap 配合
- Sitemap 覆盖率：抓取页面中存在于 Sitemap 的页面比例
- 孤立页面：抓取发现但 Sitemap 中缺失的页面
- 过期 Sitemap 条目：Sitemap 中返回 404/410 的 URL

### 与 seo-content 配合
- 内容提取：为 E-E-A-T 分析提供干净的 markdown
- 薄内容检测：批量识别少于 300 个词的页面
- 重复内容：比较页面之间的近似重复内容

### 与 seo-schema 配合
- Schema 提取：从所有抓取页面中提取 JSON-LD
- Schema 覆盖率：包含结构化数据的页面比例
- Schema 验证：批量验证提取的 schemas
...
| `429 Too Many Requests` | 请求频率受限 | 等待 60 秒，降低抓取并发数 |
| `408 Timeout` | 页面渲染速度过慢 | 增加 `timeout`，尝试不使用 JS 渲染 |
| `403 Forbidden` | 网站阻止抓取 | 检查 robots.txt，可能需要跳过此网站 |

**优雅降级：** 如果 Firecrawl 不可用，通知用户并建议：
1. 使用 `fetch_page.py` 进行单页面分析（无 API 成本）
