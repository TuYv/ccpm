...
用于处理现有代码库——分析结构、遵循约定，并设置适当的防护措施，确保不破坏任何内容。
...
**先理解，再修改。** 现有代码库有其约定、模式和历史。你的工作是在这些基础上进行，而不是重新组织它们。
...
| 模式 | 检测方式 | 含义 |
|---------|-----------|---------|
| **Monorepo** | `packages/`、`apps/`，或 package.json 中包含 `workspaces` | 多个项目，共享工具链 |
| **全栈单体应用** | 同一代码库中包含 `frontend/` + `backend/` | 单个团队，紧密耦合 |
| **职责分离** | 仅包含前端或后端代码 | 分离的代码库，独立部署 |
...
**只添加缺失的防护措施。绝不要覆盖现有配置。**

### 4.1 JavaScript/TypeScript 项目

#### Husky + lint-staged（如果尚未存在）
...
- **不要重新组织目录结构** - 在现有模式内进行工作
- **不要为了“保持一致”而重命名文件** - 遵循现有命名约定
- **不要添加新的模式** - 使用代码库中已有的模式
- **不要更改导入风格** - 遵循现有风格（相对导入或绝对导入等）
- **不要更改格式** - 遵循现有风格，或使用现有的格式化配置
- **不要轻易添加新的依赖** - 检查是否已有等效依赖

### 始终要做这些事情

- **先阅读现有代码** - 在编写新代码之前了解现有模式
- **遵循现有约定** - 命名、结构、错误处理
- **使用现有工具** - 不要重复造轮子
- **遵循现有测试模式** - 匹配测试文件的命名和结构
- **保留现有配置** - 只添加，不要修改，除非是在修复错误
...
不要一次性添加所有防护措施。请遵循以下时间安排：

| 周次 | 重点 | 原因 |
|------|-------|-----|
| 1 | 格式化（Prettier/Ruff） | 不会破坏现有功能，容易快速见效 |
| 2 | 代码检查（ESLint/Ruff） | 可捕获明显问题 |
| 3 | pre-commit hooks | 自动化第 1-2 周的工作 |
| 4 | 提交消息验证 | 保持团队一致性 |
| 5 | 类型检查严格度 | 可捕获运行时错误 |
| 6 | 安全扫描 | 可捕获漏洞 |
...
- **添加未使用的防护措施** - 只添加团队会使用的内容
- **第 1 天就采用严格规则** - 逐步引入
- **因警告而阻塞** - 先采用宽松策略，随着时间推移逐步收紧
- **忽略现有模式** - 基于现有内容开展工作
- **过度设计** - 简单规则优于复杂系统
- **跳过分析阶段** - 修改前务必先了解现状
