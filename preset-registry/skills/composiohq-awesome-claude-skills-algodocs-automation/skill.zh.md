...
requires:
  mcp: [rube]
---
# 通过 Rube MCP 的 Algodocs 自动化

通过 Rube MCP 使用 Composio 的 Algodocs 工具包自动化 Algodocs 操作。

**工具包文档**：[composio.dev/toolkits/algodocs](https://composio.dev/toolkits/algodocs)

## 前置条件

- Rube MCP 必须已连接（RUBE_SEARCH_TOOLS 可用）
- 通过 `RUBE_MANAGE_CONNECTIONS` 使用工具包 `algodocs` 建立有效的 Algodocs 连接
...
2. 使用工具包 `algodocs` 调用 `RUBE_MANAGE_CONNECTIONS`
...
这将返回可用的工具 slug、输入架构、推荐执行计划和已知陷阱。
...
- **始终先搜索**：工具架构会变化。请勿在未调用 `RUBE_SEARCH_TOOLS` 的情况下硬编码工具 slug 或参数
- **检查连接**：在执行工具前确认 `RUBE_MANAGE_CONNECTIONS` 显示 ACTIVE 状态
- **架构一致性**：使用搜索结果中的字段名和类型，且必须精确匹配
- **memory 参数**：在 `RUBE_MULTI_EXECUTE_TOOL` 调用中始终包含 `memory`，即使为空也要传入（`{}`）
- **会话复用**：在同一工作流内复用 session ID；对新工作流生成新的 session ID
- **分页处理**：检查响应中的分页 token，并持续获取直到完成

## 快速参考

| 操作 | 方法 |
|-----------|----------|
| 查找工具 | 使用 Algodocs 特定的 use case 调用 `RUBE_SEARCH_TOOLS` |
| 连接 | 使用工具包 `algodocs` 调用 `RUBE_MANAGE_CONNECTIONS` |
| 执行 | 使用已发现的工具 slug 调用 `RUBE_MULTI_EXECUTE_TOOL` |
| 批量操作 | 使用 `run_composio_tool()` 调用 `RUBE_REMOTE_WORKBENCH` |
| 完整架构 | 使用 `RUBE_GET_TOOL_SCHEMAS` 查看带 `schemaRef` 的工具 |
