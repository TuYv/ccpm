...
根据记录的证据对失败的 Agent QA 运行进行分类，而不是凭空猜测。检查运行、步骤、产物和日志；选择一个固定类别；并返回置信度、可能的责任归属以及下一步有证据支撑的行动。

## 适用场景

- 调查失败或被中断的 Agent QA 运行。
- 检查运行产物、步骤结果或执行日志。
- 比较最近的相关运行，以发现反复出现的失败模式。
- 判断某个失败应归属于测试、产品、hook、浏览器/移动端运行时，还是基础设施的负责人。

## 工作流程

1. 首先使用 `agent_qa_get_run` 获取运行状态、套件子级上下文、步骤和尝试次数。
2. 在做出判定之前，先获取证据：
   - `agent_qa_get_run_artifact`
   - `agent_qa_get_run_steps`
   - `agent_qa_get_run_logs`
   - `agent_qa_get_run_execution_logs`
3. 调用 `agent_qa_classify_failure`，除非有更强的证据与之矛盾，否则应将其类别作为默认分类。
4. 当分类器输出中提供了最近的相关运行时，对其进行比较。
5. 返回简明的分诊结果：类别、置信度、证据、可能的修复范围和下一步行动。
6. 如需修改代码，请在分诊完成后切换到 `agent-qa-debug-fix`。
...
- 引用或概述具体的产物、日志或步骤证据。
- 当产物缺失部分限制了置信度时，应予以指出。
- 不要捏造 MCP 未返回的截图、视频、日志或内存上下文。
- 如果 MCP 不可用，则回退到 dashboard REST API 或 Agent QA CLI 输出，并说明哪些证据无法获取。
- 在报告中隐去凭据、会话令牌、个人数据以及无关的应用内容。
...
- 失败类别指明的是最可能的故障面，并不能证明根本原因。
- 当截图、DOM/无障碍上下文、设备日志或此前的运行记录缺失时，必须降低置信度。
- 该技能不会修改测试或应用代码；如需经授权的修复，请使用 `agent-qa-debug-fix`。
