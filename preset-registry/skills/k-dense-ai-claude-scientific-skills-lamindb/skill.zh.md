...
LaminDB 是一个面向生物学、原生支持数据血缘的开源湖仓。它让数据集和模型具备可查询、可追溯、经过验证、可复现以及符合 FAIR（可发现、可访问、可互操作、可复用）原则的特性，同时以开放格式将数据存储在本地文件系统、S3、GCS、Hugging Face、SQLite 和 Postgres 中。

**核心价值主张：**
- **可查询性**：搜索和筛选 artifacts、records、runs、features、schemas 和 collections
- **可追溯性**：跟踪 notebooks、scripts、functions 和 pipelines 的输入、输出、参数、源代码和环境
...
**核心实体：**
- **Artifacts**：有版本的数据集（DataFrame、AnnData、Parquet、Zarr 等）
- **Records & ULabels**：实验实体、类型化 records 和简单 labels
- **Collections**：有版本且不可变的 artifacts 集合
...
**参考：** `references/core-concepts.md` - 阅读此文档以详细了解 artifacts、records、runs、transforms、features、版本控制和血缘跟踪。
...
**参考：** `references/annotation-validation.md` - 阅读此文档，了解详细的数据整理工作流、架构设计模式、验证错误处理方法和最佳实践。
...
**参考：** `references/integrations.md` - 阅读此文档，了解第三方系统的集成模式、代码示例和故障排除方法。

### 6. 设置和部署

**安装：**
- 当前稳定基线：`lamindb==2.5.1`（发布于 2026-06-01；Python >=3.10、<=3.14）
- 基本安装：`uv pip install 'lamindb==2.5.1'`
- 使用额外模块：`uv pip install 'lamindb[gcp,zarr-v2,fcs]==2.5.1'`
- 仅安装最小命名空间：`uv pip install 'lamindb-core==2.5.1'`
- Bionty 模块：已包含在 LaminDB 文档中，并可通过 `uv pip install 'bionty==2.4.0'` 获取
- 可选模块：对于 wetlab 或 clinical schema 模块，应固定使用经过审核的版本，而不是安装浮动的最新版本
...
**参考：** `references/setup-deployment.md` - 阅读此文档，了解详细的安装、配置、存储设置、数据库管理、安全最佳实践和故障排除方法。

## 安全与安全性默认设置

在帮助进行 LaminDB 设置或集成时：

- 切勿显示、记录或传输实际的 API 密钥、云凭据、数据库密码，或包含机密信息的完整连接字符串。
- 优先使用 IAM 角色、工作负载身份、密钥管理器或命名环境变量，例如 `LAMIN_DB_URL`、`AWS_ACCESS_KEY_ID`、`AWS_SECRET_ACCESS_KEY` 和 `GOOGLE_APPLICATION_CREDENTIALS`；只能检查命名变量是否存在，不得检查其值。
- 在保存来自 REST API、外部数据库或用户提供的文件的内容之前，使用显式模式或 curator 对其进行验证和清理。
- 为了实现可复现安装，请固定软件包版本或使用锁文件。只有当用户明确需要上游最新版本时，才可以接受浮动安装。

## 常见用例工作流

### 用例 1：通过本体验证进行单细胞 RNA 测序分析
...
对于原生 Nextflow 项目，在可用时优先使用 `nf-lamin` 插件和当前的 `nextflow.config` 模式；对于较小或自定义的管道步骤，则使用内联 Python 跟踪。
...
2. **尽早验证**：定义 schemas，并在进行大量分析之前验证数据

3. **使用本体**：利用公共生物本体实现标准化注释

4. **使用键组织数据**：以层次结构组织 artifact keys（例如 `project/experiment/batch/file.h5ad`）

5. **先查询元数据**：在加载大型文件之前进行筛选和搜索

6. **进行版本管理，而非复制**：对修改使用内置版本管理，而不是创建新的 keys
...
10. **从本地开始，在云端扩展**：使用 SQLite 进行本地开发，使用 PostgreSQL 部署到云端

## 参考文件

此 skill 包含按功能组织的完整参考文档：

- **`references/core-concepts.md`** - Artifacts、records、runs、transforms、features、版本管理、谱系
...
根据当前任务所需的具体 LaminDB 功能，阅读相关参考文件。
