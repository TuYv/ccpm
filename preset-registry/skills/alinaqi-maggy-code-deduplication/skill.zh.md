...
**目的：** 防止语义重复和代码膨胀。维护能力索引，使 Claude 在编写新内容之前始终了解已有的内容。
...
│                                                                 │
│  在编写任何新函数之前：                                          │
│  1. 检查 CODE_INDEX.md，确认是否已有现成能力                     │
│  2. 在代码库中搜索相似功能                                      │
│  3. 如果可能，扩展现有代码                                      │
│  4. 仅当没有合适的现有代码时才创建新的实现                       │
├─────────────────────────────────────────────────────────────────┤
│  编写完成后：立即更新索引。                                      │
│  定期执行：运行 /audit-duplicates 以发现重叠功能。               │
└─────────────────────────────────────────────────────────────────┘
```
...
在项目根目录中维护 `CODE_INDEX.md`，按**能力**而非文件位置组织：
...
❌ **不佳：** 在已有 `isEmail()` 的情况下创建 `validateEmail()`
```typescript
...
定期运行 `/audit-duplicates`，以发现语义上的重叠：

### 审计检查清单

- [ ] **工具函数**：是否有执行相似操作的函数？
- [ ] **API 调用**：是否有多种方式获取相同数据？
- [ ] **验证**：是否散落着内联验证逻辑？
- [ ] **错误处理**：错误处理模式是否不一致？
- [ ] **组件**：是否有可以合并的相似 UI 组件？
- [ ] **Hooks**：自定义 Hooks 是否存在逻辑重叠？
...
1. **暂停并检查**：“是否已经存在类似的内容？”
2. 在 CODE_INDEX.md 中搜索类似的功能
3. 如果不确定，在代码库中搜索：`grep -r "functionName\|similar_term" src/`
4. 只有在确认不存在合适的内容后，才创建新的内容
...
```
在实现之前，让我先检查一下我们是否已经有类似的内容……

[检查 CODE_INDEX.md]

找到：utils/file.ts 中的 `existingFunction()` 可以完成类似的功能。
选项：
1. 原样使用现有函数
2. 扩展它以支持新功能
3. 创建新的函数（如果确实是不同的使用场景）
