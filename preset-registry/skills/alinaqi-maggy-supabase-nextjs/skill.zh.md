...
使用 Supabase Auth 和 Drizzle ORM 的 Next.js App Router 模式。
...
**使用 Drizzle 进行查询，使用 Supabase 处理身份验证/存储，默认使用服务器组件。**

使用 Drizzle ORM 进行类型安全的数据库访问。使用 Supabase 客户端处理身份验证、存储和实时功能。优先使用服务器组件；仅在需要时使用客户端组件。
...
- **未使用 middleware 处理身份验证** - 会话刷新至关重要
- **同步调用 `cookies()`** - 在 Next.js 15+ 中必须使用 await
- **在客户端使用 Service key** - 切勿暴露，只能在服务端使用
- **缺少 revalidatePath** - 每次变更后都要重新验证
- **未处理身份验证错误** - 显示对用户友好的消息