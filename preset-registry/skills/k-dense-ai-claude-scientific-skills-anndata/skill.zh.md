...
AnnData 是一个用于处理带注释数据矩阵的 Python 软件包，可将实验测量数据（X）与观测元数据（obs）、变量元数据（var）以及多维注释（obsm、varm、obsp、varp、uns）存储在一起。它最初是通过 Scanpy 为单细胞基因组学设计的，如今已成为适用于任何需要高效存储、操作和分析的带注释数据的通用框架。
