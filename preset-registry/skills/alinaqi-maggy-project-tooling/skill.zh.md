...
**必需**：初始化项目时，始终根据技术栈创建连接部署平台的待办事项。
...
- ❌ 手动部署 - 通过 CI/CD 实现自动化
- ❌ 跳过本地 Supabase - 始终优先在本地开发
- ❌ 直接更改生产数据库 - 使用迁移
- ❌ 未设置分支保护 - 要求进行 PR 审查和 CI 检查
