...
通过 SE Ranking REST API 实时追踪 AI 可见性。

## 前置条件

- 运行 `extensions/seranking/install.sh`（或 `install.ps1`）。
- SE Ranking API 密钥（https://seranking.com/api.html）。
- 在进行任何调用之前，确认 `SERANKING_API_KEY` 存在于 `~/.claude/settings.json` 的 `env.` 下。如果不存在，请告知用户运行安装程序。
...
| `/seo seranking backlinks <url>` | 反向链接概况（Ahrefs / DataForSEO 的替代供应商来源） |
| `/seo seranking competitors <url>` | 前 10 个自然搜索竞争对手及共享关键词差距 |

## AI 声量份额评分

SE Ranking 会对每个平台的响应进行采样，以统计品牌提及次数，
覆盖一组可配置的提示词。评分器采用与 Profound / Peec AI
相同的逻辑，但整合在一个 MCP/API 中。输出字段：
...
- `ai_mode_sov`：品牌在 Google AI Mode 中的引用率（首先使用美国英语）。
...
5 个单位（每个平台 1 个）。使用 `"${CLAUDE_PLUGIN_ROOT}/scripts/claude-seo" run dataforseo_costs.py` 记录
各供应商的支出。

## 跨技能委派

- 对于传统反向链接和内容审核，回退到
  `seo-backlinks` / `seo-content`。
- 对于特定平台的深度分析（仅 ChatGPT、仅 Perplexity），优先使用专门的
  `seo-geo` 技能，其中包含 Brand Mention Correlation 指南。