...
当你需要执行以下操作时，请使用模型剪枝：
- **减小模型大小** 40-60%，同时准确率损失低于 1%
- **加速推理**，利用硬件友好的稀疏性实现 2-4× 加速
- **部署到资源受限的硬件**（移动设备、边缘设备）
- **无需重新训练即可压缩**，使用一次性剪枝方法
- **实现高效服务部署**，减少内存占用

**关键技术**：Wanda（权重 × 激活值）、SparseGPT（二阶方法）、结构化剪枝、N:M 稀疏性
...
**半结构化（N:M）**：
- 兼具两者优势
- 50% 稀疏度（2:4）→ 在 NVIDIA GPU 上实现 2× 加速
- 准确率损失极小
...
**稀疏度为 50% 时的剪枝方法**（LLaMA-7B）：

| 方法 | 准确率损失 | 速度 | 内存 | 是否需要重新训练 |
|--------|---------------|-------|---------|-------------------|
| **幅值剪枝** | -2.5% | 1.0× | -50% | 否 |
| **Wanda** | -0.8% | 1.0× | -50% | 否 |
...
- **Wanda 论文（ICLR 2024）**：https://arxiv.org/abs/2306.11695
...
- **NVIDIA 稀疏 Tensor Core**：https://developer.nvidia.com/blog/accelerating-inference-with-sparsity-using-ampere-and-tensorrt/