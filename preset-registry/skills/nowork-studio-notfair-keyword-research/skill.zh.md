...
发现、分析并确定 SEO 和 GEO 内容策略的关键词优先级。根据搜索量、竞争程度、搜索意图和业务相关性识别高价值机会。

## 必须触发此技能的情况

当对话涉及以下任一情况时，请使用此技能——即使用户没有使用 SEO 术语：

只要任务需要可复用、能够影响策略的市场情报，而不只是临时性答案，就应使用此技能。
...
2. **意图分类**：按用户意图对关键词进行分类（信息型、导航型、商业型、交易型）
3. **难度评估**：评估竞争程度和排名难度
4. **机会评分**：根据潜在投资回报率确定关键词优先级
5. **聚类**：将相关关键词归入主题集群
...
> **注意：**所有集成都不是必需的。此技能无需任何 API 密钥即可运行——在未连接任何工具时，由用户手动提供数据。

**连接 ~~SEO 工具 + ~~搜索控制台时：**
自动获取历史搜索量数据、关键词难度评分、SERP 分析、来自 ~~搜索控制台的当前排名，以及与竞争对手重叠的关键词。此技能将获取种子关键词指标、相关关键词建议和搜索趋势数据。
...
4. **对搜索意图进行分类**
   ...
   | 意图 | 信号词 | 示例 | 内容类型 |
   |--------|---------|---------|--------------|
   | 信息型 | what、how、why、guide、learn | "什么是 SEO" | 博客文章、指南 |
   | 导航型 | 品牌名称、特定网站 | "google analytics 登录" | 首页、产品页面 |
   | 商业型 | best、review、vs、compare | "[current year] 年最佳 SEO 工具" | 对比文章、评测 |
   | 交易型 | buy、price、discount、order | "购买 SEO 软件" | 产品页面、定价页面 |
...
- [ ] 明确说明每个数据点的来源（~~SEO 工具数据、用户提供的数据或估算数据）

## 示例

> **参考**：有关“小型企业项目管理软件”的完整示例报告，请参阅 [references/example-report.md](references/example-report.md)。
...
1. **从种子关键词入手**，用它们描述你的核心产品或服务
2. **不要忽视长尾关键词**——它们通常具有最高的转化率
3. **让内容匹配意图**——信息型查询需要指南，而非销售页面
4. **将内容组织成集群**，以建立主题权威性
5. **优先处理可快速见效的机会**，以积累势头和可信度
6. **在策略中包含 GEO 关键词**，以提升在 AI 中的可见性
7. **每季度复盘**——关键词动态会随时间发生变化
...
- [示例报告](references/example-report.md) — 面向项目管理软件的完整关键词研究报告示例

## 后续最佳 Skill

- **首选**：[content-writer](../content-writer/SKILL.md) — 将关键词机会转化为经过优化的内容。