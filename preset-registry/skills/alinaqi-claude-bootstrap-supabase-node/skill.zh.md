...
结合 Supabase Auth 和 Drizzle ORM 的 Express/Hono 模式。

**来源：** [Supabase JS 客户端](https://supabase.com/docs/reference/javascript/introduction) | [Drizzle ORM](https://orm.drizzle.team/)

---

## 核心原则

**使用 Drizzle 进行查询，使用 Supabase 进行身份验证和存储，使用中间件进行验证。**

使用 Drizzle ORM 实现类型安全的数据库访问。使用 Supabase 客户端进行身份验证、存储和实时通信。使用 Express 或 Hono 作为 API 层。
...
- **使用 Supabase 客户端执行数据库查询** - 使用 Drizzle
- **同步验证 JWT** - 保持异步
- **没有输入验证** - 使用 Zod 中间件
- **缺少错误处理** - 使用集中式错误处理器
- **硬编码密钥** - 使用环境变量
- **没有请求日志** - 添加 morgan 或 pino
- **阻塞事件循环** - 全程使用异步方式
- **在响应中包含服务密钥** - 绝不暴露