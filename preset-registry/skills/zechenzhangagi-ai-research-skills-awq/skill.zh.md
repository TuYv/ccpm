...
4 位量化方法，基于激活模式保留显著权重，以极小的精度损失实现 3 倍加速。

## 何时使用 AWQ

**在以下情况使用 AWQ：**
- 需要 4 位量化且精度损失 <5%
- 部署指令微调或对话模型（AWQ 的泛化能力更强）
- 希望获得比 FP16 高约 2.5-3 倍的推理加速
- 使用 vLLM 进行生产环境服务
...
**要求**：Python 3.8+、CUDA 11.8+、计算能力（Compute Capability）7.5+
...
| **精度损失** | <5% | ~5-10% | ~5-15% |
| **校准** | 极少（128-1K token） | 较多 | 无 |
| **过拟合风险** | 低 | 较高 | 不适用 |
| **最适合** | 生产推理 | GPU 推理 | 轻松集成 |
| **vLLM 支持** | 原生 | 支持 | 有限 |

**核心洞见**：AWQ 假设并非所有权重同等重要。它会保护由激活模式识别出的约 1% 的显著权重，在不引入混合精度开销的情况下降低量化误差。
...
**限制**：仅支持 batch size 为 1，不适合大上下文。

### Marlin（Ampere 及以上 GPU）
...
**注意**：融合模块无法与 FlashAttention2 同时使用。
