...
**版本说明：**示例针对 **datamol 0.12.x**（PyPI 稳定版：**0.12.5**，2024 年 6 月）。自 0.10.0 起，模块默认采用延迟加载（设置 `DATAMOL_DISABLE_LAZY_LOADING=1` 可禁用）。自 0.12.2 起，RDKit 成为 datamol 在 PyPI 上的直接依赖。指纹使用 RDKit 的 `rdFingerprintGenerator` API（0.12.5+）。
...
| # | 领域 | 涵盖内容 |
| --- | --- | --- |
| 1 | 基本分子处理 | `to_mol`、批量转换、错误处理、规范与异构体 SMILES、规范化和完整标准化 |
| 2 | 文件读写 | SDF、SMILES、CSV、带渲染结构的 Excel、通用读写器，以及云端或 HTTPS 路径 |
| 3 | 描述符与属性 | 标准描述符集、并行计算、芳香性、立体化学、柔性和筛选 |
| 4 | 指纹与相似性 | ECFP4 和其他类型、成对距离和跨集合距离、最近邻查找（Tanimoto 距离 = 1 − 相似度） |
| 5 | 聚类与多样性 | 相似性聚类、多样性子集选取和聚类中心 |
...
| 10 | 化学反应 | 反应 SMARTS，将反应应用于单个分子或整个分子库 |

三个端到端流水线——加载/筛选/分析、按骨架系列进行 SAR 分析和虚拟筛选——位于 [references/workflow_patterns.md](references/workflow_patterns.md) 中。
...
4. **仅在有请求时使用云端 I/O**——确认远程写入路径；根据需要安装 `s3fs`/`gcsfs`：
   ```python
...
Datamol 将 `scipy` 和 `scikit-learn` 作为依赖项一同提供。像普通的 PyPI 包一样导入它们——它们不是此 skill 中捆绑的脚本。
...
**问题**：远程文件访问失败
- **解决方案**：安装匹配的 fsspec 后端（`uv pip install s3fs` 或 `gcsfs`），并确认仅设置了该后端所需的提供商凭据（参见上文的远程文件支持）
