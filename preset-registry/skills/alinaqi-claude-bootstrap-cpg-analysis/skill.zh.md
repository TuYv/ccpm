...
**用途：** 超越 AST 的深度代码分析。使用 Joern 构建完整的代码属性图（控制流、数据流、程序依赖关系），并使用 CodeQL 进行跨过程污点分析和漏洞检测。

**这些是选择启用的工具。** 它们需要 Docker/JVM（Joern）或 CodeQL CLI。日常导航请使用 codebase-memory-mcp（第 1 层，始终启用）。当第 1 层不足以满足需求时，再使用这些工具进行深度分析。
...
## 第 2 层：Joern CPG（CodeBadger MCP）

### 何时使用 Joern

| 场景 | 使用 Joern 的原因 | 第 1 层无法做到的事情 |
|----------|-----------|---------------------|
| 跟踪跨函数的数据流 | 完整的 DDG 遍历 | 第 1 层不具备数据流分析能力 |
| 理解控制流路径 | 带分支条件的 CFG 分析 | 第 1 层不具备 CFG |
| 查找死代码或不可达代码 | PDG 可达性分析 | 第 1 层只能检测未使用的导出 |
| 分析复杂重构的影响 | 跨函数依赖链 | 第 1 层仅限于调用图 |
| 审计第三方库的使用情况 | 深度调用链遍历 | 第 1 层止步于导入边界 |
| 理解异常流 | CFG 包含 throw/catch 路径 | 第 1 层忽略异常 |

### 关键 MCP 工具（Joern/CodeBadger）

| 工具 | 用途 | 查询示例 |
|------|---------|---------------|
| `generate_cpg` | 为项目构建 CPG | 首次设置或发生重大变更后 |
| `get_cpg_status` | 检查 CPG 构建状态 | 查询前验证 CPG 是否已就绪 |
...
| `list_methods` | 列出项目中的所有方法 | 概览可用函数 |
| `get_method_source` | 获取方法的源代码 | 阅读特定函数的源代码 |
| `list_calls` | 列出方法发出或接收的调用 | 调用方/被调用方分析 |
| `get_call_graph` | 完整的调用图可视化 | 理解调用链 |
...
| 场景 | 为何使用 CodeQL | 其他层级无法做到的事情 |
|----------|-----------|--------------------------|
| 发布前的安全审计 | 过程间污点分析 | Joern 仅提供基础污点分析，CodeQL 的分析更深入 |
| 审查身份验证/支付代码 | 从源到汇的数据流 | 跨函数、跨文件的污点分析 |
| PR 安全审查 | 有针对性的漏洞扫描 | 预构建的 OWASP 查询包 |
| 合规性检查 | CWE/OWASP 模式匹配 | 精心编制的安全查询套件 |
...
| 反模式 | 应改为 |
|-------------|-----------------|
| 使用 Joern/CodeQL 进行简单的符号查找 | 使用第 1 层级的 `search_graph`（亚毫秒级，而非数秒） |
| 每次提交时都执行完整的 CPG 构建 | 按需构建 CPG；使用第 1 层级进行持续监控 |
| 未检查 `get_cpg_status` 就查询 Joern | 查询前始终确认 CPG 已构建且为最新状态 |
| 在没有具体安全问题的情况下运行 CodeQL | 先提出假设；CodeQL 查询成本高昂 |
| 深度分析前忽略第 1 层级的影响范围 | 始终先使用第 1 层级确定范围，再深入分析标记的区域 |
| 使用 CodeQL 执行非安全类结构查询 | 使用 Joern CPGQL 执行结构/流查询；使用 CodeQL 执行安全查询 |