...
此扩展基于 [Claude Banana](https://github.com/AgriciDaniel/banana-claude) 构建，后者是适用于 Claude Code 的独立 AI 图像生成 Skill。

此 Skill 包含两个职责不同的组件：
- **SKILL.md**（本文件）：处理用于生成图像的交互式 `/seo image-gen` 命令
- **Agent**（`agents/seo-image-gen.md`）：在 `/seo audit` 期间启动的仅审计分析 Agent，用于评估现有的 OG/社交图像并生成制作计划（绝不会自动生成图像）
...
**检查可用性：**在使用任何图像生成工具之前，请通过检查 `gemini_generate_image` 或 `set_aspect_ratio` 工具是否可用来确认 MCP 服务器已连接。如果工具不可用，请告知用户该扩展尚未安装，并提供安装说明。

## 快速参考

| 命令 | 功能 |
|---------|-------------|
| `/seo image-gen og <description>` | 生成 OG/社交预览图像（呈现 1200x630 的视觉效果） |
...
3. **设置宽高比**，通过 `set_aspect_ratio` MCP 工具完成
4. **构建推理简报**，使用 banana 创意总监流水线：
   - 加载 `references/prompt-engineering.md` 以使用六组件系统
   - 应用领域模式权重（主体 30%、风格 25%、情境 15% 等）
   - 做到具体且富有感官冲击力：描述镜头实际看到的内容
5. **生成**，通过 `gemini_generate_image` MCP 工具完成
6. **生成后的 SEO 检查清单**（见下文）
...
```
加载匹配的预设并将其用作默认设置。同时检查 `references/seo-image-presets.md`
中的 SEO 专用预设模板。

## 生成后 SEO 检查清单

每次成功生成后，就以下事项为用户提供指导：

1. **替代文本**：为生成的图像编写描述清晰、包含丰富关键词的替代文本
2. **文件命名**：重命名为对 SEO 友好的格式：`keyword-description-widthxheight.webp`
3. **WebP 转换**：转换为 WebP 以优化页面速度：
   ```bash
   magick output.png -quality 85 output.webp
   ```
4. **文件大小**：主视觉图像应小于 200KB，缩略图应小于 100KB
5. **Schema 标记**：建议为生成的图像添加 `ImageObject` schema：
   ```json
   {
     "@type": "ImageObject",
...
图像生成需要付费。应保持透明：
- 生成前显示预估成本（尤其是批量生成时）
- 如果可用，在已安装的 Banana MCP/工具账本中记录每次生成
- 如果用户询问使用情况，则使用已安装的 Banana MCP/工具使用摘要

大致成本：
- 报价前，在已安装的 MCP/工具配置中核实当前定价

## 模型路由

| 场景 | 模型 | 原因 |
|----------|-------|-----|
| OG 图像、社交预览图 | 已安装的 MCP/工具默认模型 @ 1K | 速度快、成本效益高 |
| 主视觉图像、产品照片 | 已安装的 MCP/工具质量模型 @ 2K | 兼顾质量与细节 |
| 包含文本的信息图 | 已安装的 MCP/工具文本能力模型 @ 2K，若支持则使用 thinking: high | 文本渲染效果更好 |
| 快速草稿 | 已安装的 MCP/工具草稿模型 @ 512 | 快速迭代 |
...
| MCP 不可用 | 使用 `./extensions/banana/install.sh` 配置 MCP；claude-seo 不附带本地图像生成后备脚本 |
| 扩展未安装 | 显示安装说明：`./extensions/banana/install.sh` |

## 跨技能集成

- **seo-images**（分析）为 **seo-image-gen**（生成）提供输入：`/seo images` 的审计结果会识别缺失或质量较低的图像；使用这些发现来驱动 `/seo image-gen` 命令
- **seo-audit** 会生成 seo-image-gen **代理**（而非此技能），用于分析整个网站的 OG/社交图像并生成按优先级排序的生成计划
