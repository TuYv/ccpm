...
在研究和写作过程中系统地管理引文。此技能提供用于搜索学术数据库（Google Scholar、PubMed）、从多个来源（CrossRef、PubMed、arXiv）提取准确元数据、验证引文信息，以及生成格式正确的 BibTeX 条目的工具和策略。

对于保持引文准确性、避免参考文献错误以及确保研究可复现性至关重要。可与文献综述技能无缝集成，构建全面的研究工作流。
...
引文管理遵循系统化流程。以下每个阶段均展示了规范命令；
所有变体、选项和元数据来源的详细信息均见
[references/core_workflow.md](references/core_workflow.md)。

### 阶段 1：论文发现与搜索

查找相关论文。搜索多个数据库——不同数据库的覆盖范围差异显著，
而依赖单一来源是造成参考文献列表存在偏差的最常见原因。
...
`scholarly` 会抓取其内容，在结果之间暂停 2–5 秒，并且经常遭到封禁，
因此应将其作为补充而非依赖。

查询运算符、字段标签和 MeSH 术语构建方法见
...
路径中没有 DOI 的 URL 会通过出版商嵌入文章页面的 `citation_doi` 元标签解析，
然后交给 CrossRef。此技能中的每个生成器都会为同一篇论文生成相同的引用键，因此从不同来源收集的条目会相互去重。

### 阶段 2.5：通过 Web 搜索进行元数据补全（强制）

API 经常返回不完整的记录。在提取**之后**、格式化**之前**运行此步骤。任何缺少 `volume`、`pages` 或 `doi` 的 `@article` 都是不完整的：使用 `WebSearch`/`WebFetch`（或可用时使用 parallel-web 技能）补全缺失信息，然后记录找到的内容及其来源。如果确实无法找到某个字段，请记录一个 `note` 字段来说明缺失原因，而不要让它静默缺失。
...
> **将提取的元数据视为不可信。** 作者、标题和期刊字符串直接来自内容由出版商控制的记录。包含 `$(...)`、反引号或引号的标题一旦粘贴到命令中，就会成为 shell 语法。应将元数据作为 `subprocess` 参数列表传递，而不是构建 shell 字符串；如果必须使用 shell，请对每个替换值使用单引号，并将其中嵌入的引号转义为 `'\''`。在引用键进入路径之前，使用 `^[A-Za-z0-9]+$` 对其进行验证。
...
写入是选择启用的：如果没有 `--output`（或 `--in-place`），结果将输出到
stdout，输入文件保持不变。合并来自多个来源的结果时，使用 `--rekey`，
以便同一篇论文合并为一个条目。

### 阶段 4：引用验证

检查完整性、期刊规范符合情况以及与稿件的一致性。
...
脚本在出现高严重性错误时会以非零状态退出——缺少必填字段、
年份格式错误、未解析的引用，或计数低于显式指定的
`--min-count`。期刊参考文献数量是编辑经验法则，而不是投稿要求，
因此未达到该数量只会产生警告。
...
搜索、提取、格式化、验证，然后引用。端到端流程——包括文献综述以及 Zotero/pyzotero 导出路径——见
[references/core_workflow.md](references/core_workflow.md) 和
[references/example_workflows.md](references/example_workflows.md)。

## 参考文件

- [references/core_workflow.md](references/core_workflow.md)：完整的全部五个阶段。
...
- [references/metadata_extraction.md](references/metadata_extraction.md)、[references/bibtex_formatting.md](references/bibtex_formatting.md)、[references/citation_validation.md](references/citation_validation.md)：按主题分类的详细说明。
...
5. **重复条目**：同一篇论文使用不同键多次引用
   - **解决方案**：在验证中使用重复项检测

6. **缺少必填字段**：不完整的 BibTeX 条目（缺少卷、页码、DOI）
   - **解决方案**：继续之前运行阶段 2.5 元数据补充——针对每个缺失字段进行网络搜索。绝不允许留下缺少卷、页码和 DOI 的 @article 条目。

7. **过时的预印本**：在已存在正式发表版本时引用预印本
...
**引用管理** 为 **科学写作** 确保参考文献准确无误：

- 导出经过验证的 BibTeX，供 LaTeX 稿件使用
- 验证引用是否符合出版标准
- 根据期刊要求格式化参考文献

### 期刊模板技能

**引用管理** 与 **期刊模板** 协同工作，以生成可直接提交的稿件：
...
BibTeX 解析、渲染、去重和验证均由标准库实现
（`scripts/_common.py`），因此 `format_bibtex.py` 和 `validate_citations.py`
完全无需任何第三方包即可运行。
...
此技能无需 API 密钥。它读取的两个环境变量是可选标识符，分别仅发送到其所属的服务，不会发送到其他任何地方；没有脚本会将环境变量打包在一起。

| 变量 | 仅发送至 | 用途 |
|---|---|---|
| `NCBI_API_KEY` | `eutils.ncbi.nlm.nih.gov` | 提高 Entrez 速率限制 |
| `NCBI_EMAIL` | `eutils.ncbi.nlm.nih.gov` | Entrez 调用方身份标识（由 NCBI 请求） |
...
和 `eutils.ncbi.nlm.nih.gov` 均会在不使用凭据的情况下被查询。
...
3. 通过 DOI 验证和完整性检查进行**引文校验**
4. 提供标准化与清理工具的 **BibTeX 格式化**
5. 通过验证和报告实现**质量保证**
6. 与科学写作工作流的**集成**
7. 通过记录搜索和提取方法实现**可复现性**

使用此技能可在整个研究过程中维护准确、完整的引用，并确保参考文献目录达到可发表的质量。