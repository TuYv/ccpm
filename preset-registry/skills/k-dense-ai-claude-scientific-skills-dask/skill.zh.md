...
Dask 是一个用于并行和分布式计算的 Python 库，提供三项关键能力：
- **超内存执行**，可在单台机器上处理超过可用 RAM 的数据
- **并行处理**，通过多个核心提升计算速度
- **分布式计算**，支持跨多台机器处理 TB 级数据集

Dask 可从笔记本电脑（处理约 100 GiB）扩展到集群（处理约 100 TiB），同时保持熟悉的 Python API。

**当前上游版本：** dask **2026.3.0**（PyPI，2026 年 3 月）。文档：[docs.dask.org](https://docs.dask.org/en/stable/)。自 **2025.1.0** 起，基于表达式且带有查询规划的 DataFrame API 是唯一的实现方式——不要单独安装 `dask-expr`，也不要设置 `dataframe.query-planning: False`。
...
对于使用分布式调度器和仪表板的典型 pandas/NumPy 工作流：
...
需要 **Python 3.10+**（已于 2024.12 停止支持 3.9）。DataFrame I/O 需要 **PyArrow 16+**（自 dask 2026.1.2 起）。
...
**参考文档**：有关 Dask DataFrame 的全面指导，请参阅 `references/dataframes.md`，其中包括：
...
- 处理来自其他来源的结构化数据时，应尽早转换为 DataFrame

### 2. Arrays - 并行 NumPy 操作

**用途**：使用分块算法将 NumPy 功能扩展到大于内存容量的数据集。
...
**参考文档**：有关 Dask Arrays 的全面指南，请参阅 `references/arrays.md`，其中包括：
...
**要点**：
- 分块大小至关重要（目标是每个块约 100 MB）
- 操作会并行处理各个块
- 在需要时重新分块数据，以实现高效操作
- 对于 Dask 中未提供的操作，使用 `map_blocks`

### 3. Bags - 非结构化数据的并行处理

**用途**：通过函数式操作处理非结构化或半结构化数据（文本、JSON、日志）。
...
**参考文档**：有关 Dask Bags 的全面指南，请参阅 `references/bags.md`，其中包括：
- 读取文本和 JSON 文件
...
- 操作采用流式执行且内存高效
- 对于复杂操作，请转换为结构化格式（DataFrame）

### 4. Futures - 基于任务的并行化

**用途**：构建自定义并行工作流，并对任务执行和依赖关系进行细粒度控制。
...
**参考文档**：有关 Dask Futures 的全面指南，请参阅 `references/futures.md`，其中包括：
- 设置分布式客户端
- 提交任务并使用 futures
- 任务依赖关系和数据移动
...
**何时选择调度器**：
- **线程**（默认）：NumPy/Pandas 操作、释放 GIL 的库、可受益于共享内存
- **进程**：纯 Python 代码、文本处理、受 GIL 限制的操作
- **同步**：使用 pdb 调试、性能分析、理解错误
- **分布式**：需要仪表板、多机器集群、高级功能

**参考文档**：如需关于 Dask 调度器的全面指南，请参阅 `references/schedulers.md`，其中包括：
- 详细的调度器说明和特性
- 配置方法（全局、上下文管理器、每次计算）
- 性能考量和开销
- 常见模式和故障排除
...
**关键要点**：
- 线程：开销最低（~10 µs/任务），最适合数值计算工作
- 进程：避免 GIL（~10 ms/任务），最适合 Python 工作
- 分布式：提供监控仪表板（~1 ms/任务），可扩展到集群
- 可以针对每次计算或全局切换调度器

## 最佳实践

如需全面了解性能优化指南、内存管理策略以及应避免的常见陷阱，请参阅 `references/best-practices.md`。关键原则包括：

### 从更简单的解决方案开始
在使用 Dask 之前，先探索：
- 更好的算法
- 高效的文件格式（使用 Parquet 而非 CSV）
- 编译型代码（Numba、Cython）
- 数据采样
...
**4. 选择合适的块大小**
- 目标：每个块约 ~100 MB（或每个核心在 worker 内存中有 10 个块）
- 过大：内存溢出
- 过小：调度开销
...
### 与其他库结合使用
- **XArray**: 使用带标签的维度封装 Dask 数组（地理空间、成像）
...
当用户需要有关特定 Dask 组件、操作或模式的详细信息，而不仅仅是此处提供的快速指导时，加载这些文件。