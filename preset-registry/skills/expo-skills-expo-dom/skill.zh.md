...
DOM 组件允许 Web 代码在原生平台的 WebView 中原样运行，同时在 Web 平台上保持原样渲染。借助这一功能，你可以在 Expo 应用中直接使用 `recharts`、`react-syntax-highlighter` 等仅限 Web 的库，或任何 React Web 库，而无需进行修改。

## 何时使用 DOM 组件

在以下情况下使用 DOM 组件：

- **仅限 Web 的库** — 图表库（recharts、chart.js）、语法高亮器、富文本编辑器，或任何依赖 DOM API 的库
- **迁移 Web 代码** — 无需重写即可将现有 React Web 组件引入原生应用
- **复杂的 HTML/CSS 布局** — 当 React Native 不支持所需的 CSS 功能时
...
- **简单 UI** — 对于基础布局，React Native 组件的效率更高
...
2. **单个默认导出** — 每个文件只能有一个 React 组件
3. **使用独立文件** — 不能内联定义，也不能与原生组件组合在同一个文件中
4. **仅支持可序列化的 props** — 字符串、数字、布尔值、数组和普通对象
5. **在组件文件中包含 CSS** — DOM 组件在隔离的上下文中运行

## `dom` Prop

每个 DOM 组件都会接收一个用于配置 WebView 的特殊 `dom` prop。请始终在 props 中为其声明类型：
...
由于 DOM 组件在隔离的上下文中运行，因此 CSS 导入必须位于 DOM 组件文件中：
...
expo-router 的 `<Link />` 组件和 router API 可以在 DOM 组件中使用：
...
**解决方案：**在原生父组件中读取这些值，并通过 props 传递：
...
| Web     | 原样渲染（无 webview 包装器）     |

在 Web 上，由于不需要 webview，因此会忽略 `dom` prop。

## 提示

- DOM 组件在开发期间支持热重载
- 保持 DOM 组件功能聚焦——不要将整个屏幕都放入 webview
- 使用原生组件实现导航界面元素，使用 DOM 组件实现专用内容
- 在所有平台上进行测试——Web 渲染可能与原生 webview 略有不同
- 大型 DOM 组件可能会影响性能——如有需要，请进行性能分析
- webview 拥有自己的 JavaScript 上下文——无法与原生环境直接共享状态
