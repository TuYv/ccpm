...
- 作出或自动执行患者特异性的临床决策；
- 支持床旁、即时医疗点或实时临床操作；
- 取代专业判断或经过验证且获授权的临床系统；
- 声称获得 FDA 授权、符合监管要求、符合 HIPAA 或符合法律要求。

如果请求可能影响某个人的医疗照护，请停止工作流程，并使用经当地验证且获得适当授权的系统，将该事项转交给持证医疗专业人员。不得将患者特异性照护事项转交给其他技能。
...
输出在获得合格人员批准前均为草稿。报告指南有助于提高透明度；但它不能确立研究质量、临床效用、安全性、有效性、授权或合规性。
...
1. 确认输入为合成数据或汇总数据。
2. 拒绝患者行、记录、叙述、标识符、自由文本、与个人相关联的日期、图像、波形或基因组序列。
3. 将源文件保留在本地。不得获取 URL、调用 API、读取环境变量或向模型发送数据。
4. 在生成表格之前设置披露阈值。
5. 记录数据来源、数据截点日期、研究人群、排除项、缺失情况和转换过程。

这些脚本会限制文件大小、分组数、行数和文本长度。它们会拒绝类似 URL 的路径和常见的行级键。这些控制措施可减少意外误用；但它们并不构成隐私判定。
...
- 预先规定结局、时间起点、时间范围、亚组、截点、缺失数据处理、多重性以及敏感性分析。
- 将探索性发现与验证性分析分开。
...
- 由领域专家负责临床科学背景；
- 由隐私官或合格专家负责披露决策；
- 由监管或法律顾问负责特定司法管辖区的解释；
- 由人因专家负责用户研究；
- 由获授权的治理负责人负责发布和变更控制。

脚本执行成功仅表示所声明的字段和内部一致性检查均已通过。

## GRADE 证据概况

不要根据文章正文、单独的研究设计、p 值或关键词推断证据确定性评级。不要将传统的 `1A/2B` 简写当作通用的 GRADE 输出。
...
评估器仅接受汇总混淆计数和校准分箱。它报告带有 Wilson 区间的有界描述性指标、校准差距、亚组差异以及明确的抑制措施。它不确定公平性、临床效用或适用性。要求：

- 锁定的模型/检测/版本以及预先指定的阈值来源；
- 具有代表性的内部验证和独立的外部验证；
- 适合目标的校准和判别能力；
- 包含不确定性和样本量的亚组表现；
...
- 报告分母和缺失性。
- 避免将基线显著性检验作为平衡诊断。
- 标注调整后、未调整、预先指定和探索性结果。
- 不得将关联解释为因果关系或临床可操作性。

默认阈值是一项操作性保障措施，而非 HIPAA 规则或保证。参见 `references/cohort_evaluation.md` 和 `references/privacy_and_disclosure.md`。

## 生存分析计划

共同定义时间零点、事件、竞争事件、删失、事件间事件、估计目标、时间范围、效应量度和分析人群。

- 在将风险比视为恒定之前，评估比例风险假设。
- 预先指定替代方案，例如时变效应或限制平均生存时间。
- 当竞争事件具有重要性时，使用累积发生率方法。
- 处理不死时间、信息性删失、延迟入组、缺失数据和多重性风险。
- 纳入敏感性分析和不确定性，而不仅仅是 p 值。

捆绑的辅助工具会验证计划；它不分析生存数据。参见 `references/survival_analysis.md`。

## 决策逻辑

仅记录研究或治理逻辑，例如证据纳入、验证门槛、发布搁置和人工审查检查点。每个节点都必须链接到来源 ID、测试、负责人、版本和状态。

不得编码照护路径、紧急程度、用药操作、诊断规则、警报或面向患者的输出。参见 `references/decision_logic_traceability.md`。

## 隐私和去标识化

HHS 方法包括专家判定和安全港。检查清单本身无法执行其中任一方法。不得声称移除字段列表、对标识符进行哈希处理、使用最小单元格大小或通过此脚本即可证明去标识化或 HIPAA 合规性。

该辅助工具盘点已记录的人工工作。它绝不读取数据集。将未解决事项、自由文本、日期、地理信息、罕见组合、链接风险、基因组学和纵向模式升级交由合格的隐私审查。
...
- AI 试验方案：SPIRIT-AI 和现行 SPIRIT 基础声明。
- AI 随机试验报告：CONSORT-AI 和现行 CONSORT 基础声明。
- 早期真实环境 AI 评估：DECIDE-AI——但真实环境评估超出本技能的执行范围。

这些是报告或评估工具，并非自动质量评分。参见 `references/study_reporting.md`。

## 监管与治理背景

FDA 器械状态取决于预期用途和功能，而非文档标签。FDA 2026 年 1 月的 CDS 指南区分了某些非器械 CDS 功能与器械软件功能；其示例并非自我认证核对清单。ONC HTI-1 要求适用于定义的认证范围内。ICH E6(R3) 和 E9/E9(R1) 为试验治理和统计规划提供参考，但不会使某项产物自动合规。

请使用 `references/regulatory_and_governance.md` 获取具有日期标识的背景信息。对于实际产品、研究、申报、部署或司法管辖区，请获取合格的专业建议。
...
- `references/survival_analysis.md` — 事件发生时间规划
...
- `references/sources.md` — 具有日期标识的权威来源台账
- `references/security_validation.md` — 扫描结果和已接受的 LOW 发现