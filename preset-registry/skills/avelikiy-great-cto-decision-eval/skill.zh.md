...
在架构师提出 2 个或更多方案之后、创建 gate:arch 之前调用。

## 调用时机

当以下所有条件均成立时，调用此技能：

1. ADR (`docs/decisions/ADR-*.md`) 或 ARCH 文档 (`docs/architecture/ARCH-*.md`)
   中包含列出 2 个或更多具名备选方案的章节（查找
   `## Alternatives Considered`、`## Options`，或以加粗文本开头的选项，例如
   `**Option A:**`）
2. 架构师尚未创建 `gate:arch`
3. 用户未表示 "skip scoring"、"no scoring" 或 "skip decision-eval"
4. PROJECT.md 中的 `project_size` 不是 `nano`

如果任一条件不满足，则静默跳过（甚至不要提及）。

## 调用方式

读取最新的 ADR 或 ARCH 文档，确认存在 2 个或更多方案，然后将文件路径作为上下文启动
`decision-scorer` 代理：
...
decision-scorer 代理将结果保存至：
```
docs/decisions/DECISION-<slug>-<YYYYMMDD>.md
```

代理完成后，读取输出文件并向架构师展示建议：
...
如果出现以下情况，则不输出任何内容并继续下一步：
- PROJECT.md 中存在 `project_size: nano`
- 在 ADR/ARCH 文档中找到的方案少于 2 个
- 用户消息包含 "skip scoring"、"skip decision-eval" 或 "no scoring"
- 目标文档是仅修复缺陷或仅修改文档的 ADR（检查标题中是否包含 "fix:"、"docs:"、"chore:"）

## 与架构师工作流的集成

此技能位于 `agents/architect.md` 中的第 4 步（编写 ADR）与第 5 步（创建 gate:arch）之间。架构师按名称调用它：
