...
用于具有实时数据、离线支持和安全规则的 Web 与移动应用的 Firebase/Firestore 模式。
...
**有目的地反规范化，使用规则保障安全，进行横向扩展。**

Firestore 是一种文档数据库——应采用反规范化来提高读取效率。安全规则就是你的服务端验证。应围绕访问模式进行设计。
...
- **没有安全规则** - 始终编写规则，永远不要在生产环境中使用测试模式
- **嵌套过深** - 保持文档扁平，最多 2-3 层
- **文档过大** - 最大 1MB，超过时进行拆分
- **无界数组** - 对于会不断增长的列表，使用子集合
- **没有离线处理** - 为移动端/PWA 启用持久化
- **读取所有字段** - 使用字段掩码或 Firestore Lite
- **忽略索引** - 检查控制台中是否存在缺失索引错误
- **不使用模拟器测试** - 部署前始终测试规则