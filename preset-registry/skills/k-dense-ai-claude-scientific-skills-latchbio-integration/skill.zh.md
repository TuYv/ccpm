...
此技能面向 **Latch SDK 2.76.8**，该版本于 2026 年 7 月 10 日发布。软件包
元数据支持 Python 3.9–3.12，并声明支持 Python 3.9+。

当指南与 SDK 不一致时，应以已安装的软件包及其变更日志为准。部分 Latch 指南仍保留较旧的 Python 版本范围或特定于兼容性的预发布版本固定，尤其是 Snakemake v2 教程。
在检查版本要求之前，切勿混用不同版本轨道中的命令或导入。
...
- 使用 `latch register --staging` 和 `latch develop` 预置并调试工作流镜像
...
`scripts/inspect_latch_sdk.py`。该脚本仅执行本地导入，不会进行身份验证或
发起网络请求。
...
在 Windows 上，请使用 WSL 来运行文档所述的 Linux 工作流工具。
...
当已知工作区的数字 ID 时，可以通过非交互方式选择工作区：

```bash
latch workspace --id 12345
```

`latch login` 凭据用于 SDK 和 CLI。Latch MCP 使用单独的
...
远程镜像构建是默认方式。仅当本地 Docker 守护进程可用且确实需要本地构建时，才使用 `--no-remote`。

## 最小 Python 工作流

保持工作流主体具有声明性：调用任务并返回其 promise。
在任务内部执行计算和副作用操作。
...
当生成的界面需要自定义标签、分区、验证规则、样本表或文档链接时，使用 `@workflow(metadata)`。使用 `LatchFile` 或
`LatchDir` 自动暂存任务输入并上传输出；使用 `LPath` 执行命令式远程路径操作。

## 推荐的开发生命周期

1. **检查兼容性**
   - 确认已安装的 SDK 和 Python 版本。
   - 确定项目属于 Python、Nextflow、旧版 Snakemake 标志路径，还是单独固定版本的 Snakemake v2 教程路线。

2. **定义类型化接口**
   - 为每个工作流和任务的输入、输出添加类型注解。
   - 确保模块导入时不执行网络调用、数据变更和机密信息获取。将 `workflow_reference` 等文档中说明的例外隔离出来；该对象会在其装饰器求值时解析活动工作区。
   - 对结构化参数使用数据类和枚举。

3. **配置元数据和资源**
   - 使元数据参数键与工作流签名保持一致。
   - 从具名任务装饰器开始，仅在经过测量的需求证明合理时使用 `custom_task`。

4. **在执行镜像中验证**

   全新的 Nextflow 和 Snakemake 项目必须在暂存前生成与版本兼容的 Python 入口点。在 SDK 2.76.8 中，暂存分支不会根据 `--nf-script` 或 `--snakefile` 生成入口点。
...
6. **仅在检查成本和参数后启动**
   - 交互式操作优先使用 Console 或 Latch MCP。
   - Python 自动化优先使用 `latch_cli.services.launch.launch_v2`。
   - 不要将已弃用的 `latch launch` CLI 用作新的集成模式。

7. **监控并验证**
   - 检查终端状态、任务日志、结果链接和科学输出。
   - 将编排成功视为必要但不足以完成科学验证的条件。

## 操作安全

- 在启动付费计算任务前，尤其是 GPU 或大型批处理任务，先请求确认。
- 在执行 `LPath.rmr`、`latch rmr`、Registry 删除操作或覆盖共享目标之前，先请求确认。
- 切勿记录机密信息、SDK 令牌、签名 URL 或机密值。
- 仅在任务内部调用 `get_secret()`，返回值只能用于其预期服务，绝不能将其作为工作流输出返回。
- 不要通过 shell 命令传递不受信任的字符串。优先使用带有 `subprocess.run(..., check=True)` 的参数列表。
- 为发布版本固定 SDK 和工作流依赖。只有在查看变更日志并重新运行预发布测试后，才能升级。
- 将生成的文件视为生成产物：应修改文档中说明的扩展文件，而不是编辑 CLI 将要覆盖的输出文件。
