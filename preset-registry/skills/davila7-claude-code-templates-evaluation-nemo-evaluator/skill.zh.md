...
NeMo Evaluator SDK 使用容器化、可复现的评估方式，通过多后端执行（本地 Docker、Slurm HPC、Lepton 云），在来自 18 种以上评估框架的 100 多个基准测试中评估 LLM。
...
- 需要**可复现的**容器化评估
- 针对**兼容 OpenAI 的 API**（vLLM、TRT-LLM、NIMs）进行评估
- 需要支持结果导出（MLflow、W&B）的**企业级**评估

**改用替代方案：**
- **lm-evaluation-harness**：设置更简单，适合快速本地评估
- **bigcode-evaluation-harness**：仅专注于代码基准测试
- **HELM**：斯坦福推出的范围更广的评估工具（公平性、效率）
- **自定义脚本**：高度专业化的领域评估
...
**多后端执行**：参阅 [references/execution-backends.md](references/execution-backends.md)
**配置详解**：参阅 [references/configuration.md](references/configuration.md)
**适配器和拦截器系统**：参阅 [references/adapter-system.md](references/adapter-system.md)
**自定义基准测试集成**：参阅 [references/custom-benchmarks.md](references/custom-benchmarks.md)

## 要求

- **Python**：3.10-3.13
- **Docker**：本地执行时必需
- **NGC API Key**：用于拉取容器和使用 NVIDIA Build
- **HF_TOKEN**：某些基准测试（GPQA、MMLU）需要此项
