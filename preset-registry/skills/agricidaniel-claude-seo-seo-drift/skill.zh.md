...
SEO 的 Git。捕获基线、检测回归、跟踪一段时间内的变更。
...
- **comparisons**：包含已触发规则和严重性的差异结果

URL 规范化可确保匹配的一致性：方案/主机名转换为小写，移除
...
5. 对 HTML 正文和 schema 内容计算哈希（SHA-256）
6. 将快照存储到 SQLite
...
**输出：**包含所有触发规则、旧值/新值、严重程度和操作的 JSON。
...
| CWV 获取失败（无 API 密钥） | 将 CWV 字段存储为 `null`。比较期间跳过 CWV 规则。 |
| 页面返回 4xx/5xx | 仍然捕获为基线（状态码本身就是一个受跟踪字段）。 |
...
- **所有 URL 获取操作**都通过 `"${CLAUDE_PLUGIN_ROOT}/scripts/claude-seo" run fetch_page.py`，该命令会强制执行 SSRF 防护
  （阻止私有 IP、回环地址、保留地址范围和 GCP 元数据端点）
- **不使用 curl，不使用 subprocess HTTP 调用** -- 仅使用项目经过验证的获取管道
- **所有 SQLite 查询**都使用参数化占位符（`?`），绝不进行字符串插值
- **始终验证 TLS** -- 管道中的任何位置都不得使用 `verify=False`
