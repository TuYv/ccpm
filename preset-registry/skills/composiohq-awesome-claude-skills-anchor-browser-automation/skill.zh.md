---
name: anchor-browser-automation
description: "Automate Anchor Browser tasks via Rube MCP (Composio). Always search tools first for current schemas."
requires:
  mcp: [rube]
---
# 通过 Rube MCP 的 Anchor Browser 自动化

通过 Rube MCP 使用 Composio 的 Anchor Browser 工具包自动化 Anchor Browser 操作。

**工具包文档**: [composio.dev/toolkits/anchor_browser](https://composio.dev/toolkits/anchor_browser)

## 先决条件

- Rube MCP 必须已连接（RUBE_SEARCH_TOOLS 可用）
- 通过 `RUBE_MANAGE_CONNECTIONS` 使用工具包 `anchor_browser` 建立并保持 Anchor Browser 连接
- 始终先调用 `RUBE_SEARCH_TOOLS` 以获取当前工具架构

## 设置

**获取 Rube MCP**: 在客户端配置中将 `https://rube.app/mcp` 添加为 MCP 服务器。无需 API key，只需添加该端点即可使用。

1. 通过确认 `RUBE_SEARCH_TOOLS` 有响应来验证 Rube MCP 可用
2. 调用 `RUBE_MANAGE_CONNECTIONS` 并指定工具包 `anchor_browser`
3. 若连接未处于 ACTIVE，请按照返回的授权链接完成设置
4. 在运行任何工作流前确认连接状态显示为 ACTIVE

## 工具发现

在执行工作流前始终先发现可用工具：

```
RUBE_SEARCH_TOOLS
queries: [{use_case: "Anchor Browser operations", known_fields: ""}]
session: {generate_id: true}
```

该调用返回可用的工具 Slug、输入架构、推荐执行计划以及已知陷阱。

## 核心工作流模式

### 第 1 步：发现可用工具

```
RUBE_SEARCH_TOOLS
queries: [{use_case: "your specific Anchor Browser task"}]
session: {id: "existing_session_id"}
```

### 第 2 步：检查连接

```
RUBE_MANAGE_CONNECTIONS
toolkits: ["anchor_browser"]
session_id: "your_session_id"
```

### 第 3 步：执行工具

```
RUBE_MULTI_EXECUTE_TOOL
tools: [{
  tool_slug: "TOOL_SLUG_FROM_SEARCH",
  arguments: {/* schema-compliant args from search results */}
}]
memory: {}
session_id: "your_session_id"
```

## 已知陷阱

- **始终先搜索**：工具架构会变化。未调用 `RUBE_SEARCH_TOOLS` 前，切勿硬编码工具 slug 或参数
- **检查连接**：执行工具前，请确认 `RUBE_MANAGE_CONNECTIONS` 显示 ACTIVE 状态
- **架构一致性**：使用与搜索结果完全一致的字段名和类型
- **memory 参数**：在 `RUBE_MULTI_EXECUTE_TOOL` 调用中始终包含 `memory`，即使为空也要传 `{}`
- **会话复用**：在同一工作流内复用会话 ID；为新工作流生成新的会话 ID
- **分页**：检查响应中的分页 token，并持续拉取直到完成

## 快速参考

| 操作 | 方式 |
|-----------|----------|
| 查找工具 | 使用 Anchor Browser 特定 use case 的 `RUBE_SEARCH_TOOLS` |
| 连接 | 使用工具包 `anchor_browser` 的 `RUBE_MANAGE_CONNECTIONS` |
| 执行 | 使用已发现的工具 slug 调用 `RUBE_MULTI_EXECUTE_TOOL` |
| 批量操作 | 使用 `RUBE_REMOTE_WORKBENCH` 和 `run_composio_tool()` |
| 完整架构 | 使用 `RUBE_GET_TOOL_SCHEMAS` 获取带有 `schemaRef` 的工具 |

---
*由 [Composio](https://composio.dev) 提供支持*
