...
结合 Supabase Auth 与 SQLAlchemy/SQLModel 进行数据库访问的 FastAPI 模式。

**来源：** [Supabase Python 客户端](https://supabase.com/docs/reference/python/introduction) | [SQLModel](https://sqlmodel.tiangolo.com/)

---

## 核心原则

**使用 SQLAlchemy/SQLModel 进行查询，使用 Supabase 进行身份验证和存储。**

使用 SQLAlchemy 或 SQLModel 实现类型安全的数据库访问。使用 supabase-py 实现身份验证、存储和实时功能。使用 FastAPI 作为 API 层。
...
- **使用 Supabase 客户端执行数据库查询** - 使用 SQLAlchemy/SQLModel
- **同步数据库调用** - 通过 asyncpg 使用异步调用
- **硬编码凭据** - 使用环境变量
- **未使用连接池** - asyncpg 会处理此问题
- **缺少身份验证依赖项** - 始终验证 JWT
- **未关闭会话** - 使用上下文管理器
- **在异步代码中执行阻塞式 I/O** - 使用异步库