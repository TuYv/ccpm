...
Amazon Aurora 是一种兼容 MySQL/PostgreSQL 的关系数据库，支持无服务器扩展、高可用性和企业级功能。
...
**无服务器架构使用 RDS Proxy，追求简单时使用 Data API，始终使用连接池。**

Aurora 擅长处理符合 ACID 的工作负载。对于无服务器架构（Lambda），始终使用 RDS Proxy 或 Data API 来处理连接管理。绝不要从 Lambda 函数中直接建立原始连接。
...
| 选项 | 最适合的场景 |
|--------|----------|
| **Aurora Serverless v2** | 工作负载变化大，自动扩展（0.5-128 ACUs） |
| **Aurora Provisioned** | 工作负载可预测，追求最高性能 |
| **Aurora Global** | 多区域部署，灾难恢复 |
...
### 通过 Proxy 连接（TypeScript/Node.js）
```typescript
// lib/db.ts
import { Pool } from 'pg';
...
- **直接建立 Lambda→Aurora 连接** - 始终使用 RDS Proxy 或 Data API
- **不限制连接数** - 为 Lambda 设置 `max: 1`，服务器端使用连接池
- **忽略冷启动** - Serverless v2 需要时间进行扩展；生产环境应保持最低 ACUs
- **没有只读副本** - 对于高负载工作负载，将读取操作卸载到只读副本
- **缺少 IAM 身份验证** - 在可能的情况下，使用 IAM 而不是静态密码
- **没有重试逻辑** - 处理扩展或故障转移产生的瞬态错误
- **容量配置过高** - 对于工作负载变化较大的场景，使用 Serverless v2
