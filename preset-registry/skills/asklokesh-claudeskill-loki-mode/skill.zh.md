...
**规范输入，经验证的产品输出。** 规范驱动：描述工作内容的任何材料都属于“规范”——可以是 Markdown PRD、GitHub issue、OpenAPI 文档、Jira 工单（PRD 只是规范的一种形式）。Loki 的差异化能力在于信任层：只有在验证通过后，Loki 才会认定工作已完成。RARV-C 闭环、8 个质量门、完成委员会以及已验证完成证据门都必须通过，才能接受完成状态。证据门会因以下情况阻止完成：差异为空、测试失败、应用无法健康地启动并提供服务（运行时启动轴，可通过 `LOKI_EVIDENCE_BOOT_GATE=0` 选择退出），以及变更文件中泄露凭据（秘密泄露轴，可通过 `LOKI_EVIDENCE_SECRET_GATE=0` 选择退出）——v8.0.0。

**证据收据（请自行验证）。** 每次运行都会将收据写入 `.loki/proofs/<run_id>/`（可通过 `LOKI_PROOF=0` 选择退出），并将确定性 FACTS（包含基准/当前提交 SHA 和 `diff_sha256` 的 git diff、测试命令及退出码、构建命令及退出码、每个门的判定）与 AI ASSESSMENTS（委员会判定，属于已标注的判断而非证明）分开。标题仅根据事实计算：VERIFIED（测试执行了真实命令且退出码为 0、差异非空、没有跳过任何检查）、VERIFIED WITH GAPS（逐项列出每个缺口），或 NOT VERIFIED（某项检查已运行但失败）。使用 `loki proof list|show <id>|verify <id>`（别名为 `loki receipt`）进行检查和重新验证；`loki proof verify` 会重新计算收据哈希（篡改检测），并根据记录的基准 SHA 对实时仓库重新推导差异（漂移检测），干净时退出码为 0，篡改或漂移时退出码为 1。这体现的是完成状态的诚实性，并不表示代码绝对没有缺陷。

**与提供商无关（自 v5.0.0 起稳定）：** 可在 Claude/Codex/Cline/Aider 上运行，支持抽象模型层级，并为非 Claude 提供商提供降级模式；不存在供应商锁定。Gemini 已在 v7.5.18 中弃用。参见 `skills/providers.md`。**当前路线（v8.0.0）：** Anthropic Agent SDK 路径（见下文）、针对 OpenAPI/GraphQL/Postman 契约的规范模式扩展、运行时启动和秘密泄露证据轴，以及用于运行中控制的 `loki steer` / `loki why`。更早的路线：将 LSP grounding 作为一等代理工具（v7.7.x），以及 Phase 1 RARV-C 闭环（真实提供商裁判、门失败 flock、合成 PRD e2e、`status --json`）。

**运行时迁移：** 从 Bash 迁移到 Bun。只读命令（`version`、`status`、`stats`、`doctor`、`provider show/list`、`memory list/index`）自 v7.3.0 起通过 `bin/loki` 使用 Bun 运行时。所有其他命令仍使用 Bash 运行时（`autonomy/loki`）。回滚方式：`LOKI_LEGACY_BASH=1`。参见 `UPGRADING.md` 和 `docs/architecture/ADR-001-runtime-migration.md`。

**Anthropic Agent SDK 路径（v8.0.0，可选择启用，默认关闭）：** 一条无需 claude-binary 的路径，其中 RARV 循环通过 `@anthropic-ai/claude-agent-sdk` 的 `query()` 运行，裁判则使用原始的 `@anthropic-ai/sdk` 运行。通过一个操作员开关 `LOKI_SDK_MODE` 控制（默认 `off` / `judges` / `full`），并在 bash（`autonomy/lib/sdk-mode.sh`）和 TypeScript（`loki-ts/src/runner/sdk_mode.ts`）中逐字节镜像实现。未设置时，与 claude-CLI 路径保持逐字节一致。参见 `references/sdk-mode.md`。
...
| 规则 | 含义 |
|------|------|
| **做出决定并执行** | 自主做出决定。不要向用户提问。 |
| **保持推进** | 不要等待确认。继续执行下一项任务。 |
| **持续迭代** | 总有下一项改进。主动寻找改进点。 |
| **始终验证** | 没有测试的代码是不完整的。运行测试。**绝不要忽略或删除失败的测试。** |
| **始终提交** | 每项任务完成后进行原子提交。创建进度检查点。 |
| **测试不可侵犯** | 如果测试失败，修复代码，不要删除或跳过测试。通过的测试套件是硬性要求。 |
...
| 代码审查（计划使用 3 个并行审查器） | **开发** | opus | sonnet | effort=high |
| 集成测试、E2E、部署 | **开发** | opus | sonnet | effort=high |
| 单元测试、lint、文档、简单修复 | **快速** | sonnet | haiku | effort=low |

**并行化规则（仅限 Claude）：** 对于相互独立的任务，最多同时启动 10 个代理。

**降级模式（Codex/Cline/Aider）：** 不使用并行代理或 Task 工具。Codex 支持 MCP。按顺序运行 RARV 循环。请参阅 `skills/model-selection.md`。

**Git worktree 并行：** 如需真正的并行功能开发，请将 `--parallel` 标志与 run.sh 一起使用。请参阅 `skills/parallel-workflows.md`。

**规模化模式（50+ 个代理，仅限 Claude）：** 使用评审代理、递归子规划器和乐观并发。请参阅 `references/cursor-learnings.md`。
...
ARCHITECTURE ──[设计已批准，规格已编写]──> DEEPEN_PLAN（仅标准/复杂项目）
DEEPEN_PLAN ──[计划已由 4 个研究代理增强]──> INFRASTRUCTURE
INFRASTRUCTURE ──[云/数据库已就绪]──> DEVELOPMENT
...
- **上下文窗口跟踪（v5.40.0）：** 可通过 `GET /api/context` 获取仪表盘、时间线和每个代理的明细
...
| `.loki/state/checkpoints/` | 任务完成后 | 自动及通过 `loki checkpoint` 手动创建 |

一条命令即可回滚（v7.5.2+）：`loki rollback latest` 或 `loki rollback to <id>` 可从检查点恢复 `.loki/` 状态。它会先捕获当前状态的强制回滚前快照并打印其 id，因此回滚操作本身也可以撤销（`loki rollback to <that-id>`）。使用 `loki rollback list` 查看检查点。

---

## 模块加载协议（技能）

此协议规定 **技能模块** 的加载方式，即位于 `skills/` 中、作用域限定于任务的指令文件。它与下方的记忆系统渐进式披露机制不同，后者管理 `.loki/memory/` 中的持久化 **记忆层**。
...
**记忆系统渐进式披露** 是一个独立的三层结构（`index.json` -> `timeline.json` -> `episodic/*.json`），用于检索过去的事件和模式。请参阅 `skills/memory.md` 和 `references/memory-system.md`。
...
- **Claude**：Opus 4.6，1M 上下文（beta），128K 输出，自适应思考，代理团队，完整功能（Task tool、并行代理、MCP）
- **Codex**：GPT-5.3，400K 上下文，128K 输出，支持 MCP，`--full-auto` 模式，受限（仅支持顺序执行，不支持 Task tool）
- **Cline**：多提供商 CLI，受限模式（仅支持顺序执行，不支持 Task tool）
- **Aider**：支持 18+ 个提供商后端，受限模式（仅支持顺序执行，不支持 Task tool）
...
| `loki why` | 解释当前结果；发生停滞时，会说明实际停滞原因并建议使用 `loki steer`（v8.0.0） |
...
| 类型 | 文件 | 行为 |
|------|------|----------|
| **指令** | `.loki/HUMAN_INPUT.md` | 活跃指令（需要 `LOKI_PROMPT_INJECTION=true`） |
...
与 Claude Managed Agents 的可选集成（于 2026 年 4 月发布）。为 Loki 提供跨项目的经审计记忆和真正的多代理委员会。相关功能已**内置**于现有的 RARV-C 和委员会流程中，无需学习新命令。
...
| `LOKI_MANAGED_MEMORY` | 将 `.loki/memory/` 中的内容从 REASON 增强和 REFLECT 阶段影子写入 Managed Agents 存储 | 稳定（已使用模拟对象测试） |
...
快速失败：子级开启 + 父级关闭时以 2 退出，并显示清晰错误。API 不可达时回退到本地路径，并向 `.loki/managed/events.ndjson` 写入 `managed_agents_fallback` 事件。不会产生重试风暴。
...
**未经真实 Anthropic API 测试。** 自动化 CI 使用 `memory/managed_memory/fakes.py`。Beta header 固定为 `managed-agents-2026-04-01`。如果 SDK 结构不同，调用会引发 `AttributeError`/`TypeError`，这些异常会被捕获并转换为 `ManagedUnavailable`，然后回退到本地路径。
...
| 环境变量 | 作用 |
|---------|--------|
| `LOKI_INJECT_FINDINGS=true` | 将评审委员会发现的问题和门禁失败注入下一条 REASON 提示 |
| `LOKI_OVERRIDE_COUNCIL=true` | 在可用时使用真实提供商评审器替代虚拟评审器 |
...
- **留出集规格评估：**约 25% 的检查清单项（按确定性的 `sha256(id)` 顺序，`N >= 4`）会被保留到 `.loki/checklist/held-out.json` 中，并从构建提示输入中排除；如果留出项失败，完成评审委员会会阻止完成。使用 `LOKI_HELDOUT_GATE=0` 可选择退出。诚实限制：此功能保护的是提示输入，而不是沙箱；保留文件位于磁盘上，拥有文件系统访问权限的智能体可以读取它。
- **无法得出基线时的披露：**当证据门禁无法建立差异基线（`no_git_repo` / `no_run_start_sha`）时，会写入 `.loki/state/evidence-inconclusive.json`，并且 `COMPLETION.txt` 会包含诚实的“未经独立验证”说明。它不会阻止非 git 项目；失败测试仍会阻止完成。

## Harness 智能（v8.0.0）

在现有信任核心之上叠加了四项经过测量的 harness 规范。这些规范都不会削弱任何门禁：每项功能要么增加验证，要么节省无法成功的工作所消耗的预算。
...
| `LOKI_GOAL_SCORING=0` | 开启 | 禁用目标可衡量性提示 |
| `LOKI_SMART_RETRY=0` | 开启 | 重试每一次失败，包括不可重试的失败 |
| `LOKI_SIMPLE=1` | 关闭 | 移除系统提示中的指导部分（每次迭代减少 78%，约 1562 个 token）。实验性消融组。 |

- **Prompt-cache discipline.** 提示词被拆分为缓存稳定的
  `<loki_system>` 前缀和易变的 `<dynamic_context>` 尾部，中间以显式的
  `[CACHE_BREAKPOINT]` 分隔；SDK judge 路径会在该拆分处应用 `cache_control`。
  任何新的常驻指令都应放入前缀，否则每次迭代都会使缓存失效。
- **Confidence-spike re-check.** 自报告的置信度突然跃升至接近最大值时，在 done-signal valve 强制停止运行前，必须额外执行 ONE 次验证。
  严格为增量行为：spike 只能 ADD 一次验证，不得跳过、缩短或满足任何 gate。它不能延迟 stagnation valve，且该延迟只执行一次，因此反复出现 spike 的运行无法无限期推迟该 valve。
- **Hill-climbable goal scoring.** 没有可度量目标的 `COMPLETION_PROMISE`（没有数字、比较运算符、命名指标或可验证产物）会触发提示词 advisory，要求提供可检查的成功条件。仅提供 advisory：它绝不会阻止构建，也不会重写目标。在目标缺失时，以及 perpetual mode 下会被抑制，因为开放式目标正是该配置的选择。
  在 bash 和 TypeScript 路径之间进行字节镜像。
- **Smart retry.** 对于已明确识别的永久性失败（凭据错误、未知模型、配额耗尽），会提前停止，而不是将重试预算耗费在必然重复的失败上。故障安全：无法识别的错误仍保持为 TRANSIENT，并完全按照之前的方式重试；速率限制明确排除在永久性失败集合之外。

## Operational observability (v8.0.0)

- **SDK capability-degradation event.** SDK 加载或流式传输失败时，会向 `.loki/events.jsonl` 追加一条结构化的
  `capability_degraded` 记录（使用与 hook events 相同的 `{type, source, timestamp, payload}` 信封），而不再仅作为捕获输出中的文字存在，以便无人值守的操作员区分“SDK 无法加载”和“模型工作质量不佳”。该记录会明确写入 `fail_closed: true`，而不是留待推断。无环境变量：这是操作员始终需要的信号。
- **Time to first preview.** `.loki/app-runner/first-preview.json` 记录从运行开始到应用首次提供服务所经过的秒数。该文件只写入一次，因此重启不会用一个令人乐观的热启动数字覆盖真正缓慢的首次预览；不存在基线时则完全跳过，而不是进行猜测。仅适用于 Bash 路径（app-runner 集成位于此处）。

## First-run UX (v7.29.0)

- **`loki quickstart`：** 引导完成首次构建的 4 个步骤（设置检查、单行创意、离线模板匹配、使用真实估算器数据的计划审查）；一路按 Enter 通过所有步骤即可构建示例 Todo 应用；非 TTY/CI 环境退出 2，并提供自动化提示。
- **Provider install offer：** 找不到任何 provider CLI 时，doctor 以及 start/demo/quick/quickstart 的 pre-flight 会提供安装 Claude Code 的选项。仅在交互式 TTY 中经过用户同意后执行；首先打印将要执行的单条命令；通过 `claude auth login` 完成交接，并使用 `claude auth status` 确认已准备就绪。退出方式：`LOKI_NO_INSTALL_OFFER=1`。
- **`loki demo` cost confirm：** 估算结果始终会在产生费用前打印；`--yes` 会跳过提示，但绝不会跳过估算。`LOKI_COMPLEXITY` 会被 `loki plan` 遵循，并显示诚实的强制分层提示。

---

## 并发与安全加固（v7.5.7 - v7.5.13）

连续三个补丁修复了跨进程和安全方面的漏洞。默认流程的用户可见行为没有变化；请通过所引用的路径进行验证。

- **跨进程文件锁**应用于追加或重写状态，因此并行运行、dashboard、MCP 不会损坏共享文件：门控计数器（`autonomy/run.sh` 门控计数器写入）、任务队列（`autonomy/run.sh` 队列读-修改-写入）、检查点索引（`autonomy/run.sh` 检查点索引更新）、`events.jsonl` 追加（`events/emit.sh` 和 `autonomy/run.sh` 中的事件发射路径）、人工干预信号文件（`autonomy/run.sh:check_human_intervention()`，状态机文档中约第 8059 / 7897 行）。
- **MCP 路径验证**：`mcp/server.py` 工具的文件/路径参数会被规范化；如果路径逃逸出项目根目录，则会被拒绝（v7.5.8 修复的路径遍历问题）。
- **Dashboard 身份验证**现在要求用于 `dashboard/server.py` 中的 `/api/memory/*`、`/api/learning/*` 和 `/api/status`（此前未经身份验证即可读取这些路径）。
- **Bash 引号加固**覆盖 `autonomy/run.sh` 和 `autonomy/loki`：命令替换和 `[ ]` 测试中的变量展开均已加引号，以防止路径包含空格时发生单词拆分。

请参阅 `CHANGELOG.md` 中的 [7.5.7]、[7.5.8]、[7.5.13] 条目，了解各项修复的详细列表和审查者签字确认。
...
| 功能 | 目标 | 备注 |
|---------|--------|-------|
| Bun runtime（第 2 阶段及以后） | 待定 | 迁移写入路径命令；在 `feat/bun-migration` 上跟踪 |
| Managed Agents 多智能体路径 | 待定 | `LOKI_EXPERIMENTAL_MANAGED_*` 标志 -- 研究预览版，不在生产 API 上提供 |
...
**v9.49.4 | [Autonomi](https://www.autonomi.dev/) 的旗舰产品 | 核心代码约 410 行**