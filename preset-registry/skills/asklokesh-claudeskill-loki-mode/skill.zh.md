...
**输入规约，输出经过验证的产品。** 规约驱动：“规约”是描述工作的任何内容——Markdown PRD、GitHub issue、OpenAPI 文档、Jira 工单（PRD 是规约的一种形式）。其差异化优势在于信任层：在完成验证之前，Loki 不会认定工作已经完成。必须通过 RARV-C 闭环、8 个质量门、完成委员会和已验证完成证据门，才会接受完成状态。遇到空差异、测试失败、可提供服务的应用运行状况异常（运行时启动轴，可通过 `LOKI_EVIDENCE_BOOT_GATE=0` 选择退出），以及变更文件中泄露凭据（密钥泄露轴，可通过 `LOKI_EVIDENCE_SECRET_GATE=0` 选择退出）时，证据门会阻止通过——v8.0.0。

**证据回执（请自行验证）。** 每次运行都会将回执写入 `.loki/proofs/<run_id>/`（可通过 `LOKI_PROOF=0` 选择退出），其中将确定性事实（包含基础/头部 SHA 和 `diff_sha256` 的 git 差异、测试命令及退出码、构建命令及退出码、每个门的判定结果）与 AI 评估（委员会的判定，明确标记为判断而非证明）分开记录。摘要结论仅根据事实计算得出：已验证（测试运行了真实命令且以 0 退出、差异非空、没有跳过任何项）、已验证但存在缺口（逐一列出每个缺口的名称），或未验证（某项检查已运行但失败）。可使用 `loki proof list|show <id>|verify <id>`（别名为 `loki receipt`）进行检查和复核；`loki proof verify` 会重新计算回执的哈希值（检测篡改），并以记录的基础 SHA 为基准，根据当前仓库重新推导差异（检测漂移），无异常时以 0 退出，存在篡改或漂移时以 1 退出。这是对“已完成”这一状态的诚实证明，而不是声称代码没有缺陷。

**提供商无关（自 v5.0.0 起保持稳定）：** 可在 Claude/Codex/Cline/Aider 上运行，使用抽象模型层级，并为非 Claude 提供商提供降级模式；无供应商锁定。Gemini 已于 v7.5.18 弃用。请参阅 `skills/providers.md`。**当前路线（v8.0.0）：** Anthropic Agent SDK 路径（见下文）、面向 OpenAPI/GraphQL/Postman 契约的规约模式扩展、运行时启动和密钥泄露证据轴，以及用于运行中控制的 `loki steer` / `loki why`。此前的路线包括：将 LSP 基础支撑作为一等智能体工具（v7.7.x），以及 RARV-C 闭环第一阶段（真实提供商裁判、门失败智能体群、合成 PRD 端到端测试、状态 `--json`）。
...
**Anthropic Agent SDK 路径（v8.0.0，可选启用，默认关闭）：** 一条无需 claude 二进制文件的路径，其中 RARV 循环基于 `@anthropic-ai/claude-agent-sdk` 的 `query()` 运行，裁判则基于原始 `@anthropic-ai/sdk` 运行。只需一个操作者开关 `LOKI_SDK_MODE`（`off` 默认值 / `judges` / `full`），其内容在 bash（`autonomy/lib/sdk-mode.sh`）和 TypeScript（`loki-ts/src/runner/sdk_mode.ts`）中逐字节保持一致。未设置时，其行为与 claude-CLI 路径逐字节一致。请参阅 `references/sdk-mode.md`。
...
| 规则 | 含义 |
|------|---------|
| **决策并行动** | 自主做出决策。不要向用户提问。 |
| **保持推进** | 不要暂停以等待确认。继续执行下一个任务。 |
| **持续迭代** | 总有可以进一步改进的地方。找出它。 |
...
**自 v5.3.0 起的默认设置（在 v7.5.13 中再次确认）：** 为保证质量，Haiku 默认禁用。使用 `--allow-haiku` 或 `LOKI_ALLOW_HAIKU=true` 启用。
...
**Git 工作树并行化：** 如需真正并行地开发功能，请对 run.sh 使用 `--parallel` 标志。参见 `skills/parallel-workflows.md`。

**大规模模式（50 个以上代理，仅限 Claude）：** 使用裁判代理、递归式子规划器和乐观并发。参见 `references/cursor-learnings.md`。
...
**转换要求：** 所有阶段质量门禁均已通过。不存在严重/高优先级问题（中/低优先级问题仅作建议）。
...
- 使用带子代理的 Task 工具进行探索（隔离上下文）
- **上下文窗口跟踪（v5.40.0）：** 可通过 `GET /api/context` 查看仪表盘量表、时间线和各代理的明细
- **通知触发器（v5.40.0）：** 可配置在上下文超过阈值、任务失败或达到预算限制时发出提醒。通过 `GET/PUT /api/notifications/triggers` 进行管理
...
| `.loki/specs/openapi.yaml` | 进行 API 工作前 | API 变更后 |
...
| `.loki/state/checkpoints/` | 任务完成后 | 自动写入，或通过 `loki checkpoint` 手动写入 |

单命令回滚（v7.5.2+）：`loki rollback latest` 或 `loki rollback to <id>` 可从检查点恢复 `.loki/` 状态。它会先强制捕获当前状态的回滚前快照并输出其 id，因此回滚操作本身也可以撤销（`loki rollback to <that-id>`）。使用 `loki rollback list` 查看检查点。

---

## 模块加载协议（Skills）

本协议用于管理**技能模块**的加载——即 `skills/` 中任务范围内的指令文件。它不同于下文所述的记忆系统渐进式披露机制，后者用于管理 `.loki/memory/` 中持久化的**记忆层**。
...
**统一入口点（v6.84.0）：**`loki start [SPEC|ISSUE-REF]` 会自动检测输入是 PRD 文件、议题 URL、议题编号，还是其他规范格式（例如 OpenAPI）。无需在 `loki start` 和 `loki run` 之间进行选择——一个命令即可处理所有情况。
...
- **Codex**：GPT-5.3、400K 上下文、128K 输出、支持 MCP、--full-auto 模式、降级模式（仅可顺序执行，无 Task 工具）
- **Cline**：多提供商 CLI、降级模式（仅可顺序执行，无 Task 工具）
- **Aider**：支持 18 个以上提供商后端、降级模式（仅可顺序执行，无 Task 工具）
...
| `loki why` | 解释当前结果；停滞时会指出真正的停滞原因并建议使用 `loki steer`（v8.0.0） |
...
出于企业安全考虑，**默认禁用**。除非明确启用，否则会阻止通过 `HUMAN_INPUT.md` 进行提示词注入。
...
与 Claude Managed Agents 的可选集成（发布于 2026 年 4 月）。为 Loki 提供跨项目的、可审计的记忆，以及真正的多智能体委员会。这些功能已内置于现有的 RARV-C 和委员会流程中——无需学习新命令。
...
| `LOKI_MANAGED_MEMORY` | 对 REASON 进行增强，并在 REFLECT 阶段将内容从 `.loki/memory/` 影子写入 Managed Agents 存储区 | 稳定（已使用模拟对象测试） |
| `LOKI_MANAGED_MEMORY_HYDRATE` | 会话启动时从存储区拉取语义模式和技能 | 稳定（已使用模拟对象测试） |
...
快速失败：子开关开启而父开关关闭时，进程将以状态码 2 退出，并给出明确错误。API
不可访问时将回退到本地路径，并向 `.loki/managed/events.ndjson` 写入一个 `managed_agents_fallback`
事件。不会发生重试风暴。

**推荐的逐步启用顺序：**
1. `LOKI_MANAGED_AGENTS=true LOKI_MANAGED_MEMORY=true`（内存镜像）。
2. 稳定运行一周后添加 `LOKI_MANAGED_MEMORY_HYDRATE=true`。
...
当前开发分支将真实证据接入 RARV-C 反馈。相关说明记录于此处以及 `loki internal --help` 中：

| 环境变量 | 效果 |
|---------|--------|
| `LOKI_INJECT_FINDINGS=true` | 将评审委员会的发现和门禁失败信息注入下一次 REASON 提示词 |
| `LOKI_OVERRIDE_COUNCIL=true` | 在真实提供商裁判可用时，优先使用它们而非模拟裁判 |
...
- **留出规范评估：** 约 25% 的检查清单项（按确定性的 `sha256(id)` 顺序，`N >= 4`）会被保留到 `.loki/checklist/held-out.json` 中，并从构建提示词的信息流中排除；如果任一留出项失败，完成评审委员会将阻止完成。可通过 `LOKI_HELDOUT_GATE=0` 选择退出。客观限制：这保护的是提示词信息流，而非沙箱；保留文件位于磁盘上，拥有文件系统访问权限的智能体可以读取它。
- **基线不确定披露：** 当证据门禁无法建立差异基线（`no_git_repo` / `no_run_start_sha`）时，它会写入 `.loki/state/evidence-inconclusive.json`，并且 `COMPLETION.txt` 会包含一行如实说明“未经独立验证”的文字。它绝不会阻止非 Git 项目；测试失败仍会阻止完成。

## 测试框架智能（v8.0.0）

在现有信任核心之上增加了四项可度量的测试框架规范。它们都无法
削弱门禁：每一项要么增加验证，要么避免在不可能成功的工作上
浪费预算。
...
| `LOKI_CONFIDENCE_SPIKE_MIN` | `90` | 首次出现时被视为突增的绝对置信度水平 |
| `LOKI_GOAL_SCORING=0` | 开启 | 禁用目标可度量性建议 |
| `LOKI_SMART_RETRY=0` | 开启 | 重试每次失败，包括不可重试的失败 |
| `LOKI_SIMPLE=1` | 关闭 | 移除系统提示词中用于指导的部分（-78%，每次迭代约减少 1562 个 token）。实验性消融组。 |

- **提示词缓存规范。** 提示词在显式的 `[CACHE_BREAKPOINT]` 处分为缓存稳定的
  `<loki_system>` 前缀和易变的 `<dynamic_context>` 尾部；SDK 评判路径会在该分界处应用 `cache_control`。
  任何新增的始终启用的指令都应放入前缀，否则每次迭代都会导致缓存失效。
- **置信度飙升时重新检查。** 当自行报告的置信度跃升至接近最大值时，
  会强制额外执行一次验证，然后完成信号阀才会强制停止运行。
  此机制严格为增量式：置信度飙升只能增加一次验证流程，绝不能跳过、
  缩短或满足某个关卡。它不能延迟停滞阀，而且这种延迟是一次性的，因此反复出现置信度飙升的运行
  无法无限期推迟该阀的触发。
- **可通过爬坡优化的目标评分。** 如果 `COMPLETION_PROMISE` 没有可衡量的
  目标（没有数字、比较符、具名指标或可验证的产物），系统会在提示词中建议提供一个可检查的成功条件。
  这仅是建议：绝不会阻止构建，也绝不会重写目标。如果目标缺失，
  或处于永久模式，则不会显示该建议，因为开放式目标正是所选配置。
  bash 和 TypeScript 路径中的内容按字节保持一致。
- **智能重试。** 对于明确识别出的永久性故障（凭据错误、
  模型未知、配额耗尽），系统会提前停止，而不是将重试预算浪费在必然得到相同结果的失败上。
  故障安全机制：无法识别的错误仍会被视为 TRANSIENT，并完全按照原有方式重试；速率限制则被明确
  排除在永久性故障集合之外。

## 运行可观测性 (v8.0.0)

- **SDK 能力降级事件。** SDK 加载或流式传输失败时，会向 `.loki/events.jsonl` 追加一条
  结构化的 `capability_degraded` 记录（使用与钩子事件相同的
  `{type, source, timestamp, payload}` 封装），而不是仅以文字形式存在于捕获的输出中，这样无人值守的运维人员就能
  区分“SDK 无法加载”和“模型工作质量不佳”。
  该记录会明确写入 `fail_closed: true`，而不是让人自行推断。没有对应的环境
  变量：这是运维人员始终需要的信号。
- **首次预览耗时。** `.loki/app-runner/first-preview.json` 会记录从运行开始到应用首次提供服务所经过的
  秒数。该文件只写入一次，因此重启无法用看起来更理想的
  热启动数据覆盖真正缓慢的首次预览数据；如果不存在基准数据，则会完全跳过，而不是进行猜测。
  仅适用于 Bash 路径（app-runner 集成位于该路径中）。

## 首次运行体验 (v7.29.0)

- **`loki quickstart`：** 引导式的 4 步首次构建流程（设置检查、用一行描述想法、离线模板匹配、使用真实估算器数据审核计划）；一路按 Enter 键即可构建示例 Todo 应用；在非 TTY/CI 环境中会以状态码 2 退出，并提供自动化提示。
- **提供安装提供商的选项：** 如果未找到提供商 CLI，doctor 以及 start/demo/quick/quickstart 的预检流程会提议安装 Claude Code。仅在交互式 TTY 中且获得用户同意后才会安装；执行前会先打印唯一要运行的命令；通过 `claude auth login` 移交身份验证，并使用 `claude auth status` 确认就绪状态。选择退出：`LOKI_NO_INSTALL_OFFER=1`。
- **`loki demo` 成本确认：** 在产生费用之前始终会打印估算结果；`--yes` 只会跳过提示，绝不会跳过估算。`loki plan` 会遵循 `LOKI_COMPLEXITY`，并如实注明强制指定的层级。

---

## 并发与安全加固（v7.5.7 - v7.5.13）

连续三个补丁修复了跨进程与安全方面的缺口。默认流程中没有面向用户的行为变更；请通过所引用的路径进行验证。

- 对追加或重写的状态采用**跨进程文件锁**，防止并行运行、仪表板或 MCP 损坏共享文件：门禁计数器（`autonomy/run.sh` 中的门禁计数器写入）、任务队列（`autonomy/run.sh` 中的队列读取-修改-写入）、检查点索引（`autonomy/run.sh` 中的检查点索引更新）、追加 `events.jsonl`（`events/emit.sh` 和 `autonomy/run.sh` 中的事件发出路径）、人工干预信号文件（`autonomy/run.sh:check_human_intervention()`，根据状态机文档，位于约第 8059 / 7897 行）。
- **MCP 路径验证**——传递给 `mcp/server.py` 工具的文件/路径参数会进行规范化处理；如果路径超出项目根目录，则予以拒绝（v7.5.8 中的路径遍历修复）。
- **仪表板身份验证**现在是访问 `dashboard/server.py` 中 `/api/memory/*`、`/api/learning/*` 和 `/api/status` 的必要条件（此前这些读取路径无需身份验证）。
- 对 `autonomy/run.sh` 和 `autonomy/loki` 进行全面的 **Bash 引用加固**——为命令替换和 `[ ]` 测试中的变量展开添加引号，以防止包含空格的路径发生分词。

有关各项修复的列表和审阅者签核，请参阅 `CHANGELOG.md` 中的 [7.5.7]、[7.5.8]、[7.5.13] 条目。
...
| 质量门禁三审阅者系统 | v5.35.0 | `skills/quality-gates.md` 中包含 5 名专业审阅者；在 run.sh 中执行 |
| 记忆系统（情景/语义/程序性） | v5.15.0 | 完整实现在 `memory/` 中 |
| 上下文窗口跟踪 | v5.40.0 | 仪表板仪表盘，通过 `GET /api/context` 查看各智能体的明细 |
| 通知触发器 | v5.40.0 | `GET/PUT /api/notifications/triggers` |
| GitHub 集成 | v5.42.2 | 导入、同步回写、创建 PR、导出。CLI：`loki github`，API：`/api/github/*` |
| 旧系统修复 | v6.67.0 | `loki heal <path>`——将阻碍视为语义、特征测试 |
| 统一的 `loki start` | v6.84.0 | 自动检测规范（PRD、OpenAPI 等）或议题输入 |
| 托管智能体（记忆镜像） | v7.2.0 | 通过 `LOKI_MANAGED_AGENTS` 选择启用——参见“托管智能体”部分 |
| Bun 运行时（第 1 阶段） | v7.3.0 | 只读命令通过 `bin/loki` 路由；设置 `LOKI_LEGACY_BASH=1` 可恢复原有方式 |
| 第 1 阶段 RARV-C 闭环 | v7.5.x | 发现项注入、真实评判器、自动学习、handoff.md |
| Anthropic SDK 路由 | v8.0.0 | 选择启用，默认关闭；使用单一开关 `LOKI_SDK_MODE`——参见 `references/sdk-mode.md` |
| 工具链智能 | v8.0.0 | 提示词缓存规范、置信度激增时重新检查、目标评分、智能重试 |
...
| 可选启用的构建分析 | v8.0.0 | `build_verified` 事件受严格的第二道门禁控制，仅允许白名单字段 |
...
**v9.22.10 | [Autonomi](https://www.autonomi.dev/) 旗舰产品 | 核心代码约 410 行**