...
- 实施 Organization schema，以增强品牌展示
- 任何可通过富媒体搜索结果提升可见度的页面
...
使用提供的数据执行完整工作流。在输出中注明哪些数据来自自动提取，哪些数据由用户提供。
...
1. **识别内容类型和富媒体搜索结果机会**
   ...
   参考 CORE-EEAT 基准中的 **O05（Schema 标记）**项，将内容类型映射到 schema：
...
2. **生成 Schema 标记**
   ...
   > **参考资料**：有关所有 Schema 类型的完整、可直接复制使用的 JSON-LD 模板（包含必需属性和可选属性），请参阅 [references/schema-templates.md](references/schema-templates.md)。
   ...
   在一个页面上组合多个 Schema 类型时，请将它们包装在单个 `<script type="application/ld+json">` 标签内的 JSON 数组中。
...
> **参考**：完整决策树（内容到 Schema 的映射）、特定行业建议、实施优先级层级（P0-P4）以及验证快速参考，请参阅 [references/schema-decision-tree.md](references/schema-decision-tree.md)。
