...
（自然搜索结果 + 图片）、关键词指标、反向链接概况、页面分析、内容
分析、商业列表、AI 可见性检查，以及跨 9 个 API 模块的 LLM 提及跟踪，
包含 79+ 个 MCP 工具。

## 前置条件

此技能要求安装 DataForSEO 扩展：
```bash
./extensions/dataforseo/install.sh
```

**检查可用性：** 在使用任何 DataForSEO 工具之前，通过检查
`serp_organic_live_advanced` 或任意 DataForSEO 工具是否可用，确认 MCP
服务器已连接。如果工具不可用，请告知用户扩展尚未安装，并提供安装说明。

## API 额度注意事项

DataForSEO 按 API 调用次数收费。请高效使用：
- 优先使用批量端点，而不是多次调用单个端点
- 除非用户另有指定，否则使用默认参数（美国、英语）
- 在会话中记住结果缓存；不要重复获取相同数据
- 在运行高成本操作（完整反向链接抓取、大量关键词列表）之前警告用户
...
- 如果 `"status": "blocked"` → 告知用户将超出每日预算限制；不要继续执行
...
- `/seo dataforseo costs summary` → 显示过去 7 天的支出历史
- `/seo dataforseo costs config --mode threshold --threshold 0.50` → 配置审批模式

加载 `references/cost-tiers.md` 以获取完整的价格表、预算预设和降低成本的技巧。

## 快速参考

| 命令 | 功能 |
|---------|-------------|
| `/seo dataforseo serp <keyword>` | Google 自然搜索结果 |
| `/seo dataforseo serp-images <keyword>` | Google 图片搜索结果 |
...
**还支持：** `serp_organic_live_advanced` 工具通过 `se` 参数支持 Google、Bing 和 Yahoo。指定 `"bing"` 或 `"yahoo"` 可切换搜索引擎。

**输出：** 排名、URL、标题、描述、域名、精选摘要、AI 概览引用、People Also Ask。

### `/seo dataforseo serp-youtube <keyword>`

获取 YouTube 搜索结果。这对 GEO 很有价值。YouTube 提及与 AI 引用之间的相关性最强。
...
对特定 YouTube 视频进行深度分析：信息、评论和字幕。部分第三方研究报告称，YouTube 提及与 AI 可见性之间存在 0.737 的相关性，因此应将其视为一种取决于方法的 GEO 信号。
...
**输出：** 视频元数据（标题、频道、观看次数、点赞数、描述）、按互动量排序的热门评论、字幕/文字记录内容。

### `/seo dataforseo serp-images <keyword>`

获取实时 Google 图片搜索结果。查看某个关键词对应的排名图片、哪些域名在图片结果中占据主导地位，并识别视觉内容机会。

**MCP 工具：** `serp_google_images_live_advanced`

**默认参数：** `location_code=2840`（US）、`language_code=en`、`device=desktop`、`depth=100`

**参数：** `keyword`（必需）、`depth`（可选，最大值为 700，按每 100 条结果为一个计费增量）、`search_param`（可选，例如 `"site:example.com"`）

**费用警告：** 使用 `site:` 或 `filetype:` 运算符会产生 **5 倍 API 费用**。运行经过筛选的查询前，先向用户发出警告。

**输出：** 位置、标题、替代文本、源页面 URL、直接图片 URL、域名、编码后的 URL。

**应提供的分析：**
- 域名主导性：哪些网站占据最多的图片位置（按数量列出排名前 10 的域名）
- 替代文本模式：排名靠前的图片中常见的标题/替代文本模式
- 格式分布：前列结果中 WebP、JPEG 和 PNG 的分布（根据 `image_url` 扩展名推断）
- 机会识别：用户已有自然搜索排名但没有图片展示的位置
...
**默认参数：** 每个子调用的 limit=100

**输出：** 反向链接总数、引用域名数、域名排名、垃圾链接分数、热门锚文本、随时间变化的新建/丢失反向链接、dofollow 比率、热门引用域名。
...
**参数：** domains（2-20 个元素的数组）

**输出：** 每个域名对应排名位置的共享关键词、共享反向链接来源、每个域名的独有关键词。
...
**用法：**
- `on_page_instant_pages`：快速页面分析（状态码、元标签、内容大小、页面计时、断链、页面内检查）
- `on_page_content_parsing`：提取并解析页面内容（纯文本、字数、结构）
- `on_page_lighthouse`：完整 Lighthouse 审计（性能分数、可访问性、最佳实践、SEO、Core Web Vitals）

**输出：** 抓取的页面、状态码、元标签、标题、内容大小、加载时间、Lighthouse 分数、断链、资源分析。
...
**输出：** 内容匹配结果及质量分数、情感分析、可读性指标、随时间变化的短语趋势数据。

### `/seo dataforseo listings <keyword>`

搜索商业列表，用于本地 SEO 竞争分析。
...
抓取 ChatGPT 网页搜索针对某个查询返回的结果。检查 ChatGPT 的可见性：查看 ChatGPT 针对目标关键词引用了哪些来源。在可用时，使用 GSC 的生成式 AI 报告检查 Google AI Overviews 和 AI Mode。
...
跟踪 LLM 如何提及品牌、域名和主题。这对 GEO 至关重要。衡量品牌在多个 LLM 平台上的实际 AI 可见度。

**MCP 工具：** `ai_opt_llm_ment_search`、`ai_opt_llm_ment_top_domains`、`ai_opt_llm_ment_top_pages`、`ai_opt_llm_ment_agg_metrics`

**参数：** keyword、location_code（可选）、language_code（可选）。使用 `ai_opt_llm_ment_loc_and_lang` 获取可用的位置/语言，并使用 `ai_optimization_llm_models` 获取支持的 LLM 模型。

**工作流：**
1. 使用 `ai_opt_llm_ment_search` 搜索 LLM 提及内容（查找品牌/关键词在 LLM 响应中的提及）
2. 使用 `ai_opt_llm_ment_top_domains` 获取被引用最多的域名（针对该主题被引用最多的域名）
3. 使用 `ai_opt_llm_ment_top_pages` 获取被引用最多的页面（具体哪些页面被引用最多）
4. 使用 `ai_opt_llm_ment_agg_metrics` 获取聚合指标（整体提及量、趋势）
...
其他 DataForSEO MCP 工具可供内部使用，但没有专用命令。需要查找特定实用工具时，加载 `references/tool-catalog.md`（位置查询、批量操作、历史数据、筛选选项）。

## 跨技能集成

当 DataForSEO MCP 工具可用时，其他 claude-seo 技能可以利用实时数据：

- **seo-audit**：派生 `seo-dataforseo` agent，以获取真实的 SERP、反向链接、页面和本地列表数据
...
- **MCP 服务器未连接**：报告未安装 DataForSEO 扩展或无法访问 MCP 服务器。建议运行 `./extensions/dataforseo/install.sh`
- **API 身份验证失败**：报告凭据无效。建议检查 MCP 配置中的 DataForSEO API login/password
- **超出速率限制**：报告触发的限制，并建议等待后重试
- **未返回结果**：针对该查询报告“未找到数据”，而不是进行猜测。建议扩大查询范围，或检查位置/语言代码
