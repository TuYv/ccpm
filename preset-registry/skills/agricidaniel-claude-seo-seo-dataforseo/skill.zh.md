...
（自然搜索 + 图片）、关键词指标、反向链接概况、页面分析、内容
分析、商家列表、AI 可见性检查以及大语言模型提及追踪，
涵盖 9 个 API 模块和 79+ 个 MCP 工具。

## 前置条件

此 Skill 需要安装 DataForSEO 扩展：
```bash
./extensions/dataforseo/install.sh
```

**检查可用性：** 使用任何 DataForSEO 工具之前，请检查
`serp_organic_live_advanced` 或任意 DataForSEO 工具是否可用，以确认 MCP 服务器
已连接。如果工具不可用，请告知用户扩展尚未安装，并提供安装说明。

## API 额度注意事项

DataForSEO 按 API 调用收费。请高效使用：
- 优先使用批量端点，而不是进行多次单项调用
- 除非用户另有指定，否则使用默认参数（美国、英语）
- 在同一会话中记住结果；不要重复获取相同数据
- 在运行高成本操作（完整反向链接抓取、大型关键词列表）之前提醒用户
...
- 如果 `"status": "blocked"` → 告知用户这将超出每日预算限额；不要继续
...
| `/seo dataforseo ranked <domain>` | 域名获得排名的关键词 |
...
| `/seo dataforseo ai-mentions <keyword>` | 用于 GEO 的大语言模型提及追踪 |
...
**还支持：** `serp_organic_live_advanced` 工具可通过 `se` 参数支持 Google、Bing 和 Yahoo。指定 "bing" 或 "yahoo" 即可切换搜索引擎。
...
深入分析特定 YouTube 视频：信息、评论和字幕。一些第三方研究报告称，YouTube 提及与 AI 可见度之间的相关系数为 0.737，因此应将其视为依赖具体方法论的 GEO 信号。
...
获取实时 Google 图片搜索结果。查看哪些图片针对某个关键词获得排名、
哪些域名在图片结果中占据主导地位，并识别视觉内容机会。

**MCP 工具：** `serp_google_images_live_advanced`

**默认参数：** location_code=2840 (US), language_code=en, device=desktop, depth=100

**参数：** keyword（必需）、depth（可选，最大值为 700，按每 100 条结果递增计费）、search_param（可选，例如 "site:example.com"）

**成本警告：** 使用 `site:` 或 `filetype:` 运算符会产生 **5 倍 API 成本**。运行筛选查询前应警告用户。

**输出：** 排名位置、标题、替代文本、来源页面 URL、图片直链 URL、域名、编码后的 URL。

**需提供的分析：**
- 域名主导程度：哪些网站占据最多图片排名位置（按数量排名前 10 的域名）
- 替代文本模式：排名靠前的图片中常见的标题/替代文本模式
- 格式分布：排名靠前的结果中 WebP、JPEG 与 PNG 的分布情况（根据 image_url 扩展名推断）
...
**输出：** 关键词、难度分数（0-100）、难度解读（简单/中等/困难/非常困难）。
...
**输出：** 反向链接总数、引荐域名、域名排名、垃圾信息评分、热门锚文本、随时间变化的新增/丢失反向链接、dofollow 链接占比、热门引荐域名。
...
**输出：** 竞争域名、关键词重叠百分比、预估流量、域名排名、共同关键词。

### `/seo dataforseo ranked <domain>`

列出某个域名参与排名的关键词及其排名位置和页面数据。
...
运行页面分析，包括 Lighthouse 审核和内容解析。

**MCP 工具：** `on_page_instant_pages`、`on_page_content_parsing`、`on_page_lighthouse`

**用法：**
- `on_page_instant_pages`：快速页面分析（状态码、元标签、内容大小、页面耗时、失效链接、页面检查）
- `on_page_content_parsing`：提取并解析页面内容（纯文本、字数、结构）
- `on_page_lighthouse`：完整的 Lighthouse 审核（性能评分、无障碍性、最佳实践、SEO、核心网页指标）

**输出：** 已抓取页面、状态码、元标签、标题、内容大小、加载时间、Lighthouse 评分、失效链接、资源分析。
...
抓取 ChatGPT 网页搜索针对某个查询返回的内容。ChatGPT 可见性检查：查看 ChatGPT 针对你的目标关键词引用了哪些来源。在可用时，通过 GSC 生成式 AI 报告检查 Google AI Overviews 和 AI Mode。
...
跟踪 LLM 如何提及品牌、域名和主题。这对 GEO 至关重要。衡量跨多个 LLM 平台的实际 AI 可见性。
...
**工作流程：**
1. 使用 `ai_opt_llm_ment_search` 搜索 LLM 提及（查找 LLM 响应中对某个品牌/关键词的提及）
2. 使用 `ai_opt_llm_ment_top_domains` 获取引用次数最多的域名（该主题最常引用哪些域名）
3. 使用 `ai_opt_llm_ment_top_pages` 获取引用次数最多的页面（具体哪些页面最常被引用）
4. 使用 `ai_opt_llm_ment_agg_metrics` 获取汇总指标（总体提及量、趋势）

**输出：** LLM 提及次数、最常引用的域名及频次、最常引用的页面、提及量随时间变化的趋势、跨平台可见性得分。

**高级：** 使用 `ai_opt_llm_ment_cross_agg_metrics` 进行跨模型比较（比较 ChatGPT、Claude、Perplexity 等平台之间的提及差异）。

---

## 可用的实用工具

还有其他 DataForSEO MCP 工具可供内部使用，但没有专用命令。当你需要查找特定的实用工具（位置查询、批量操作、历史数据、筛选选项）时，请加载 `references/tool-catalog.md`。

## 跨 Skill 集成

当 DataForSEO MCP 工具可用时，其他 claude-seo Skill 可以利用实时数据：
...
- **seo-images**：使用 `serp_google_images_live_advanced` 获取竞争对手的图片 SERP 数据，并与页面内图片审计结果进行交叉比对
- **seo-geo**：使用 `ai_optimization_chat_gpt_scraper` 获取真实的 ChatGPT 可见性，使用 `ai_opt_llm_ment_search` 跟踪 LLM 提及
