...
DynamoDB 是一款完全托管的 NoSQL 数据库，旨在提供任意规模下个位数毫秒级的性能。掌握单表设计和访问模式建模。
...
**围绕访问模式进行设计，而不是围绕实体。以访问模式为先。**

DynamoDB 要求你在设计架构之前就明确查询方式。应围绕访问数据的方式进行建模，而不是围绕数据之间的关系。单表设计使用通用键属性，将多种实体类型存储在同一个表中。
...
| **Sort Key (SK)** | 可选的辅助键，用于分区内的范围查询 |
| **GSI** | 全局二级索引——备用的分区键/排序键 |
| **LSI** | 本地二级索引——相同分区，不同排序键 |
| **Item** | 单条记录（最大 400 KB） |
| **Attribute** | 项中的字段 |
...
- **Scan 操作** - 始终使用带有适当键条件的 Query
- **热分区** - 使用高基数分区键分散写入
- **大型项目** - 将项目大小保持在 400KB 以下；大型数据使用 S3
- **GSI 过多** - 每个 GSI 都会复制数据；请谨慎设计
- **忽略容量** - 监控已消耗容量，对可变负载使用按需模式
- **没有条件表达式** - 始终使用 ConditionExpression 进行验证
- **获取所有属性** - 使用 ProjectionExpression 限制数据
- **无理由地采用多表设计** - 除非访问模式不重叠，否则优先采用单表设计