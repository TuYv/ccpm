...
### HookConsumerWidget（搭配 flutter_hooks）
```dart
...
- ❌ **Provider 未使用 autoDispose** - 使用 `.autoDispose` 防止内存泄漏
- ❌ **在回调中使用 watch** - 在 onPressed/回调中使用 `ref.read()`，而不是 `ref.watch()`
- ❌ **在 widget 中编写业务逻辑** - 将其移至 Notifiers/providers
- ❌ **Provider 中存在可变状态** - 使用 Freezed 创建不可变模型
- ❌ **未使用 AsyncValue** - 使用 `when()` 处理加载/错误状态
- ❌ **将 setState 与 Riverpod 一起使用** - 使用 providers 管理共享状态
- ❌ **将 ref 传递给函数** - 将 ref 的使用限制在 widgets/providers 内部
- ❌ **Consumer 嵌套过深** - 改用 ConsumerWidget
- ❌ **未对参数使用 family** - 对参数化 providers 使用 `.family`
- ❌ **全局 GoRouter 实例** - 使用 Provider 管理包含重定向逻辑的 router
- ❌ **跨异步操作使用 BuildContext** - 在 await 前存储所需值，而不是存储 context
- ❌ **忽略 dispose** - 在 ConsumerStatefulWidget 中清理 controllers
