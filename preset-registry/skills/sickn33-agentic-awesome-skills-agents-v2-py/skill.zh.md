...
**最低 SDK 版本：** 需要 `2.0.0b3` 或更高版本才能支持托管代理。
...
3. **能力主机** - 启用 `enablePublicHostingEnvironment=true` 的账户级能力主机
...
| `environment_variables` | `dict[str, str]` | 否 | 容器环境变量 |
...
| `InvalidContainerImage` | 镜像不存在 | 验证镜像路径和标签在 ACR 中存在 |
| `CapabilityHostNotFound` | 未配置能力主机 | 创建账户级能力主机 |
| `ProtocolVersionNotSupported` | 协议版本无效 | 使用 `AgentProtocol.RESPONSES` 和版本 `"v1"` |

## 最佳实践

1. **版本化镜像** - 在生产环境中使用具体标签，而不是 `latest`
2. **最小化资源** - 从最小 CPU/内存开始，根据需要向上扩展
3. **环境变量** - 用于全部配置，切勿硬编码
4. **错误处理** - 用 `try/except` 块包裹代理创建
5. **清理** - 删除未使用的代理版本以释放资源

## 参考链接

- [Azure AI Projects SDK](https://pypi.org/project/azure-ai-projects/)
- [Hosted Agents 文档](https://learn.microsoft.com/azure/ai-services/agents/how-to/hosted-agents)
