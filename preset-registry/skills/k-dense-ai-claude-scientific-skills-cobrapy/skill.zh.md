...
COBRApy 使用 [optlang](https://optlang.readthedocs.io/) 管理求解器。GLPK 会通过 `swiglpk` 自动安装。对于大型 MILP/QP，cobra 0.29+ 增加了一个**混合**求解器（HIGHS/OSQP）；`model.solver = "osqp"` 现在会通过混合求解器路由，并且在未来版本中可能会在普通 LP 上报错——在可用时优先使用 `model.solver = "hybrid"`。

## 核心功能

COBRApy 提供了按多个关键领域组织的全面工具：
...
### DictList 对象
模型对反应、代谢物和基因使用 `DictList` 对象——其行为同时类似于列表和字典：
```python
# Access by index
...
2. 在分析前使用 `model.slim_optimize()` **验证模型**，以确保可行性
3. 优化后 **检查解状态**——`optimal` 表示求解成功
4. 当热力学可行性很重要时，**使用无环 FVA**
5. 在 FVA 中适当设置 **fraction_of_optimum**，以探索次优空间
6. **并行化** 计算开销较大的操作（采样、双重删除）——对于全基因组尺度模型，应从较小的 `n` 和 `processes=1` 开始
7. 对于模型交换和长期存储，**优先使用 SBML 格式**
8. 当仅需要目标值以提升性能时，**使用 slim_optimize()**
9. **验证通量样本**，以确保数值稳定性
10. 从工作流示例写入 CSV/PNG 文件前，**确认输出路径**
