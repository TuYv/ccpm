...
分析此代码仓库，并为只读命令生成推荐的 Claude Code `settings.json` 权限。
...
MCP 服务器在 `.mcp.json`（而不是 `settings.json`）中配置。检查现有配置：
...
添加到 `.mcp.json`（将 `{org-slug}` 和 `{project-slug}` 替换为你的 Sentry 组织和项目 slug）：
...
**注意**：切勿建议使用 GitHub MCP。对于 GitHub，始终使用 `gh` CLI 命令。
...
- **其他包管理器** - 如果项目使用 pnpm，则不要包含 npm/yarn 命令
...
如果存在多个锁文件，则仅包含每个检测到的包管理器所对应的命令。