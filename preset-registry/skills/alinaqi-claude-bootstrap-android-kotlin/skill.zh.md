...
- ❌ **在主线程上阻塞协程** - 切勿在主线程上使用 `runBlocking`
- ❌ **使用 GlobalScope** - 使用 viewModelScope/lifecycleScope 实现结构化并发
- ❌ **在 init 中收集流** - 使用 `repeatOnLifecycle` 或 `collectAsStateWithLifecycle`
- ❌ **暴露可变状态** - 暴露 `StateFlow`，而不是 `MutableStateFlow`
- ❌ **未处理流中的异常** - 始终使用 `catch` 操作符
- ❌ **对可空值使用 Lateinit** - 使用 `lazy` 或带有 `?` 的可空类型
- ❌ **硬编码调度器** - 注入调度器以提高可测试性
- ❌ **未使用密封类** - 对有限状态集合优先使用密封类
- ❌ **在 Composable 中产生副作用** - 使用 `LaunchedEffect`/`SideEffect`
