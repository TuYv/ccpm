...
## Activity 与 Fragment 模式

### 使用 ViewBinding 的 Activity
```java
...
- ❌ **Context 泄漏** - 切勿在静态字段或单例中持有 Activity/Fragment 引用
- ❌ **回调中的内存泄漏** - 始终使用 WeakReference，或在 onDestroy 中清除回调
- ❌ **在后台线程更新 UI** - 对 UI 的更改必须始终提交到主线程
- ❌ **硬编码字符串** - 对所有用户可见文本使用字符串资源
- ❌ **上帝 Activity** - 将 Activity 控制在 200 行以内，并将逻辑提取到 ViewModel
- ❌ **NetworkOnMainThreadException** - 切勿在主线程执行网络调用
- ❌ **忽略生命周期** - 始终遵循 Activity/Fragment 的生命周期状态
- ❌ **阻塞主线程** - 将主线程操作控制在 16ms 以内
- ❌ **未处理配置变更** - 使用 ViewModel 在屏幕旋转后保留数据
- ❌ **硬编码尺寸** - 使用 dp/sp 单位和尺寸资源
- ❌ **过深的视图层次结构** - 将布局深度控制在 10 层以内，并使用 ConstraintLayout
- ❌ **未关闭资源** - 始终关闭 Cursor、InputStream 和数据库连接
