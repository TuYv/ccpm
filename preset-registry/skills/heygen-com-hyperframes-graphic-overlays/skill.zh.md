...
图形叠加功能以一段在本地**完整播放**的视频为基础，在其上按时间顺序叠加一系列经过设计的**图形卡片**——包括标题、下三分之一字幕条、数据标注、引语、侧边面板、画中画等，并与讲述内容同步。智能体负责设计卡片（时间安排 + 内容），并**直接在对话中编写每张卡片的 HTML**，然后组装成单个合成 HTML，并通过 `hyperframes` 渲染为 MP4。这里没有固定的原型列表，也没有预设的卡片结构——叠加内容根据转录文本实际表达的内容自然形成。

> **开始构建前，请先确认应采用的工作流。** 此技能使用**经过设计的图形卡片**（标题、下三分之一字幕条、数据标注、引语、侧边面板、画中画）来包装一段**现有的出镜讲述视频片段**。如果用户需要的是**纯字幕**（将口述内容显示为文字）→ `/embedded-captions`；如果需要的是**单个简短、无旁白的**元素（一个徽标短动画 / 下三分之一字幕条）→ `/motion-graphics`。**视频片段将保持原样播放**——重新调整时间、调色、重构画面、重新排序或处理音频都属于非线性编辑，**不在此技能的范围内**。根据 URL / 主题 / PR 从头构建视频 → 使用创作工作流。不确定应该使用图形叠加还是字幕？**请先阅读 `/hyperframes`。**

> **这是 `embedded-captions` 的图形包装同类技能。** 字幕将_口述内容_
> 作为可读字幕添加到视频中；本技能则在播放的视频上叠加_经过设计的图形_。
> 纯字幕 → `embedded-captions`。从头构建视频 → 使用创作工作流
> （`product-launch-video` / `faceless-explainer` / …）。
...
- `storyboard.json` — 轻量级卡片大纲（智能体的规划）
- `public/cards/card-XX.html` — 每张卡片对应一个 HTML 片段
- `public/index.html` — 最终组装完成的合成文件
- `output.mp4` — 渲染后的视频
...
此技能完全依赖 **hyperframes** CLI 以及系统中的 `ffmpeg` / `ffprobe` 运行。转录通过 `hyperframes transcribe` 在本地使用 **Whisper** 完成——不需要第三方服务、API 密钥或存在速率限制的代理。
...
- `ffmpeg` / `ffprobe`（系统）
- `<SKILL_DIR>/assets/fonts/*.woff2`、`<SKILL_DIR>/assets/vendor/gsap.min.js`（随此技能捆绑，在第 9 步暂存到工作目录中）

转录不需要密钥——`hyperframes transcribe` 会在本地运行 Whisper（第 4 步）。

在 macOS 上运行 `hyperframes render` 时，强烈建议设置：
...
所有产物都存放在 `videos/<project-name>/` 下——这与其他视频工作流（`product-launch-video` / `faceless-explainer` / `pr-to-video`）采用相同的约定。将当前工作目录保持在工作区根目录；以下所有内容都将写入这一个子目录中。
...
输出：`metadata.json`（读取 `width`/`height`/`duration`；fps = 对 `r_frame_rate` 分数求值，例如 `30000/1001 → 29.97`）和 `audio.mp3`。
...
使用本地 **Whisper**——无需 API 密钥、无需代理、无速率限制。它会将词级别的 `transcript.json` 写入工作目录（每个词包含 `text` 以及 `start` / `end` 时间戳）。读取该文件以获取第 6 步中控制卡片时间的词语/句子时间；如果需要分段级别的内容块，请自行根据标点/停顿将词语组合成句子。

**限制在媒体时长范围内。** Whisper 返回的最后一个词的 `end` 可能会略微超出实际片段长度——请将每张卡片的 `endSec` 和 `composition.durationSeconds` 限制在 `metadata.json` 中的时长范围内，否则渲染结果会在视频结束后出现黑屏尾部。

### 5. 修正转录文本

`transcript.json` 是一个**由词对象组成的扁平数组**——`[{ "text": "...", "start": s, "end": s }, …]`（没有 `segments` 数组，也没有 `words` 包装层；每个词使用的键是 **`text`**）。读取该文件并修正明显的 ASR 错误：

- 同音词、产品名称、技术术语和标点
- 直接修改词的 `text`；**保留其 `start` / `end`** 时间戳
- 文件中没有预先分组的 `segments` 数组——当卡片时间需要分段级别的内容块时，**请自行将词语组合成句子**（在句末标点/停顿处拆分）

### 6. 起草轻量级故事板（在聊天中）

**不涉及 CLI。** 读取 `transcript.json` 和 `metadata.json`，直接设计卡片。`storyboard.json` 是供智能体内部使用的规划产物——没有任何 CLI 命令会读取它；它的存在是为了让你在编写每张卡片的 HTML 之前，能够清晰地规划时间和内容。请保持与下面示例一致的结构，以便同一份大纲能够驱动你在第 9 步中创作的合成内容：
...
| field                   | type                                       | purpose                                                                                               |
| ----------------------- | ------------------------------------------ | ----------------------------------------------------------------------------------------------------- |
| `id`                    | string                                     | 在 Card HTML 和 GSAP 选择器中使用的稳定 ID                                                          |
| `intent`                | string                                     | 自然语言描述；作为 Card 合成的输入                                                                    |
| `startSec` / `endSec`   | number                                     | 以秒为单位的时间（endSec > startSec）                                                                 |
| `accentIndex`           | 0 \| 1 \| 2 \| 3 \| 4                      | 此 Card 使用 5 种主题强调色中的哪一种                                                                 |
| `zone`                  | enum（见下文）                             | Card 位于画布上的哪个区域                                                                             |
| `contentHints`          | object                                     | 自由格式的数据集合；智能体在此放置 kicker/title/detail/data/quote                                    |
| `archetype`（可选）     | string                                     | 可附加的自由格式标签，用于记住 Card 的模式；缺省即为自由格式，也是默认设置                           |
| `transition`（可选）    | enum: `cut` \| `fade` \| `slide` \| `wipe` | 声明式的 Card 间过渡效果                                                                               |

**五种 `zone` 值：**

| zone              | 解析后的边界                                   | 适用场景                                |
| ----------------- | ---------------------------------------------- | --------------------------------------- |
| `fullscreen`      | 覆盖整个画布                                   | 高光时刻、大数字、箴言                  |
| `whiteboard-area` | 内缩 40px 边距（或竖屏高度的 45%）             | 密集数据／带注释的内容                  |
| `lower-third`     | 底部 30% 区域                                  | 叠加在可见视频上的注释                  |
| `side-panel`      | 右侧 42%（横屏）或底部 40%（竖屏）             | 数据位于一侧，视频位于另一侧            |
| `video-overlay`   | 整个画布，预期 Card 大部分透明                 | 全出血视频上的注释叠层                  |

在步骤 9 中组装合成内容时，请按照上表将每个 Card 的 `zone`
解析为 Card 宿主包装器上的像素边界。
视频边界只在合成层级设置**一次**（`videoTrack.bounds`）；
若要让视频看起来在不同 Card 之间“移动”，请在合成内容的 `<script>` 中针对
`#video-wrap` 编写 GSAP 补间动画（参见步骤 9）。

**没有预设的 Card 角色，也没有预设的叙事弧线。** Card 应根据
视频实际讲述的内容自然产生——可以全部是引语，也可以全部是数据，
可以用一个数字开场，也可以从一个故事开始。让转录文本来驱动节奏。

**应该有多少张要点卡片？——根据时长和信息密度自动推断。** 不设固定上限。先根据视频时长选择一个**基础节奏**，再根据**信息密度**进行调整。只有**下限是固定的：至少 5 张卡片**，这样即使是短视频也有节奏感。
...
| > 30 min           | **30–60s**               | 分段式、接近章节的感觉                 |

**第 2 步——信息密度乘数**（与基础节奏相乘）：

| 转录文本中的信号                                                                                                            | 乘数       | 效果                     |
| --------------------------------------------------------------------------------------------------------------------------- | ---------- | ------------------------ |
| **高密度**——数字多、观点鲜明、节奏短促、采用列表式列举，每 1–2 句话就出现一个新想法                                         | **× 0.7**  | 切换更快，卡片更多       |
| **中等密度**——数据与叙事兼具的混合表达                                                                                      | **× 1.0**  | 保持基础节奏             |
| **低密度**——一个展开较长的故事、反复换角度重述、缓慢而沉思的节奏、围绕单一论点逐步展开                                     | **× 1.5**  | 切换更慢，卡片更少       |
...
- **30s 短视频，只有一个包袱（低密度）** → 7 × 1.5 = 10.5s/card → round(30/10.5)=3 → 提升至下限 **5** 张卡片
- **60s 沉思式独白（低密度）** → 10 × 1.5 = 15s/card → **4** → 提升至下限 **5** 张卡片
- **121s 数据丰富的正对镜头讲述（高密度）** → 10 × 0.7 = 7s/card → **17** 张卡片
- **5 min 访谈，混合密度** → 16 × 1.0 = 16s/card → **19** 张卡片
- **10 min 深度解析，高密度** → 16 × 0.7 = 11s/card → **55** 张卡片
- **30 min 讲座，中等密度** → 28 × 1.0 = 28s/card → **64** 张卡片
- **1 hr 播客，低密度** → 45 × 1.5 = 67.5s/card → **53** 张卡片

当一张卡片的停留时间超过约 15s 时，应规划内容更丰富的卡片（数据区块、多步骤揭示、多个子要点通过错落动画逐步展开）——静态的单行文案超过 8s 就会变得乏味。对于许多卡片停留时间超过 30s 的长篇内容，可以考虑**将时间线拆分为多个子合成**（每个章节使用一个 .html，并通过 `data-composition-src` 挂载），使每个文件中的 GSAP 时间线保持易于管理——参见 `timeline_track_too_dense` HyperFrames lint 警告。

`content` 可以是纯字符串（"Title: annualized 5.69%\nNotes: ..."），也可以是任何能够承载数据的 JSON 结构。智能体会为每张卡片决定具体结构。

**可选的片尾。** 此 Skill **不附带固定的品牌片尾**。如果用户需要结束卡片，请自行设计一个中性的片尾（文字标识 + 单行标语，约 1.5–2 秒，淡入 -> 短暂停留 -> 淡出），将其追加到 `cards[]`，并将 `composition.durationSeconds` 延长至其 `endSec`。否则，以最后一张内容卡片结束。

### 7. 决定渲染策略

#### 与用户确认视觉方向（务必先做此项）

在开始设计卡片或确定边界之前，**请用户选择输出比例、布局、风格以及卡片密度预设**。边框会根据所选的布局 × 风格组合自动选择（请参阅下方的“自动选择边框”表格）。在发出问题之前，**预先计算以下两项内容**：

1. 根据源视频的宽高比（`metadata.json` 的 width / height）计算 **`recommendedRatio`**：
   - `sourceAspect = width / height`
   - `sourceAspect ≥ 1.5`（≥ 约 3:2 宽屏）→ 推荐 **`16:9`**
   - `sourceAspect ≤ 0.7`（≤ 约 9:13 竖屏）→ 推荐 **`9:16`**
   - `0.7 < sourceAspect < 1.5`（接近正方形）→ 推荐 **`4:5`**

   在推荐选项的标签中添加“（推荐 · 与源视频 X:Y 匹配）”，让用户了解推荐原因。
...
1. **`AskUserQuestion`**（Claude Code、Anthropic Console）——使用下方包含 4 个问题的结构化调用。
2. **其他原生澄清工具**（例如 `ask_question`、`request_user_input`、IDE 特定提示工具）——使用该工具，并沿用相同的 4 个问题文本和选项列表。保留推荐标记和预先计算的值。
3. **没有原生工具**（Codex CLI、仅支持纯文本的运行时）——**直接在普通对话中提问**。使用本节末尾的纯文本模板。将其控制在**一条消息、4 个编号问题**之内（全局上限为每轮 2–5 个问题；这里符合限制）。

适用于所有渠道的规则：

- 每轮**最多提出 2–5 个问题**。这里的 4 个问题符合要求。
- 即使缺失的信息不会阻碍渲染，也应**询问一次，以确认会实质影响最终输出的参数**（比例、布局、风格、cardCount）。
- 如果用户已经预先同意使用默认值（“直接使用默认值”“不需要询问”“全部自动选择”），或要求你不要提问——则**完全跳过提问**，并使用：`recommendedRatio`、`layout="stack"`（跨比例最稳妥的默认值）、根据转录文本语气从最中性的组别（编辑/数据）中选择 `style`，以及 `autoCount`。用一句话告知用户你选择了什么，然后继续。
...
**关于“其他”** — `AskUserQuestion` 会自动在卡片数量问题中添加一个“其他”选项。用户可以直接输入一个数字（例如“8”“20”），将其作为 cardCount 目标值。将输入解析为整数：如果解析成功 → 使用该值（下限为 5）；如果解析失败 → 回退到“auto”。

**通道 B — 纯文本回退方案**（Codex CLI、没有原生提问工具的运行时）。将以下内容作为一条普通消息发送，然后等待回复。采用 1/2/3/4 项目符号样式，以便解析回复：
...
- 如果任何答案存在歧义 → 仅重新询问存在歧义的选项（仍需遵守 2–5 个的上限）。
- 如果用户表示“default / auto / use all recommendations” → 跳过且不再重新询问。
...
1. 根据比例答案**确定输出画布** — 以下是要写入的准确 `storyboard.composition.width / height` 值：
   ...
   | `4:5`       | **1080 × 1350**            | `"portrait"`（schema 将 4:5 视为竖屏 — 高度 > 宽度） |

   对于 **`references/layouts/*.html` 中的 4:5 边界** — 这些文件仅记录了横屏（1920×1080）和竖屏（1080×1920）。对于 4:5（1080×1350），通过**从竖屏按比例缩放**推导边界：保持水平值不变，将垂直值乘以 `1350/1920 ≈ 0.703`。示例：`overlay` 竖屏卡片 =
   `{ x: 24, y: 1280, w: 1032, h: 564 }` → 4:5 卡片 =
   `{ x: 24, y: round(1280 × 0.703), w: 1032, h: round(564 × 0.703) }`
   = `{ x: 24, y: 900, w: 1032, h: 397 }`。

2. **根据转录文本的语气，将风格组映射到具体风格**——选择最契合的一种，但必须限定在用户选择的风格组内。如果你无法确定该组内的两种具体风格中哪一种更合适，请再次发送 `AskUserQuestion`，并提供 2–4 个具体风格选项。
...
4. **根据下表自动选择视频边框**（无需询问用户——边框由布局 × 风格决定）：

   | 布局      | 暖纸张风格（academic / whiteboard / editorial / xhs） | 临床风格（audit / swiss / terminal / minimal） | 实验性风格（geom / spotlight） |
   | --------- | ----------------------------------------------------- | ---------------------------------------------- | ------------------------------ |
   | `split`   | `polaroid`                                            | `hairline`                                     | `clean`                        |
   | `stack`   | `polaroid`                                            | `hairline`                                     | `clean`                        |
   | `pip`     | `clean`（pip 胶囊本身已有装饰框架）                   | `clean`                                        | `clean`                        |
   | `overlay` | `clean`（全出血布局禁止使用装饰边框）                 | `clean`                                        | `clean`                        |

5. **用一句话告诉用户你选择了什么**——比例（+ 画布尺寸）、布局、具体风格、边框和最终 cardCount——然后继续执行第 7 步的其余部分（逐卡片布局、动效模式）。
6. 在工作记忆中记录这五个值（ratio / layout / style / frame / cardCount）（无需 schema 字段）；在第 8 步编写每张卡片的 HTML，以及读取匹配的 `references/<dim>/<key>.html` 以获取设计令牌和结构时，你都需要引用它们。

如果用户通过“其他”选择了一个不在 10 种风格库中的自由文本风格名称，请将其视为自行设计全新卡片视觉效果的提示，但仍需以所选布局的边界为基准。

#### 渲染策略输入

在第 7.0 步中锁定 ratio / layout / style / cardCount / frame 后，剩余的逐卡片决策包括：

- **源视频在 GSAP 目标区域内的适配方式**：视频元素使用 `object-fit: cover`，并被裁剪到 `#video-wrap` 的补间动画边界内。如果你不希望发生任何裁剪（例如，纵向源视频放在横向画布上时，不应裁掉顶部/底部），请将补间动画的目标设为与源视频宽高比一致的矩形，并让周围的画布透出（或使用卡片 / 背景板填充）。
- **每张卡片的 `card.zone`**：根据你选择的构图布局推导（split → side-panel、stack → lower-third、pip → fullscreen、overlay → video-overlay），或者为一次性变体选择不同的区域（hero / quote 使用 fullscreen，密集数据使用 whiteboard-area）。
- **每张卡片的 `accentIndex`**：每张卡片使用 5 种主题强调色之一。跨卡片变化以形成节奏；当两张卡片属于同一个叙事节拍时，复用相同的索引。
- **动效词汇**：从 `data-anim` 类型中选择 2–3 种可重复使用的模式（参见后面的表格），并始终沿用这些模式，使构图保持连贯一致。

请从以下 `themeId` 调色板中选择（在你的构图 `<style>` 块中将其用作 `--accent-N` /
`--bg` / `--text` CSS 变量）：
...
可用字体（woff2 文件位于 `<SKILL_DIR>/assets/fonts/` 中，会在第 9 步暂存到工作目录）：`Caveat`（手写体）、
`LXGW WenKai TC`（中文手写体）、`Inter`（现代无衬线体）、`Virgil`
（几何手写体）。可通过 `@font-face` 或直接通过 `font-family` 引用。

若要获取视觉样式方面的灵感，`<SKILL_DIR>/references/styles/`
提供了 10 个自包含的参考卡片（academic / editorial / minimal
/ spotlight / geom / whiteboard / audit / terminal / swiss / xhs），
你可以复制它们作为起点——但**不要觉得必须与其中任何一种保持一致**。每张卡片都应采用你自己的设计。

#### 视觉设计库（<SKILL_DIR>/references/）

除了构图级别的 `themeId` 外，本 Skill 还在 `<SKILL_DIR>/references/` 中提供了一个内容更丰富的**参考
库**，涵盖三个可以自由混搭的**正交**
视觉维度：
...
| 维度       | 键                                                                                                | 决定的内容                                                               |
| ---------- | ------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| **style**  | `academic` `editorial` `minimal` `spotlight` `geom` `whiteboard` `audit` `terminal` `swiss` `xhs` | 卡片的视觉语言——字体、颜色、装饰和卡片内部布局                           |
| **layout** | `split` `stack` `pip` `overlay`                                                                   | 源视频与卡片如何共享画布                                                 |
| **frame**  | `clean` `hairline` `polaroid`                                                                     | 视频元素周围的装饰性边框                                                 |

请阅读 `<SKILL_DIR>/references/DESIGN_INDEX.md`
以查看完整矩阵和宽松的选择指南（访谈 / 产品发布 / 数据分析 /
社交媒体短片 / 技术教程 / 情感故事……）。当你决定使用特定的
样式 / 布局 / 边框时，请读取对应的文件：

- `references/styles/<key>.html` —— 自包含的卡片片段，包含该
  样式的 CSS 令牌（颜色、字体、内边距、装饰）和占位核心要点。复制 `.card[data-card-id="ref-<key>"]` 样式块，将
  data-card-id 重命名为你的卡片 ID，把占位内容替换为
  实际的核心要点，即可完成。
- `references/layouts/<key>.html` —— 提供横屏和竖屏的精确 `videoBounds` + `cardBounds`，
  以及可复制粘贴到
  `storyboard.json` 的每卡片 `layout` 字段中的 JSON 片段。
- `references/frames/<key>.html` —— 作为
  `#video-wrap` 同级元素添加的装饰性 HTML，以及构图 CSS 的放置说明。

为**每张卡片**选择 `style × layout × frame`——只要过渡衔接自然，你可以在卡片之间更改全部三个选项。常见的节奏是：
以 `editorial × overlay × clean` 开场，在数据卡片处切换为 `audit × split × hairline`，
最后以 `whiteboard × pip × polaroid` 收尾。

这 10 种样式是 Skill 端的设计令牌，**而不是合成层级的主题**——
无需在 `storyboard.composition` 中声明；它们存在于
每张卡片的 HTML 内部。`themeId` 字段仍可选择
合成层级的调色板（见上表），用于控制页面主体背景
和视频边框装饰。

#### 布局合成（卡片 + 视频）

每张卡片通过两个相互协调的决策，定义它如何与
源视频共享画布：

- **`card.zone`**（在 `storyboard.json` 中声明）——5 个 schema
  值之一；在第 9 步编写卡片宿主包装器的内联 `style` 时，
  按照第 6 步中的表格将其解析为像素边界。
- **该卡片时间窗口内的 `#video-wrap` 边界**（以命令式方式
  在合成的 GSAP 时间轴中声明）——智能体会在每次布局过渡时，
  将 `#video-wrap` 补间到目标矩形。

Schema **不会**存储每张卡片的视频边界。`videoTrack.bounds` 是
合成层级的**一次性**设置（默认为完整画布）。视频在卡片之间的
“移动”完全是在 `index.html` 中创作的 GSAP 动画。不存在 `card.layout`
字段——本文档的早期版本曾虚构过该字段；实际 schema 只有 `card.zone`。

**4 种合成布局**（来自 `references/layouts/`）——每一种都是
将 `zone` 与 `#video-wrap` 补间目标配对的方案：

| 合成布局           | 推荐的 `card.zone`       | `#video-wrap` 的 GSAP 目标（横屏 1920×1080）                              | `#video-wrap` 的 GSAP 目标（竖屏 1080×1920）                       | 适用场景                                        |
| ------------------ | ----------------------- | ------------------------------------------------------------------------- | ----------------------------------------------------------------- | ----------------------------------------------- |
| `split`            | `side-panel`            | `{ left: 960, top: 0, width: 960, height: 1080 }`                         | `{ left: 0, top: 960, width: 1080, height: 960 }`（下半部分）      | 演讲者与数据并排 / 50:50 权重                   |
...
| `overlay`          | `video-overlay`         | `{ left: 0, top: 0, width: 1920, height: 1080 }`（全出血）                | `{ left: 0, top: 0, width: 1080, height: 1920 }`                   | 电影感 / 戏剧化 / 全屏视频上的玻璃质感卡片      |

对于 4:5（1080×1350），将纵向画布的 y/h 值乘以 `1350/1920 ≈ 0.703`
（参见步骤 7.0 Channel A / Channel B 的 `recommendedRatio` 分辨率
表）。

**用于一次性变体的其他区域值**（仍使用 `card.zone`；不要使用虚假的
"layout" 字段）：

| `zone`            | 解析后的边界                                           | 常见用途                              |
| ----------------- | ------------------------------------------------------ | ------------------------------------- |
| `fullscreen`      | 覆盖整个画布                                           | 主视觉卡片、视频通过补间过渡到隐藏/pip |
| `whiteboard-area` | 横向画布内缩 40px 边距，纵向画布使用底部 45%          | 密集数据卡片、留出自由边距            |
| `lower-third`     | 底部 30% 区域                                          | 人物讲解标注                          |
| `side-panel`      | 横向画布右侧 42%，纵向画布底部 40%                    | 侧边栏 / "split" 方案                 |
| `video-overlay`   | 整个画布；卡片根元素应为透明                           | 全出血视频上的玻璃质感叠加层          |

你可以为每张卡片混用不同方案——根据当前时刻的需要选择 `card.zone`，
然后编写 GSAP 补间动画，使 `#video-wrap` 在卡片之间过渡。

#### 故事板渲染契约

`storyboard.json` 是供智能体内部使用的规划产物——没有任何 CLI
命令会解析它。它的作用是让你在编写每张卡片的 HTML 之前，
明确记录时间和内容决策。请遵循下面的 v3 风格
结构，以便在步骤 9 中组装合成内容时沿用同一份大纲。
...
- `subtitles: { enabled, ... }`
- `cards[]`——每张卡片都有 6 个必填字段：`id`、`intent`、`startSec`、`endSec`、`accentIndex`、`zone`、`contentHints`

规则：

- 卡片时间必须位于 `composition.durationSeconds` 范围内，且除非有意为之，否则不应重叠（重叠时使用 `data-track-index` 控制 z 轴顺序）。
- 视觉细节应放在卡片 HTML 片段中（步骤 8），**不要**放在 `contentHints` 中。`contentHints` 是你自己用来设计卡片的结构化提示；最终渲染的外观由 HTML 决定。
- 保持故事板结构稳定——尽管没有任何程序会解析它，但你会在编写步骤 8/9 时重新读取它，而一致的结构能让卡片 ID 和时间保持同步。
- 像 "I picked overlay × geom × clean" 这样的智能体侧决策**不要**写入 `storyboard.json`——将其保留在工作记忆中，并在编写卡片 HTML 和 GSAP 补间动画时使用。

**与视频共享画布的卡片应使用透明背景。**
当 GSAP 补间动画让视频在卡片后方或旁边保持可见时（overlay
方案、pip 方案，或任何 `card.zone = 'lower-third' | 'video-overlay'`
...
对于 `side-panel` 区域中的卡片（分栏方案），卡片宿主本身已经
只占画布的一半，因此使用不透明的卡片背景没有问题——它只会覆盖
自己所在的那一半。

### 8. 编写每张卡片的 HTML

为每张卡片创建 `$WORK_DIR/public/cards/{card-id}.html`。每个文件
包含一个遵循以下约定的单根 HTML 片段：
...
**硬性规则**（违反这些规则将无法通过 `hyperframes` lint）：

- 只能有一个根元素 `<div class="card" data-card-id="{cardId}">`
- 内联 `<style>` 中的规则必须以上述作用域选择器为前缀
- **禁止使用 `<script>` 标签**
- `src=` / `href=` 中**禁止使用外部 URL**（禁止 CDN，禁止远程字体）
- **禁止使用内联事件处理程序**（如 `onclick=` 等）
- 所有资源均通过相对路径引用同一 `public/` 目录中的文件
- 颜色使用 `var(--accent-N)` 等变量，以便适配不同主题

**动画通过声明定义，而不是通过代码实现。**只能使用 `data-anim-*` 属性；
绝不要编写 `<script>` 来实现动画。在第 9 步中，将每条 `data-anim-*`
声明编译到唯一的 GSAP 主时间线中。

#### 卡片尺寸——竖屏下移动端优先

这 10 个 `references/styles/*.html` 的尺寸适用于 **1920×1080 横屏**
预览。当 `storyboard.layout = "portrait"` 时（1080×1920，社交媒体/
移动端最常见的情况），**放大所有视觉尺寸**——手机屏幕离用户更近，
而相同像素数的内容在横屏电视式画布上看起来会更大。
...
**经验法则：** `portraitPx = round(landscapePx × 1.3)`，然后向下取整到接近的 4px 倍数，以保持视觉韵律。主视觉标题可放大至 ×1.4；较小的元信息文本保持在 ×1.2，以避免拥挤。

竖屏布局中的内边距会**略微缩小**——卡片更窄，因此较大的横屏内边距（40–64px）会占用过多宽度。竖屏布局的水平内边距应使用 24–36px。

如果你要制作一张必须同时适用于**两种**布局的卡片，优先在卡片根元素上使用 `@container` 查询，而不是硬编码尺寸：
...
但对于大多数卡片，选择一种布局即可——只需选用与故事板 `layout` 字段相匹配的尺寸表列。
...
| `typewriter`    | 逐字符淡入           | 与 kinetic-chars 相同，但默认的 stagger 更慢                                                    |
| `count-up`      | 数字递增动画         | `at`, `duration`, `from`, `to`, `format=.0f\|.1f\|.2f\|,d`                                      |
| `draw-path`     | 显示 SVG 路径        | `at`, `duration` — 元素应为 `<path>`                                                            |
| `grow-y`        | 条形高度             | `at`, `duration`, `target-h` (px) — 元素初始为 `height:0`                                       |
| `grow-x`        | 条形宽度             | `at`, `duration`, `target-w` (px) — 元素初始为 `width:0`                                        |
| `scale-pop`     | 弹出式进入           | `at`, `duration`                                                                                |
| `blur-in`       | 失焦 → 聚焦          | `at`, `duration`                                                                                |
| `mask-reveal`   | 裁剪显示             | `at`, `duration`, `direction=left\|right\|top\|bottom`                                          |
...
视频容器的选择器是 `#video-wrap`。使用 `tl.to('#video-wrap', { ...bounds }, T)` 在不同卡片之间对其边界进行动画过渡。初始边界应以内联方式设置在元素上，以匹配 card-01 的布局。过渡时长选择 0.5–0.7s，并使用 `ease: 'power2.inOut'`。

**装饰边框**（`clean` / `hairline` / `polaroid`）作为 `#video-wrap` 的**同级元素**放置，并在布局过渡时跟随它。有关每种边框的放置 HTML、建议 CSS 及其适配的布局，请参阅
[`references/frames/`](references/frames/)。简单规则：
`overlay` 布局会隐藏装饰边框（全出血视频会与装饰框架产生冲突）；PiP 布局已有自己的胶囊式处理
（圆角 + 白色描边 + 阴影），因此只需在 `split` / `stack` 上叠加装饰边框。

各构图布局下 `#video-wrap` 的 **GSAP 目标值查找表**
（横屏 1920×1080——竖屏和 4:5 规格请参阅 `references/layouts/*.html`，其中列出了全部三种宽高比）：

| 构图布局                             | 典型 card.zone     | `#video-wrap` GSAP 目标值                                                 | 额外 CSS 类                                 |
| ------------------------------------ | ----------------- | ------------------------------------------------------------------------- | ------------------------------------------ |
...
进入或离开 pip 时，按如下方式切换 pip-pill 装饰效果（圆角 + 白色描边 + 投影）：
...
**card-host 的边界与区域相匹配**。使用第 6 步顶部的表格将卡片的 `zone` 解析为
像素边界，然后将这些值写入 card-host 的内联 `style="left:Xpx;top:Ypx;width:Wpx;
height:Hpx;..."`。对于 `video-overlay` 区域（叠加层方案），
card-host 会填满整个画布——由 `.card .root` 内的 CSS
决定实际可见卡片的位置。

#### HyperFrames 布局 / 动画 QA 规则

- 首先构建每张卡片的静态主视觉帧：即卡片完全可见且可读的时刻。
- 确认视频、卡片、字幕/说明文字和图示不会发生非预期重叠。
- 确认被隐藏的视频区域会被画框裁剪，且不会显示在预期边界之外。
- 注册一个暂停状态的主时间线，命名为 `window.__timelines["graphic-overlays"]`。
- 在页面加载时同步构建时间线；不要使用 `async`、`setTimeout`、Promises 或媒体 `play()` 调用。
- 不要在渲染路径中使用 `Math.random()` 或 `Date.now()`。
- 不要使用 `repeat: -1`；应根据视频时长计算有限的重复次数。
- 对于运动效果，优先使用 GSAP 的变换和不透明度属性（`x`、`y`、`scale`、`rotation`、`opacity`），而不是布局属性（`top`、`left`、`width`、`height`）。
- 为 `#video-wrap` 等包装元素添加动画，不要直接对视频元素的尺寸添加动画。
- 避免同时从多个时间线对同一元素的同一属性添加动画。
- 使用 `data-track-index`，不要使用 `data-layer`；使用 `data-duration`，不要使用 `data-end`。
- 每个定时元素（`card-host`、子合成等）除了自身类之外，都必须包含 `class="clip"`——例如 `class="card-host clip"`。HyperFrames 运行时使用 `.clip` 将可见性限制在 `data-start … data-start+data-duration` 时间窗口内。缺少该类时，元素将在整个视频期间可见（lint：`timed_element_missing_clip_class`）。
- 对于 body / 全局 `font-family`，应列出**具体字体名称**（`'Inter', 'Caveat', …`），不要使用 `var(--font-family)` 这样的 CSS 变量。HyperFrames 字体解析器在静态分析期间不会展开 CSS 变量（lint：`font_family_without_font_face`）。卡片内部仍可使用 `var(--font-family)`，因为其 `@font-face` 声明会被加载。
...
强烈建议在 macOS 上使用 `PRODUCER_BROWSER_GPU_MODE=hardware` 标志（或 `--browser-gpu`）——
仅使用软件的 Chrome 渲染在大多数笔记本电脑上都会超时。

在进行完整渲染之前，可在特定时间戳捕获单帧以进行合理性检查：
...
**可选的实时预览（仅在用户请求时）。** 剪辑会在 `public/index.html` 中保持原样播放，并在其上方显示叠加层，因此能够忠实预览。**运行期间不要打开它。** 当用户提出请求时，在渲染**之后**启动一个长期运行的服务器，并报告 URL：
