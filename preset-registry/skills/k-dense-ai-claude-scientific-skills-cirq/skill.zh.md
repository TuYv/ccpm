...
Cirq 是 Google Quantum AI 的开源框架，用于设计、模拟以及在量子计算机和模拟器上运行量子电路。

## 何时使用此 Skill

在以下情况使用此 Skill：
- 在 Python 中构建、模拟或优化 NISQ 电路
...
对于 IBM 硬件，请使用 **qiskit**；对于使用自动微分的量子机器学习，请使用 **pennylane**；对于物理仿真，请使用 **qutip**。

## 安装

需要 Python 3.11+。当前稳定版本：**1.6.1**（2025 年 8 月）。供应商软件包共用相同的版本号。

```bash
uv pip install "cirq==1.6.1"
```

用于硬件集成（为实现可复现性，请固定匹配的版本）：
```bash
...
在开发期间如需使用最新功能，可省略版本固定；对于生产环境或硬件运行，请将所有软件包固定为相同的 Cirq 发行版本。
...
### 电路构建
有关构建量子电路的全面信息，包括量子比特、量子门、操作、自定义门和电路模式，请参阅：
...
### 模拟
有关量子电路模拟的详细信息，包括精确模拟、含噪模拟、参数扫描和量子虚拟机，请参阅：
...
- 将门分解为设备原生门集
- 查看 `hardware.md` 了解特定于设备的编译方法

**含噪模拟速度过慢：**
- 密度矩阵模拟的复杂度为 O(2^2n)，考虑减少量子比特数量
- 仅在关键操作上选择性地应用噪声模型
- 查看 `simulation.md` 了解性能优化方法