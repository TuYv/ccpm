...
**模型架构**：参见 [references/architecture.md](references/architecture.md)，其中以通俗易懂的方式讲解了 GPT 块结构、多头注意力和 MLP 层。

**训练循环**：参见 [references/training.md](references/training.md)，了解学习率调度、梯度累积和分布式数据并行设置。

**数据准备**：参见 [references/data.md](references/data.md)，了解分词策略（字符级 vs BPE）以及二进制格式细节。
