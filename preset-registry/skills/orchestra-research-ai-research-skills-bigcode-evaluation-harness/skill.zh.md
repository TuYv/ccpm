...
---
# BigCode 评估框架 - 代码模型基准测试

## 快速开始

BigCode 评估框架用于在 15 个以上的基准测试中评估代码生成模型，包括 HumanEval、MBPP 和 MultiPL-E（18 种语言）。
...
**Python 代码生成**（最常用）：
- **HumanEval**：164 道手工编写的问题，函数补全
- **HumanEval+**：与 HumanEval 相同的 164 道问题，但测试数量增加了 80 倍（更严格）
- **MBPP**：500 道众包问题，入门级难度
- **MBPP+**：399 道精选问题，测试数量增加了 35 倍

**多语言**（18 种语言）：
- **MultiPL-E**：将 HumanEval/MBPP 翻译为 C++、Java、JavaScript、Go、Rust 等语言

**高级**：
- **APPS**：10,000 道问题（入门/面试/竞赛）
- **DS-1000**：涵盖 7 个库的 1,000 道数据科学问题
...
- **instruct-humaneval**：使用指令提示词的 HumanEval
...
- 使用单元测试来测试**功能正确性**（pass@k）
- 针对 **BigCode/HuggingFace 排行榜**进行基准测试
- 评估**中间填充**（FIM）能力

**以下情况改用替代方案：**
- **lm-evaluation-harness**：通用大语言模型基准测试（MMLU、GSM8K、HellaSwag）
...
| HumanEval+ | 164 | Python | pass@k | 更严格的评估（测试数量为 80 倍） |
| MBPP | 500 | Python | pass@k | 入门级问题 |
| MBPP+ | 399 | Python | pass@k | 更严格的评估（测试数量为 35 倍） |
...
| `--tasks` | - | 以逗号分隔的任务名称 |
| `--n_samples` | 1 | 每个问题的样本数（计算 pass@k 时为 200） |
