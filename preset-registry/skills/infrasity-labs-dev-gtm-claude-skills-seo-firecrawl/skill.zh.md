...
**检查可用性：** 在使用任何 Firecrawl 工具之前，请检查 `firecrawl_scrape` 或任意 Firecrawl 工具是否可用，以确认 MCP 服务器已连接。如果工具不可用，请告知用户该扩展尚未安装，并提供安装说明。
...
2. **聚焦特定分区的抓取**：使用 `includePaths` 仅审计 `/blog/*` 或 `/products/*`
3. **失效链接检测**：使用 `["links"]` 格式抓取，检查所有 href 是否返回 404
4. **内容清单**：批量提取所有页面标题、元描述和 H1
5. **SPA/JS 渲染站点**：Firecrawl 会渲染 JavaScript，从而解决问题 #11
...
- 在进行大规模抓取之前，务必告知用户预计的额度使用量

### map -- 站点结构发现

发现网站上的所有 URL，而不获取内容。速度快且节省额度。
...
**SEO 使用模式：**
1. **站点地图对比**：映射站点，将发现的 URL 与 XML 站点地图进行比较
2. **孤立页面检测**：发现存在于站点地图中、但未被任何页面链接的 URL
3. **抓取预算分析**：对比可索引页面总数与从首页链接的页面数
4. **URL 模式分析**：识别 URL 结构模式、重复项和参数膨胀
5. **审计前发现**：先运行 map，然后针对关键分区进行定向抓取
...
- `waitFor`：等待内容出现的 CSS 选择器或等待的毫秒数
...
| 遇到速率限制/被阻止 | `firecrawl_scrape`（可处理反机器人机制） |

### search -- 站内搜索

在网站内搜索特定内容。适用于查找
与某个主题相关的页面，而无需抓取整个网站。
...
### 与 seo-audit（完整审计）集成
在 `/seo audit` 期间 Firecrawl 可用时：
1. 使用 `firecrawl_map` 发现所有网站 URL
2. 与 XML 站点地图（seo-sitemap）进行比较，以发现孤立页面/缺失页面
3. 选择重点页面进行深度分析
...
### 与 seo-sitemap 配合使用
- 站点地图覆盖率：已抓取页面中存在于站点地图的页面百分比
- 孤立页面：抓取时发现但站点地图中缺失的页面
- 过期的站点地图条目：站点地图中返回 404/410 的 URL

### 与 seo-content 配合使用
- 内容提取：将干净的 Markdown 提供给 E-E-A-T 分析
- 单薄内容检测：大规模识别字数少于 300 的页面
- 重复内容：跨页面比较内容以发现近似重复项
...
| `408 Timeout` | 页面渲染速度过慢 | 增大 `timeout`，尝试不使用 JS 渲染 |
| `403 Forbidden` | 站点阻止抓取 | 检查 robots.txt，可能需要跳过此站点 |
