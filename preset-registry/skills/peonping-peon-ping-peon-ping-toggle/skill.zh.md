...
**如需仅控制通知**，请使用 `/peon-ping-config` 将 `desktop_notifications: false`。这样会继续播放声音，同时禁止桌面弹窗。

## 示例

“完全静音 peon-ping” → 设置 `enabled: false`
“只禁用弹窗，但保留声音” → 设置 `desktop_notifications: false`（请改用 `/peon-ping-config`）

## 配置更改

如需更改其他 peon-ping 设置（音量、包轮换、类别、活动包等），请使用 `peon-ping-config` skill。