...
此技能可创建有吸引力且经过优化的 Meta 标签，从而提高搜索结果的点击率，并增强社交媒体分享效果。它涵盖标题标签、Meta 描述和社交 Meta 标签。
...
1. **创建标题标签**：编写有吸引力且经过关键词优化的标题
2. **编写 Meta 描述**：创建能够吸引点击的描述
3. **Open Graph 优化**：为页面的社交分享做好准备
4. **Twitter Card 设置**：优化 Twitter 专用的 Meta 标签
5. **点击率分析**：提出改进建议以提高点击率
6. **字符计数**：确保长度适合在 SERP 中显示
7. **A/B 测试建议**：提供可供测试的不同版本
...
自动获取当前 Meta 标签、按查询划分的点击率数据、竞争对手的标题/描述模式、SERP 预览数据以及展示/点击指标，以识别优化机会。
...
使用所提供的数据执行完整工作流。在输出中注明哪些指标来自自动收集，哪些来自用户提供的数据。
...
4. **创建 Open Graph、Twitter Card 和其他元标签**
   ...
   生成 OG 标签（og:type、og:url、og:title、og:description、og:image）、Twitter Card 标签、规范 URL、robots、viewport、author 以及文章专用标签。然后将它们组合成完整的元标签代码块。
...
- [CTR 和社交媒体参考](references/ctr-and-social-reference.md) — 页面类型模板、CTR 数据和 OG 最佳实践

## 下一个最佳 Skill

- **首选**：[schema-markup-generator](../schema-markup-generator/SKILL.md) — 使用结构化数据完善 SERP 信息呈现。