...
使用 LoRA、QLoRA 和 25 种以上的适配器方法，仅训练 <1% 的参数即可微调大语言模型。

## 何时使用 PEFT

**在以下情况下使用 PEFT/LoRA：**
- 在消费级 GPU（RTX 4090、A100）上微调 7B-70B 模型
- 需要训练 <1% 的参数（6MB 适配器，而完整模型为 14GB）
- 希望通过多个任务专用适配器实现快速迭代
- 基于一个基础模型部署多个微调变体
...
1. **从 r=8-16 开始**，如果质量不足再增大
2. **使用 alpha = 2 * rank** 作为起点
3. **以注意力层 + MLP 层为目标模块**，以获得最佳的质量与效率
4. **启用梯度检查点**以节省内存
5. **频繁保存适配器**（文件小，易于回滚）
6. **合并前在留出数据上进行评估**
