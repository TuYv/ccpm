...
捕获一个已知良好的基线，并在后续运行时准确报告发生了哪些漂移，以便
用户能够在 CMS 覆盖、迁移失误或排名缓慢下滑造成流量损失之前发现问题。
...
（目前还没有可供比较的内容）。将快照存储在用户选择的报告位置下
（默认：与其他审计日志并列的 `seo-drift/` 文件夹）。

## 阶段 0 — 预检与数据

阅读并遵循 `../shared/preamble.md`。强烈建议在此使用 GSC——排名
和已编入索引的数量是信号最强的漂移指标。

## 阶段 1 — 捕获快照

针对一组明确的**关键 URL**（按流量排名靠前的页面 + 用户指定的页面）收集：

- **GSC（如果已连接）** — 每个查询的排名与展示次数；已编入索引的页面总数
  （索引覆盖率）；按点击次数排名靠前的页面。
- **页面内容（实时抓取）** — 标题、元描述、H1、规范 URL、
...
- **排名** — 下降 ≥ N 个位置的查询；完全消失的查询。
- **索引情况** — 已编入索引的页面数量下降；当前缺失的具体关键页面。
- **元数据** — 发生变化的标题/元描述/H1（标记空白值或类似
  "Home | Site" 的模板化默认值——这是重新部署导致覆盖的典型特征）。
- **指令** — 规范 URL 发生变化/被移除；原本应该编入索引的页面上新出现
  `noindex`（这是最危险的单一回归问题——优先呈现）。
- **Schema** — 消失的结构化数据。

## 阶段 3 — 报告

生成一份**漂移报告**：按严重程度排序列出变更（严重 =
意外出现 noindex / 高价值页面被移出索引；警告 = 排名下滑 / 标题变更；
信息 = 符合预期的内容更新），每项都包含变更前→变更后的值以及可能的
原因。最后为每个严重问题提供建议措施。问题解决后，主动询问是否更新
基线。使用用户的语言编写。