...
记录“正在升级 NotFair v{old} → v{new}...”并继续执行步骤 2。
...
如果复制失败，则警告：“升级失败——旧版本仍处于活动状态。请手动运行 `/notfair:upgrade`。”然后停止。

### 步骤 5：更新 installed_plugins.json
...
删除旧的带版本号的缓存目录（仅保留新目录）。绝不要删除 `dev` 符号链接：
...
读取 `$NEW_CACHE_DIR/CHANGELOG.md`。找出旧版本与新版本之间的所有版本条目。按主题归纳为 3–7 个要点——重点关注面向用户的变更，跳过内部重构。
...
如果检测到 `DEV_SYMLINK`：告知用户“NotFair 以 dev 符号链接的形式安装——它始终指向你的本地源代码（v$(cat "$CACHE_DIR/VERSION" 2>/dev/null | tr -d '[:space:]')）。无需升级。”然后**停止**。不要继续执行第 2–8 步。

---

## 独立使用

直接以 `/notfair:upgrade` 调用时：

1. 检查 dev 符号链接（参见上面的“Dev 符号链接检测”）。如果检测到，则停止。

2. 强制执行全新的更新检查（绕过缓存和暂缓状态）：
```bash
