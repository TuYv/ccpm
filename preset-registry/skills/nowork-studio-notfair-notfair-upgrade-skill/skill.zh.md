...
| 插件缓存 | `~/.claude/plugins/cache/nowork-studio/notfair/<version>/` |
| 已安装插件 | `~/.claude/plugins/installed_plugins.json` |
| 更新状态 | `~/.toprank/`（有意保留——参见 CHANGELOG 0.24.0） |

---

## 内联升级流程

当技能前置流程输出 `UPGRADE_AVAILABLE` 时，使用本节中的流程。

### 第 1 步：自动升级

记录“正在将 NotFair 从 v{old} 升级到 v{new}...”，然后继续执行第 2 步。

---

### 第 2 步：检测当前安装

首先检查开发环境符号链接（参见“开发环境符号链接检测”一节）。如果检测到，则停止——不要升级。
...
如果复制失败，则发出警告：“升级失败——旧版本仍处于启用状态。请手动运行 `/notfair:upgrade`。”，然后停止。

### 第 5 步：更新 installed_plugins.json

读取 `~/.claude/plugins/installed_plugins.json`，然后更新 `notfair@nowork-studio` 条目：
...
删除旧的带版本号缓存目录（仅保留新目录）。绝不要删除 `dev` 符号链接：
...
读取 `$NEW_CACHE_DIR/CHANGELOG.md`。查找旧版本与新版本之间的所有版本条目。按主题分组，将其总结为 3–7 个要点——重点关注面向用户的变更，跳过内部重构。
...
如果检测到 `DEV_SYMLINK`：告知用户“NotFair 以 dev 符号链接的形式安装——它始终指向你的本地源代码（v$(cat "$CACHE_DIR/VERSION" 2>/dev/null | tr -d '[:space:]')）。无需升级。”并**停止**。不要继续执行步骤 2–8。

---

## 独立使用

直接通过 `/notfair:upgrade` 调用时：

1. 检查 dev 符号链接（参见上面的“Dev 符号链接检测”）。如果检测到，则停止。
...
3. 如果输出 `UPGRADE_AVAILABLE <old> <new>`：按照上面的步骤 2–8 操作。
