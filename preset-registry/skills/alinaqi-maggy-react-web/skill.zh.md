...
**关键：必须在实现代码之前编写测试。对于前端组件，这是不可妥协的要求。**
...
1. 先编写测试文件 → 定义预期行为
2. 运行测试（测试失败）→ 确认测试有效
3. 编写最少量代码 → 只实现足以通过测试的内容
4. 运行测试（测试通过）→ 验证实现
5. 如有需要则重构 → 测试可以捕获回归问题
```
...
- ❌ 在渲染过程中编写逻辑 - 提取到 hooks 中
- ❌ 组件嵌套层级过深 - 展平层级结构
- ❌ 在列表中使用索引作为 key - 使用稳定的 ID
- ❌ 直接修改状态 - 始终使用 setter
- ❌ Prop drilling 超过 2 层 - 使用 context 或状态管理
- ❌ 使用 useEffect 处理派生状态 - 使用 useMemo
- ❌ 在 useEffect 中获取数据 - 使用 React Query
- ❌ 将业务逻辑与 UI 混合 - 保持 core/ 纯粹
- ❌ 组件过大（超过 100 行） - 拆分为更小的部分
- ❌ 使用 JS 对象编写 CSS - 使用 CSS modules 或 Tailwind
