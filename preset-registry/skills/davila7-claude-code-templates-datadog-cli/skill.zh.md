...
一个供 AI 智能体使用 Datadog 日志和指标进行调试与问题分类的 CLI 工具。
...
对于非美国区的 Datadog 站点，请使用 `--site` 标志：
```bash
...
| `logs context` | 获取某个时间戳之前和之后的日志 |
| `logs patterns` | 对相似的日志消息进行分组 |
| `logs compare` | 比较不同时段的日志数量 |
| `logs multi` | 并行运行多个查询 |
| `logs agg` | 按分面聚合日志 |
...
更多调试工作流，请参阅 [workflows.md](references/workflows.md)。