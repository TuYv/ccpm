...
用于使用 Medusa 构建无头电子商务应用——开源、原生支持 Node.js，并且完全可定制。
...
| 特性 | 优势 |
|---------|---------|
| **开源** | 支持自行托管、无供应商锁定，采用 MIT 许可证 |
| **Node.js 原生** | 使用 TypeScript、技术栈熟悉、易于定制 |
| **无头架构** | 可搭配任意前端（Next.js、Remix、移动端） |
| **模块化** | 仅使用所需功能，可扩展任何部分 |
| **内置管理后台** | 包含管理仪表板，并且支持定制 |
...
| `http://localhost:8000` | 店面前端（如果已安装） |
...
| `order.completed` | 订单已履约 |
| `customer.created` | 新客户注册 |
| `product.created` | 添加新产品 |
| `product.updated` | 产品被修改 |
| `inventory.updated` | 库存发生变化 |
...
- **编辑 .medusa 文件夹** - 该文件夹自动生成，并会被覆盖
- **直接访问数据库** - 使用服务和模块
- **复杂操作跳过工作流** - 工作流提供回滚能力
- **硬编码 URL** - 使用环境变量
- **忽略 TypeScript 错误** - 框架依赖类型系统