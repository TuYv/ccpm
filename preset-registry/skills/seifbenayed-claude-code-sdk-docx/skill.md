...
allowed-tools: Document,Read,Write,Edit,Glob,Grep
---
...
```
Document { action: "create", output_path: "report.docx", content: {
  "title": "Quarterly Report",
  "sections": [
    {
      "heading": "Executive Summary",
      "paragraphs": ["Revenue grew 15% YoY.", {"text": "Key highlight", "bold": true}]
    },
    {
      "heading": "Financial Data",
      "table": [["Metric", "Q1", "Q2"], ["Revenue", "100k", "115k"], ["Costs", "80k", "85k"]]
    }
  ]
}}
```
...
```
Document { action: "unpack", file_path: "template.docx", output_path: "/tmp/unpacked/" }
# → Edit XML files in /tmp/unpacked/word/document.xml with the Edit tool
Document { action: "pack", file_path: "/tmp/unpacked/", output_path: "output.docx" }
```

### When to fall back to Python/Bash
- Converting legacy .doc → .docx (requires LibreOffice)
- Accepting tracked changes (requires LibreOffice macro)
- XML validation against OOXML schemas
- Adding comments with commentsExtended/commentsIds (complex XML)

## Overview

A .docx file is a ZIP archive containing XML files.

## Quick Reference (LEGACY — prefer Document tool above)

| Task | Approach |
|------|----------|
| Read/analyze content | `Document { action: "read_text" }` |
| Create new document | `Document { action: "create" }` |
| Edit existing document | `Document { action: "unpack" }` → Edit XML → `Document { action: "pack" }` |
