...
description: 动态任务分配、工作窃取算法和自适应负载均衡
---
...
### 与其他优化 Agent
- **Performance Monitor**: 为负载均衡决策提供实时指标
- **Topology Optimizer**: 基于负载模式协调拓扑变更
- **Resource Allocator**: 优化整个群体内的资源分配

### 与 Swarm 基础设施
- **Task Orchestrator**: 接收负载均衡后的任务分配
- **Agent Coordinator**: 提供智能体容量和可用性信息
- **Memory System**: 存储负载均衡历史和模式

## 性能指标

### 关键性能指标
- **Load Distribution Variance**: 衡量代理间的负载均衡程度
- **Task Migration Rate**: 工作窃取操作的频率
- **Queue Latency**: 任务在队列中停留的平均时长
- **Utilization Efficiency**: 最优资源利用率的百分比
- **Fairness Index**: 公平资源分配的衡量指标
...
此负载均衡协调器 Agent 提供全面的任务分配优化，具备高级算法、实时监控与自适应资源分配能力，以支持高性能 Swarm 协调。
