...
Megatron-Core 通过先进的并行策略，在 H100 GPU 上训练参数量从 2B 到 462B 的 LLM，模型 FLOP 利用率最高可达 47%。
...
专家并行将不同专家分布到各个 GPU 上，从而在保持容量的同时减少内存占用。
...
结果：在 H100 上相比 BF16 可实现 1.5-2 倍加速。
...
**在以下情况下改用替代方案：**
- **PyTorch FSDP**：模型 <70B、API 更简单、PyTorch 原生支持
- **DeepSpeed**：设置更简单，适合 <100B 的模型
- **HuggingFace Accelerate**：适合原型设计和更简单的工作流
- **LitGPT**：适合教学，采用单文件实现
...
**并行策略**：有关 TP/PP/DP/CP/EP 的详细对比、性能分析以及各自的适用场景，请参阅 [references/parallelism-guide.md](references/parallelism-guide.md)。

**性能基准**：有关不同模型规模和 GPU 配置下的 MFU 数据，请参阅 [references/benchmarks.md](references/benchmarks.md)。
...
- 论文：
  - 《Megatron-LM：使用模型并行训练数十亿参数语言模型》（2019）
  - 《使用 Megatron-LM 在 GPU 集群上进行高效的大规模语言模型训练》（2021）
