...
**安全不是可选项。** 每个项目都必须通过安全检查后才能合并。应假定所有输入都是恶意的、所有密钥一旦提交就会泄露，并且所有依赖项都存在漏洞。
...
**绝不要将密钥放入客户端可访问的环境变量中：**

| 框架 | 客户端可访问的前缀 | 仅限服务端 |
|-----------|----------------------|-------------|
| Vite | `VITE_*` | 无前缀 |
| Next.js | `NEXT_PUBLIC_*` | 无前缀 |
| Create React App | `REACT_APP_*` | 不适用（无服务端） |
...
**Vercel 环境变量：**
- 在 Vercel 控制面板中，不带 `VITE_` 前缀的密钥仅限服务端使用
- 只有 `VITE_*` 变量会被打包进客户端代码
- 始终在浏览器开发者工具 → Sources → 你的代码包中验证密钥未被暴露
...
- ❌ 将密钥存储在 `VITE_*`、`NEXT_PUBLIC_*` 或 `REACT_APP_*` 环境变量中（会暴露给客户端！）
- ❌ 将代码或配置文件中的密钥提交到 git
- ❌ .env 文件未加入 .gitignore
- ❌ 使用字符串拼接构造 SQL 查询
- ❌ 未经净化处理便使用 `dangerouslySetInnerHTML`
- ❌ 将用户输入传给 `eval()` 或 `new Function()`
- ❌ 以纯文本或弱哈希（MD5、SHA1）存储密码
- ❌ JWT 没有过期时间或过期时间很长
- ❌ 身份验证端点没有速率限制
- ❌ 记录敏感数据（密码、令牌、PII）
- ❌ 在生产环境中将 CORS 来源设置为 `*`
