...
> moai-adk-go 本地开发参考资料 — 将 `CLAUDE.local.md` §5/§7/§10 中针对特定工作的详细内容迁移至此处（lazy-load）。这些参考资料并非每个会话都需要，用于减少始终加载的上下文。
> 所有者：`hns-*` 命名空间（用户所有，`moai update` 时保留 — CLAUDE.local.md §24）。禁止镜像到模板。
...
moai-adk-go 使用 shell 脚本而非 Python 实现 hook：
...
**关键规则：**
- [HARD] 始终用引号包裹 `$CLAUDE_PROJECT_DIR`：`"$CLAUDE_PROJECT_DIR"`
- [HARD] 使用 hook 包装器脚本的完整路径
