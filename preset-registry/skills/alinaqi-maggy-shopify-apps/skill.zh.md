...
用于使用 Remix、Shopify 应用框架和结账 UI 扩展构建 Shopify 应用。

**来源：** [Shopify 开发者文档](https://shopify.dev/docs/apps) | [Shopify CLI](https://shopify.dev/docs/apps/tools/cli) | [Admin API](https://shopify.dev/docs/api/admin-graphql)
...
**所有 Shopify 应用都必须处理 GDPR webhook。** 这是获得 App Store 审核通过的要求。
...
- **将密钥存储在 metafields 中** - 使用环境变量
- **忽略速率限制** - 实现指数退避
- **跳过 GDPR webhook** - App Store 要求必须实现
- **大型 GraphQL 查询** - 使用分页，并且只查询所需字段
- **轮询更新** - 改用 webhook
