...
**目的：** 构建可离线运行、能像原生应用一样安装，并可在所有设备上提供快速、可靠体验的渐进式 Web 应用。
...
- [ ] 图标：至少包含 192x192 和 512x512 尺寸
- [ ] 包含用于 Android 自适应图标的可遮罩图标
- [ ] `theme_color` 与应用设计相匹配
...
| 策略 | 使用场景 | 说明 |
|----------|----------|-------------|
| **缓存优先** | 静态资源（CSS、JS、图像） | 检查缓存，未命中时回退到网络 |
| **网络优先** | API 响应、动态内容 | 尝试网络，失败时回退到缓存 |
| **立即返回缓存并在后台重新验证** | 半静态内容（头像、文章） | 立即返回缓存，并在后台更新 |
...
有关配置，请参阅上方的 Workbox with Vite 部分。
