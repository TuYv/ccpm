...
vLLM 通过 PagedAttention（基于块的 KV 缓存）和连续批处理（混合预填充/解码请求），实现了标准 transformers 24 倍的吞吐量。
...
验证 TTFT（首个令牌生成时间）< 500ms，并且吞吐量 > 100 req/sec。
...
- **TensorRT-LLM**：仅使用 NVIDIA，且需要极致性能
...
**服务器部署模式**：有关 Docker、Kubernetes 和负载均衡配置，请参阅 [references/server-deployment.md](references/server-deployment.md)。

**性能优化**：有关 PagedAttention 调优、连续批处理详情和基准测试结果，请参阅 [references/optimization.md](references/optimization.md)。

**量化指南**：有关 AWQ/GPTQ/FP8 设置、模型准备和准确率对比，请参阅 [references/quantization.md](references/quantization.md)。
...
- 论文：《使用 PagedAttention 实现大语言模型服务的高效内存管理》（SOSP 2023）
