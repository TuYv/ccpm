...
### HookConsumerWidget（使用 flutter_hooks）
```dart
...
- ❌ **未使用 autoDispose 的 Provider** - 使用 `.autoDispose` 以防止内存泄漏
- ❌ **在回调中使用 watch** - 在 onPressed/回调中使用 `ref.read()`，而不是 `ref.watch()`
- ❌ **在小部件中编写业务逻辑** - 将其移至 Notifiers/providers
- ❌ **Provider 中的可变状态** - 使用 Freezed 创建不可变模型
- ❌ **未使用 AsyncValue** - 使用 `when()` 处理加载/错误状态
- ❌ **将 setState 与 Riverpod 一起使用** - 使用 providers 管理共享状态
- ❌ **向函数传递 ref** - 将 ref 的使用限制在小部件/providers 内
- ❌ **深层嵌套 Consumer** - 改用 ConsumerWidget
- ❌ **未针对参数使用 family** - 对参数化 providers 使用 `.family`
- ❌ **全局 GoRouter 实例** - 使用带有重定向逻辑的 Provider 管理路由器
- ❌ **跨异步操作使用 BuildContext** - 在 await 之前存储值，而不是存储 context
