...
Cloudflare D1 是专为 Cloudflare Workers 设计的无服务器 SQLite 数据库，支持全球分布且无冷启动。
...
**在边缘使用 SQLite，在版本控制中管理迁移，并通过 Drizzle 实现类型安全。**

D1 将 SQLite 的简洁性带入无服务器环境。应针对水平扩展（多个小型数据库）而非垂直扩展（一个大型数据库）进行设计。使用 Drizzle ORM 实现类型安全的查询和迁移。
...
- **单个大型数据库** - 应按多个较小的数据库进行设计（每个租户一个）
- **不使用迁移** - 始终对架构变更进行版本控制
- **到处使用原始 SQL** - 使用 Drizzle 保障类型安全
- **不连接远程数据库** - 部署前始终针对真实 D1 进行测试
- **在 D1 中存储大型二进制对象** - 使用 R2 存储文件
- **复杂连接查询** - D1 基于 SQLite；请保持查询简单
- **不使用批处理** - 对多个操作使用批处理
- **忽略限制** - 使用免费套餐时监控用量