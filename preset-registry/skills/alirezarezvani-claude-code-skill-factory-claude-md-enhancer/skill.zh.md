...
此技能为 Claude Code 项目提供全面的 CLAUDE.md 文件生成和增强功能。它会分析现有文件、依据最佳实践进行验证，并根据你的项目类型、技术栈和团队规模生成定制化指南。

## 功能

- **🆕 交互式初始化**：智能工作流会探索你的代码仓库、检测项目类型和技术栈、请求确认，然后创建定制化的 CLAUDE.md 文件
- **✨ 100% 符合原生格式**：所有生成的文件均遵循 Claude Code 官方格式，包含项目结构图、设置说明、架构章节和文件结构说明（与 `/update-claude-md` 斜杠命令保持一致）
- **分析现有文件**：扫描并评估当前 CLAUDE.md 文件的结构、完整性和质量
- **验证最佳实践**：依据 Anthropic 指南进行检查（文件长度、必需章节、格式标准）
- **生成新文件**：为新项目从头创建完整的 CLAUDE.md 文件
- **增强现有文件**：添加缺失章节、改进结构，并更新至最新最佳实践
- **模块化架构**：支持在子目录（backend/、frontend/、docs/）中使用特定于上下文的 CLAUDE.md 文件
...
- 如果推荐使用模块化结构，则创建模块化文件（backend/、frontend/ 等）
- 应用特定于技术栈的最佳实践
...
### validator.py
依据最佳实践和 Anthropic 指南验证 CLAUDE.md 文件。

**关键函数**：
- `validate_length()` - 检查文件长度（超过 300 行时发出警告）
...
- `generate_context_file()` - 创建特定上下文的文件（后端、前端等）
- `generate_section()` - 生成单独的章节（技术栈、工作流等）
...
**“在宣布完成之前，始终对照官方原生示例验证输出。”**

在最终确定任何 CLAUDE.md 生成内容之前：
1. 将输出与 `/update-claude-md` 斜杠命令格式进行比较
2. 查阅 Claude Code 官方文档，确认所需章节
3. 验证所有原生格式章节均已包含（概述、项目结构、文件结构、设置与安装、架构等）
4. 与 `examples/` 文件夹中的参考示例进行交叉核对

### 对于新项目
1. 从最小模板（50-100 行）开始，并根据需要逐步扩展
2. 对包含 3 个以上主要组件的项目使用模块化架构
3. 立即包含技术栈参考信息
4. 在团队规模超过 5 人之前添加工作流说明
...
### 不适用的情况
- 用于非 Claude AI 工具（此功能专用于 Claude Code）
- 用于不使用 Claude Code 或类似 AI 助手的项目
- 当你需要高度专业化的领域指南时（法律、医疗合规）

## 模板类别

### 按规模
- **最小型**（50 行）——个人开发者、原型、黑客松
- **核心型**（100-150 行）——小型团队、MVP、标准项目
- **详细型**（200-300 行）——大型团队、生产系统、企业级项目

### 按项目类型
- **Web 应用** - 以前端为主（React、Vue、Angular）
- **API** - 后端服务（REST、GraphQL、微服务）
- **全栈** - 集成前端与后端
- **CLI** - 命令行工具和实用程序
- **库** - 可复用的软件包和框架
- **移动端** - React Native、Flutter、原生 iOS/Android
...
请记住：目标是让 Claude 更高效、更具上下文感知能力，而不是制造官僚流程。从简单做起，根据实际使用情况持续迭代，并尽可能自动执行质量检查。