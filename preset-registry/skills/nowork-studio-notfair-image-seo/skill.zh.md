...
收集**目标**（`$URL`）——可以是一个页面、一个模板（例如所有产品页面）或整个网站。如果范围较广，请选择 3–5 个具有代表性的页面；图片问题通常存在于模板层面。

## 阶段 0 — 预检与数据

阅读并遵循 `../shared/preamble.md`。GSC 为可选项——如果已连接，请使用**搜索结果 → 搜索外观 → 图片**过滤条件，查看当前的图片流量以及已经从中获得流量的页面。

## 阶段 1 — 抓取图片

对于每个页面，提取所有 `<img>`、`<picture>`/`<source>`，以及承载实际含义的 CSS 背景图片，并记录：`src`、`alt`、固有尺寸、字节大小、格式、`loading`、`srcset`/`sizes`、显式设置的 `width`/`height`。

## 阶段 2 — 审计每张图片

- **替代文本**——应存在、具有描述性、与关键词相关（但不能堆砌关键词）；仅装饰性图片可使用空的 `alt=""`。标记缺失、重复或类似 `"image123.jpg"` 的替代文本。
- **文件名**——应具有描述性、使用连字符分隔，并采用小写 ASCII 字符（例如
  `automatic-sliding-door.webp`），而不是 `IMG_4821.JPG`。
- **格式**——照片使用 WebP/AVIF；图标和线稿使用 SVG；标记体积过大的 PNG/JPEG。
- **体积**——标记超过约 150–200 KB，或远大于其显示尺寸所需大小的图片。
- **响应式**——应提供 `srcset`/`sizes`，避免移动设备下载桌面端资源。
- **CLS**——为每个 img 显式设置 `width`/`height`（或宽高比），以预留空间。
- **延迟加载**——首屏以下的图片使用 `loading="lazy"`；LCP/主视觉图片绝不能进行延迟加载（这是一个常见且代价高昂的错误——请务必检查）。
...
输出：一个**图片 SEO 评分**、一张逐图片问题表（图片 | 问题 |
修复方案）、**按影响程度排序的首要修复项**（通常包括：补充缺失的替代文本、转换为 WebP、修复被延迟加载的 LCP 图片、添加 width/height），以及对预期 CWV 影响的说明。使用用户的语言撰写。