...
requires:
  mcp: [rube]
---
# 通过 Rube MCP 实现 Adrapid 自动化

通过 Rube MCP 使用 Composio 的 Adrapid 工具包来自动化 Adrapid 操作。
...
- 通过 `RUBE_MANAGE_CONNECTIONS` 使用工具包 `adrapid` 建立活动中的 Adrapid 连接
...
2. 调用 `RUBE_MANAGE_CONNECTIONS` 并使用工具包 `adrapid`
3. 如果连接未显示为 ACTIVE，请按返回的授权链接完成设置
...
这会返回可用的工具标识符、输入架构、推荐执行计划和已知注意事项。
...
| 操作 | 方法 |
|-----------|----------|
| 查找工具 | 使用针对 Adrapid 的 use case 调用 `RUBE_SEARCH_TOOLS` |
| 连接 | 使用工具包 `adrapid` 调用 `RUBE_MANAGE_CONNECTIONS` |
| 执行 | 使用发现到的工具标识符调用 `RUBE_MULTI_EXECUTE_TOOL` |
| 批量操作 | 使用 `RUBE_REMOTE_WORKBENCH` 与 `run_composio_tool()` |
| 完整架构 | 对带有 `schemaRef` 的工具使用 `RUBE_GET_TOOL_SCHEMAS` |
