...
适用于具有实时数据、离线支持和安全规则的 Web 和移动应用程序的 Firebase/Firestore 模式。

**来源：** [Firebase 文档](https://firebase.google.com/docs) | [Firestore 最佳实践](https://firebase.google.com/docs/firestore/best-practices) | [安全规则](https://firebase.google.com/docs/rules)

---

## 核心原则

**有目的地进行反规范化，通过规则保障安全，并进行水平扩展。**

Firestore 是一个文档数据库——应利用反规范化来提高读取效率。安全规则是服务端验证机制。请根据访问模式进行设计。
...
| **Authentication** | 用户身份验证、OAuth、匿名会话 |
| **Storage** | 具有安全规则的文件上传服务 |
...
- **没有安全规则** - 始终编写规则，切勿在生产环境中使用测试模式
- **嵌套过深** - 保持文档扁平化，最多嵌套 2-3 层
- **文档过大** - 最大 1MB，超过则进行拆分
- **无界数组** - 对持续增长的列表使用子集合
- **没有离线处理** - 为移动应用/PWA 启用持久化
- **读取所有字段** - 使用字段掩码或 Firestore Lite
- **忽略索引** - 检查控制台中是否存在缺少索引的错误
- **没有使用模拟器测试** - 部署前始终测试规则