...
使用此技能构建、运行和操作 DNAnexus 工作负载，而无需臆测平台语义。
涵盖：
...
当行为可能已发生变化时，请查阅 `references/sources.md` 和当前版本说明。

## 操作规范

DNAnexus 操作可能暴露受监管数据、删除不可变对象、更改权限，或产生计算和出口费用。遵循以下规则：

1. 从只读操作开始。在执行变更前确认用户、项目 ID、区域、文件夹、对象 ID
   和执行目标。
2. 在执行计费启动、会产生大量出口流量的上传或下载、归档/取消归档请求、删除、项目移除、
   权限更改、令牌撤销或应用发布前获取确认，除非用户已经明确请求了该确切操作及目标。
3. 在破坏性操作前显示已解析的 ID 和影响。绝不要根据不唯一的名称推断删除目标。
4. 绝不要打印、记录、返回或持久化 `DX_SECURITY_CONTEXT` 或 API 令牌。
   不要在捕获的日志中运行 `dx env` 或 `dx env --bash`，因为二者都会暴露当前令牌。
...
8. 优先使用可复现的依赖项、范围狭窄的网络允许列表、明确的输出文件夹、成本限制和有界等待。
...
对于非交互式环境，只通过环境或密钥管理器注入指定的 DNAnexus 密钥。绝不要回显它、将其包含在命令输出中、
提交它，或检查整个环境。参见
...
如果 shell 环境变量与已保存的 CLI 会话发生冲突，请遵循
...
Upload Agent 默认会压缩未压缩的输入，并追加 `.gz`。如果需要逐字节保留内容或必须保留原始名称，请使用
`--do-not-compress`。请参阅
`references/data-operations.md`。

### 使用 dxpy 进行精确搜索

除非提供 `name_mode`，否则 `find_data_objects()` 使用精确的名称匹配。不要在未指定 `name_mode="glob"` 的情况下传入 `"*.bam"`。
...
通过指定项目、文件夹、时间范围和 `limit`，限制范围过大的搜索。
...
相对于此 skill 目录解析捆绑的辅助程序。从 skill 根目录执行：
...
`regionalOptions.<region>.systemRequirements`。`dxapp.json` 顶层的 `resources` 和
...
保留交互式使用时的正常确认提示。仅在确切的可执行文件、项目、输入、目标位置和费用策略均已获批准的审查后自动化流程中添加 `--yes`。
...
`DXJobFailureError`。在对其进行分类之前，重新描述远程状态；参见
...
- 在 Ubuntu 24.04 中，即使 AEE 设置了
  `PIP_BREAK_SYSTEM_PACKAGES=1`，也应优先为 Python 依赖使用虚拟环境；否则系统依赖与 PyPI 依赖之间的冲突可能导致
  `DXExecDependencyError`。
- 运行时的 `execDepends` 可能发生漂移。对于生产环境，优先使用固定版本的资源包、打包的依赖项或固定版本的容器。
- 动态实例选择通过 `instanceTypeSelector.allowedInstanceTypes` 配置，并且可能需要组织许可证。
- `AppInsufficientResourceError` 之后的自动扩容，同时需要执行重启策略和允许升级实例的组织策略。
- 创建或更新应用/app­let 时会拒绝已退役的实例类型。应发现可用的实例类型，而不是复制过时的列表。
- 作业通常有 30 天的运行时限。
- 当前 API/CLI 会展示下载安全状态。除非用户明确批准安全隔离流程，否则应将恶意文件警告视为停止条件。

## 打包的辅助工具

下面的命令假设当前目录是此 skill 的根目录。否则，请根据已加载的 skill 目录解析
`scripts/` 的相对路径。
...
此离线验证器可以捕获结构错误、已弃用的位置、过于宽泛的访问权限以及不一致的区域要求。它是对
`dx build` 验证的补充，而不是替代。
...
这会执行离线符号和签名检查。不会进行身份验证，也不会发起网络请求。
...
- `references/job-execution.md` — 作业、分析、监控、链式调用、复用、重试和成本控制
- `references/workflow-languages.md` — 原生工作流、使用 `dxCompiler` 的 WDL/CWL 以及 Nextflow
- `references/operations-and-troubleshooting.md` — 运维操作手册和故障诊断
