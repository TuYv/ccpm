...
阅读 `../shared/operating-contract.md`。将工具发现结果视为事实依据；产品文档描述的是预期用途，而不是当前会话的授权状态。

## 承诺前先验证

1. 检查可用工具列表。在通用 NotFair MCP 上，首先调用 `listConnectedPlatforms`。请求使用 Google 或 Meta 时，请使用对应的共享前置说明；对于 X Ads 和 LinkedIn Ads，请解析带平台前缀的工具，并在声称拥有访问权限之前执行一次无风险的账户/设置读取操作。
2. 当连接器提供无风险的账户列表或读取操作时，使用该操作确认 OAuth/账户访问权限。
3. 记录确切的平台、所选账户、可访问的日期范围，以及该接口是只读还是支持变更操作。
...
| TikTok、Amazon、ChatGPT Ads | 此插件未声明第一方 NotFair 接口 | 请求经过验证的连接器或导出数据；仅进行规划/审核 |

绝不要虚构路由器、端点、工具名称、账户 ID 或平台能力。仅当用户询问时，才可引用当前官方文档中的价格、配额和平台资格要求；不要根据某个方案或其他连接器推断这些信息。