...
阅读并遵循 `../shared/preamble.md`（MCP 检测、账号选择）和 `../shared/analysis-principles.md`（证据要求、防护规则）。二者适用于本技能的整个执行过程——以下每个维度都是测量结果，而非主观意见。

# 落地页评分与诊断

Google Ads 广告系列失败在落地页上的情况比失败在竞价环节的情况更常见。一个出色的 RSA，如果将流量导向缓慢、重点不明确或信息不匹配的页面，会造成双重预算浪费——一次浪费在点击上，另一次浪费在转化流失上。本技能从 **5 个加权维度**对落地页进行评分，并给出具体的修复措施。

仅对实际承接广告流量的页面进行评分。不要为随机的营销页面评分。以下情况应运行本技能：用户直接提出请求时；由 `/google-ads-audit` 自动移交时（高 CTR / 低 CVR 的广告组）；QS 诊断标记“Landing Page Experience: Below Average”时；或者在 `/google-ads-copy` 为一个从未经过验证的页面撰写新文案之前，将其作为预检步骤运行。

当问题涉及广告与页面的契合度、高 CTR / 低 CVR、LPX，或广告与落地页的联合测试时，请在评分前阅读 `references/message-chain-testing.md`。它能让诊断聚焦于付费搜索的信息链，避免偏离为通用的网页设计审计。

## 参考资料

- `references/scoring-rubric.md` — 包含 5 个维度的加权评分标准、阈值和证据字段。评分前请先阅读。
- `references/message-chain-testing.md` — 查询 → 广告 → 页面信息链诊断，以及广告与落地页的测试设计。
- `../manage/references/quality-score-framework.md` — 仅当用户的明确目标是提升 QS 时使用。

## 阶段 1：确定目标页面

确定要对哪些 URL 进行评分。优先级顺序如下：

1. **用户提供了 URL** — 对该页面进行评分，跳过发现流程。
2. **用户提供了广告组或广告系列名称** — 使用 `runScript` 对 `ad_group_ad` 执行 GAQL 查询，并筛选到该广告组；提取唯一的 `final_urls`。进行规范化处理（移除跟踪参数，保留会影响路由的路径和查询参数）。
3. **由 `/google-ads-audit` 自动移交** — 移交内容会包含被标记的具体广告组。以相同方式提取其最终 URL。
4. **没有参数** — 使用 `runScript` 在整个账号中执行 `ad_group_ad` 查询，按过去 30 天的支出对最终 URL 进行排名，提出排名前 3 的 URL，并请用户确认。

**积极去重。** 许多广告会指向同一个最终 URL——每个唯一 URL 只评分一次，然后将评分结果映射回使用该 URL 的所有广告组。

## 阶段 2：收集信号（并行）

在单次工具调用轮次中完成以下所有操作：

1. **使用 WebFetch 获取落地页** — 采集可见的标题、副标题、主要 CTA 文本、表单字段、信任信号和正文文案语气。采集完整 HTML，以便发现脚本臃肿和首屏内容方面的问题。
2. **调用 PageSpeed Insights API** — 通过 WebFetch 请求 `https://www.googleapis.com/pagespeedonline/v5/runPagespeed?url={url}&strategy=mobile&category=performance&category=accessibility&category=best-practices&category=seo`。单 URL 查询不需要 API 密钥。从 `lighthouseResult.audits` 中提取 LCP、CLS、INP、TTI、性能得分以及排名前 3 的优化机会。
3. **提取引流广告文案和广告组的转化指标** — 使用一次 `runScript` 调用，通过 `ads.gaqlParallel` 查询 `ad_group_ad`（获取标题和描述文本——作为信息匹配基准）以及 `ad_group` 或 `keyword_view`（获取点击次数、转化次数、CVR——用于为金额影响估算提供数据依据）。一次调用即可涵盖两者。
4. **读取 `{data_dir}/business-context.json`** — 获取品牌调性、差异化优势、优惠和目标受众。如果文件缺失，请引导用户先运行 `/google-ads-audit`。不要猜测业务信息。

如果任何单次调用失败，继续执行——在报告中注明数据缺口，而不是阻塞流程。PageSpeed Insights 可能会触发速率限制；如果发生这种情况，则回退到手动计时注释（"PSI unavailable — could not score Page Speed"），并降低最终报告的置信度，而不是跳过该维度。

## 阶段 3：为页面评分

阅读 `references/scoring-rubric.md`，根据证据为每个维度给出 0-100 分。各维度分数都是真实测量结果（PageSpeed Insights 数值、逐字文案对比、表单字段数量等）——它们不是人为评级，而是观测结果。

仅将加权综合分计算为下方金额提升公式所需的**内部参考数值**。不要将其呈现为字母等级。用户看到的是各维度的测量结果和预计金额提升——综合分只是内部衔接数据。
...
**金额提升是核心结论。**如果 `business-context.json.unit_economics` 包含 `aov_usd` + `profit_margin`，则计算将综合分提高 15 分所带来的预计每月提升（参见 `../shared/ppc-math.md`）：
...
将提升表述为 `fixing this page is worth ~$X/mo in profit`——绝不要将其表述为保证。将 CVR 提升上限设为 50%，并将分数提升上限设为 15 分，可以避免估算脱离现实。如果没有 `unit_economics`，则完全跳过金额行，不要编造数字——各维度测量结果本身仍然成立。

## 阶段 4：交付报告

最多 60 行。以金额提升（如有）和最重要的一项修复措施开头。不要使用字母等级。
...
`internal_composite` 仅用于趋势跟踪——它是金额提升公式使用的内部参考数值，绝不会以字母评级的形式展示给用户。后续针对同一 URL 运行时，对比各维度的原始测量值和金额提升：`LCP 4.2s → 2.1s · Page Speed 45 → 78 · estimated lift $380/mo → $120/mo remaining`。三项测量值发生了变化，没有人为制造的评级跳变。

## 规则

1. **未使用 WebFetch 获取页面时，绝不能对其评分。** 评分标准要求提供证据。没有 WebFetch = 没有评分。如果页面受限或需要身份验证，请用户协助。
2. **绝不能报告未经实际测量的 PSI 数值。** 如果 PSI 失败，请说明“PSI 不可用”——不要估算。
3. **除非用户要求多个页面，否则一次只处理一个页面。** 在一轮中对三个页面评分会产生难以阅读的报告。仅在用户明确要求时进行批量处理。
4. **不要在这里重写文案。** 此技能用于诊断页面。需要新标题时转交给 `/google-ads-copy`，需要调整出价、否定关键词或预算时转交给 `/google-ads`。
5. **考虑利润率的金额影响需要经过验证的单位经济数据。** 如果 `unit_economics.source == "inferred_from_template"`，请在提升金额所在行追加 `_(using industry defaults — confirm your AOV/margin for sharper estimates)_`。
6. **始终持久化保存。** 每个已评分的页面都必须写入 `landing-page-history.json`，即使用户没有提出要求——未来的审计依赖该基准。