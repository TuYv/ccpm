...
适用于能够让搜索引擎和 AI 爬虫（GPTBot、ClaudeBot、PerplexityBot）发现内容的技术网站结构。
...
埋没在糟糕架构中的优质内容不会被发现。本技能涵盖相关技术基础，使以下平台能够发现你的内容：
- Google、Bing（传统搜索引擎）
...
| 规则 | 示例 |
|------|---------|
| 仅使用小写字母 | 使用 `/blog/my-post`，而不是 `/Blog/My-Post` |
| 使用连字符而非下划线 | 使用 `/my-page`，而不是 `/my_page` |
| 不使用尾部斜杠 | 使用 `/about`，而不是 `/about/` |
| 使用描述性 slug | 使用 `/pricing`，而不是 `/p` |
| 内容 URL 不使用查询参数 | 使用 `/blog/post-title`，而不是 `/blog?id=123` |
| 最多深入 3-4 层 | `/blog/category/post` |
