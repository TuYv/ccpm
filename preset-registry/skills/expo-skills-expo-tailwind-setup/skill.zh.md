...
license: MIT
---
# 使用 react-native-css 为 Expo 配置 Tailwind CSS

本指南介绍如何在 Expo 中使用 react-native-css 和 NativeWind v5 配置 Tailwind CSS v4，以便在 iOS、Android 和 Web 上实现通用样式。

## 概述

此配置使用：

- **Tailwind CSS v4** - 现代的 CSS 优先配置
...
添加 resolutions 以确保 lightningcss 兼容性：
...
- Expo 中不需要 autoprefixer，因为使用了 lightningcss
- expo 默认已包含 postcss
...
使用 Tailwind v4 和 NativeWind v5 时，不需要为 Tailwind 配置 babel.config.js。如果存在任何 NativeWind babel 预设，请将其删除：
...
由于 react-native-css 要求显式包装 CSS 元素，因此需要创建可复用组件：
...
1. **不需要 babel.config.js** - 现在采用 CSS 优先的配置方式
...
5. **组件包装器** - 必须使用 `useCssElement` 包装组件，以支持 className
...
2. 对 Web/Android 使用 `light-dark()` 作为回退方案

### TypeScript 错误

将 className 添加到组件 props 中：
...
## 提交反馈
如果你发现此 Skill 中存在错误、误导性信息或过时信息，请进行报告，以便 Expo 改进：
```bash
