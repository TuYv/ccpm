...
metadata:
  ...
  category: seo
---
# FLOW 框架：Find · Leverage · Optimize · Win

FLOW 是一个以证据为导向、专为 AI 搜索时代打造的 SEO 运营模型。Claude SEO
集成了 FLOW 提示词库（分为 5 个阶段，共 41 个提示词），因此每项分析都可以由结构化、基于证据的 AI 提示词驱动，而不是依赖临时编写的查询。
...
| `/seo flow find [url\|topic]` | Find 阶段：关键词研究、差距分析、SERP 意图映射（5 个提示词） |
| `/seo flow leverage [url]` | Leverage 阶段：反向链接策略、站外权威建设（1 个提示词） |
| `/seo flow optimize [url]` | Optimize 阶段：根据上下文从 21 个提示词中选择最相关的 2-3 个 |
| `/seo flow win [url]` | Win 阶段：BOFU、转化率、双表面评分卡（3 个提示词） |
| `/seo flow local [url]` | Local 阶段：GBP 优化、元数据、标题标签、本地 SEO 审计（11 个提示词） |
...
2. 读取之前的分析上下文（URL、行业垂直领域，以及对话中之前的任何 skill 输出）
...
Optimize 阶段有 21 个提示词。全部输出会产生噪声，请按优先级进行选择：

1. **行业垂直领域**（SaaS → 页面内优化 + 技术优化；本地业务 → 引用 + GBP；出版商 → E-E-A-T + 内容时效性）
2. **之前的技能输出**（seo-technical 标记了抓取问题 → 技术优化提示词；seo-content 标记了 E-E-A-T 缺口 → 内容优化提示词）
3. **URL 信号**（产品页面 → 转化；博客 → 内容时效性 + 权威性）
...
按需加载，不要在启动时全部加载：
- `references/flow-framework.md`：FLOW 运营模型（每次激活 `/seo flow` 时加载）
...
- `references/prompts/optimize/`：21 个提示词；用于 `/seo flow optimize` 时按需选择性加载
...
| 场景 | 操作 |
|----------|--------|
| 缺少 `references/flow-framework.md` | “FLOW 参考文件未同步。运行：`/seo flow sync`” |
