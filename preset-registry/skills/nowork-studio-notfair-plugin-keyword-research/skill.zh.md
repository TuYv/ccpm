...
发现、分析并确定用于 SEO 和 GEO 内容策略的关键词优先级。根据搜索量、竞争程度、意图和业务相关性识别高价值机会。

## 必须触发此技能的情况

当对话涉及以下任何情况时，都应使用此技能——即使用户没有使用 SEO 术语：

只要任务需要可复用、能够影响策略的市场情报，而不只是临时答案，就应使用此技能。
...
> **注意：**所有集成都不是必需的。即使没有任何 API 密钥，此技能也可以工作——未连接任何工具时，由用户手动提供数据。
...
2. 目标受众和地理位置
3. 业务目标（流量、潜在客户、销售）
4. 当前域名权威度（如果已知）或网站年限
5. 任何已知的关键词效果数据或搜索量估算

使用所提供的数据执行完整分析。在输出中注明哪些指标来自自动采集，哪些来自用户提供的数据。
...
4. **对搜索意图进行分类**
   ...
   | 意图 | 信号词 | 示例 | 内容类型 |
   |--------|---------|---------|--------------|
   | 信息型 | 什么、如何、为什么、指南、学习 | “什么是 SEO” | 博客文章、指南 |
   | 导航型 | 品牌名称、特定网站 | “google analytics 登录” | 主页、产品页面 |
   | 商业型 | 最佳、评测、对比、比较 | “[当前年份] 最佳 SEO 工具” | 对比文章、评测 |
   | 交易型 | 购买、价格、折扣、下单 | “购买 SEO 软件” | 产品页面、定价页面 |
...
9. **生成输出报告**
   ...
   生成一份包含以下内容的报告：执行摘要、主要关键词机会（快速见效、增长、GEO）、主题集群、内容日历和后续步骤。
...
- **竞品差距**：`What keywords do [competitor 1], [competitor 2] rank for that I'm missing?`
- **本地关键词**：`Research local keywords for [business type] in [city/region]`

## 成功要点

1. **从种子关键词开始**，用它们描述你的核心产品或服务
2. **不要忽视长尾关键词**——它们通常具有最高的转化率
3. **让内容匹配意图**——信息型查询需要指南，而不是销售页面
4. **将内容组织成集群**，以建立主题权威性
5. **优先处理可快速见效的机会**，以积累势头和可信度
6. **将 GEO 关键词纳入策略**，以提高在 AI 中的可见性
