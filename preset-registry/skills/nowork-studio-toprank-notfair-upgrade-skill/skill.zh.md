...
| 更新状态 | `~/.toprank/`（特意保留——参见 CHANGELOG 0.24.0） |

---

## 内联升级流程

当技能前置流程输出 `UPGRADE_AVAILABLE` 时，使用本节。

### 第 1 步：自动升级

记录“正在升级 NotFair v{old} → v{new}...”并继续执行第 2 步。
...
如果复制失败，则发出警告：“升级失败——旧版本仍处于活动状态。请手动运行 `/notfair:upgrade`。”然后停止。
...
删除旧的版本化缓存目录（仅保留新目录）。切勿删除 `dev` 符号链接：
...
读取 `$NEW_CACHE_DIR/CHANGELOG.md`。查找旧版本与新版本之间的所有版本条目。按主题分组并总结为 3–7 个要点——重点关注面向用户的变更，跳过内部重构。
...
如果检测到 `DEV_SYMLINK`：告知用户“NotFair 是以开发符号链接的形式安装的——它始终指向你的本地源代码（v$(cat "$CACHE_DIR/VERSION" 2>/dev/null | tr -d '[:space:]')）。无需升级。”然后**停止**。不要继续执行步骤 2–8。
...
1. 检查开发符号链接（参见上面的“开发符号链接检测”）。如果检测到，则停止。

2. 强制执行全新的更新检查（绕过缓存和延后状态）：
```bash
