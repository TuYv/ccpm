...
**目的：** 针对 AI 引擎（ChatGPT、Claude、Perplexity、Google AI Overviews）优化内容，使你的品牌在 AI 生成的答案中被引用。
...
- ChatGPT 提示词平均包含 23 个单词（Google 为 4.2 个）
...
出现在多个可信来源中的事实更容易获得信任和被重复使用。

**如何建立共识：**
- 在自己的各个页面中一致地重复关键事实
- 使用与行业领导者相同的术语
- 链接到权威外部来源，并争取从这些来源获得链接
- 创建相互强化的内部内容集群

### 2. 信息增益

全新的洞见胜过泛泛的建议。AI 引擎更偏好能够带来额外价值的内容。
...
明确的实体和整洁的结构可以减少歧义，并提高内容被引用的可能性。
...
**链接规则：**
- Category Explainer 向下链接到所有产品页面
- 产品页面向上链接到 Category Explainer
- 产品页面横向链接到相关功能
- 博客文章向上链接到产品页面
- 对比页面链接到 Category Explainer + 相关产品页面
...
- [ ] 遵循 Feature → How → Outcome 模式
- [ ] 包含 2-4 个句子（便于快速浏览）
- [ ] 包含 1-2 个语义三元组
- [ ] 指明具体实体（不要使用含糊的“它”或“此项”）
- [ ] 使用主动语态动词
...
- [ ] 每个关键类别都存在 Category Explainer
- [ ] 产品页面链接回 Category Explainer
- [ ] 已记录内容集群架构
- [ ] 存在包含资历信息的作者简介页面
- [ ] 所有页面使用一致的术语
...
| **AI 中的品牌提及** | 在 AI 引擎中搜索“[brand] + [category]” |
| **答案份额** | 与竞争对手相比，你被引用的频率 |
| **LLM 流量** | 来自 chatgpt.com、claude.ai、perplexity.ai 的 GA4 引荐 |
| **展示次数与点击次数差距** | GSC 展示次数与实际点击次数对比 |

### 工具

- **HubSpot AEO Grader** - 评估品牌的 AI 可见度
- **Google Analytics 4** - 跟踪 LLM 引荐流量
- **Google Search Console** - 监控展示次数与点击次数之间的差距
- **Manual AI queries** - 定期在 AI 引擎中测试品牌
...
| 错误 | 修复方法 |
|---------|-----|
| 语言模糊（“它可以帮助处理一些事情”） | 使用具体的实体和三元组 |
| 没有清晰的结构 | 使用功能 → 方式 → 结果 |
| 缺少结构化数据 | 添加 Article + FAQ 结构化数据 |
| 没有作者署名 | 添加作者姓名、简介和资历 |
| 内容泛泛而谈 | 添加原创数据、示例和观点 |
| 孤立页面 | 将页面链接到内容集群中 |
| 模棱两可（“视情况而定”） | 明确表明立场 |
...
| 方面 | 传统 SEO | AEO |
|--------|-----------------|-----|
| **目标** | 排名进入第 1 页 | 在 AI 回答中被引用 |
| **成功指标** | 点击率 | 回答中的占比 |
| **内容重点** | 关键词 | 实体 + 事实 |
| **结构** | 便于浏览的标题 | 便于提取的三元组 |
| **链接** | 通过反向链接建立权威性 | 通过引用建立共识 |
| **更新方式** | 定期刷新 | 持续确保准确性 |
