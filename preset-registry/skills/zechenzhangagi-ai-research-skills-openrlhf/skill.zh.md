...
OpenRLHF 是一个基于 Ray 的 RLHF 框架，针对分布式训练进行了优化，并支持 vLLM 推理加速。
...
- 需要基于 Ray 的分布式架构
- 拥有多节点 GPU 集群
- 需要在单一框架中使用 PPO/GRPO/RLOO/DPO

**算法选择**：
- **PPO**：控制力最强，最适合复杂奖励
- **GRPO**：显存高效，无需 critic
- **RLOO**：带逐 token KL 的改进版 PPO
...
- **DeepSpeedChat**：与 DeepSpeed 生态集成
...
**混合引擎 GPU 共享**：关于 vLLM 休眠模式、DeepSpeed 休眠模式以及最优节点分配，参见 [references/hybrid-engine.md](references/hybrid-engine.md)。

**算法对比**：关于 PPO、GRPO、RLOO 与 REINFORCE++ 的基准测试和超参数，参见 [references/algorithm-comparison.md](references/algorithm-comparison.md)。

**多节点配置**：关于 Ray 集群配置和容错，参见 [references/multi-node-training.md](references/multi-node-training.md)。

**自定义奖励函数**：关于强化微调和智能体 RLHF，参见 [references/custom-rewards.md](references/custom-rewards.md)。
