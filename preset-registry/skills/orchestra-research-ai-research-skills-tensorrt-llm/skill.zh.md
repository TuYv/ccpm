...
NVIDIA 开源的库，用于在 NVIDIA GPU 上以业界领先的性能优化 LLM 推理。

## 何时使用 TensorRT-LLM

**在以下情况下使用 TensorRT-LLM：**
- 部署在 NVIDIA GPU（A100、H100、GB200）上
- 需要最大吞吐量（Llama 3 上达到 24,000+ tokens/sec）
- 实时应用需要低延迟
- 使用量化模型（FP8、INT4、FP4）
- 跨多个 GPU 或节点进行扩展

**在以下情况下改用 vLLM：**
- 需要更简单的设置和以 Python 为首要语言的 API
- 希望使用 PagedAttention，而无需 TensorRT 编译
- 使用 AMD GPU 或非 NVIDIA 硬件

**在以下情况下改用 llama.cpp：**
- 部署在 CPU 或 Apple Silicon 上
- 需要在没有 NVIDIA GPU 的环境中进行边缘部署
- 希望使用更简单的 GGUF 量化格式
...
- **Flash Attention**：经过优化的注意力内核
- **Quantization**：FP8、INT4、FP4，可实现 2-4× 更快的推理
- **CUDA graphs**：降低内核启动开销

### 并行化
- **Tensor parallelism (TP)**：将模型拆分到多个 GPU 上
- **Pipeline parallelism (PP)**：按层分配
- **Expert parallelism**：适用于 Mixture-of-Experts 模型
- **Multi-node**：扩展到单台机器之外

### 高级功能
- **Speculative decoding**：使用草稿模型加速生成
- **LoRA serving**：高效部署多个适配器
- **Disaggregated serving**：分离 prefill 和 generation
