...
用于构建能够使用工具执行多步骤任务的自主 AI 智能体。

**来源：** [Claude Agent SDK](https://docs.anthropic.com/en/docs/agents-and-tools/claude-agent-sdk) | [Anthropic Claude Code 最佳实践](https://www.anthropic.com/engineering/claude-code-best-practices) | [Pydantic AI](https://ai.pydantic.dev/) | [Google Gemini 智能体开发](https://developers.googleblog.com/en/building-agents-google-gemini-open-source-frameworks/) | [OpenAI 构建智能体](https://developers.openai.com/tracks/building-agents/)
...
| 语言/框架 | 默认选择 | 原因 |
|-------------------|---------|-----|
| **Python** | **Pydantic AI** | 类型安全、Pydantic 验证、支持多模型、可用于生产环境 |
| **Node.js / Next.js** | **Claude Agent SDK** | Anthropic 官方 SDK、工具支持、多智能体、原生流式传输 |
...
在执行前先进行研究和规划的智能体，表现始终优于直接采取行动的智能体。将复杂任务拆分为可验证的步骤，审慎使用工具，并在整个执行过程中保持清晰的状态。
...
| 通用 | gpt-4.1, claude-sonnet | 均衡 |
| 复杂推理 | o4-mini, claude-opus | 更高准确性 |
| 深度规划 | gpt-5 + reasoning, ultrathink | 最强能力 |
...
- **执行前不做规划** - 直接采取行动的智能体更容易出错
- **单体智能体** - 一个拥有 50 个工具的智能体会陷入混乱
- **不进行验证** - 智能体必须验证自己的工作
- **硬编码工具调用顺序** - 让模型决定工具顺序
- **缺少防护措施** - 所有智能体都需要安全边界
- **没有状态管理** - 在多次工具调用之间丢失上下文
- **只测试正常路径** - 测试失败情况和边界情况
- **忽略模型差异** - 推理模型需要不同的提示词
- **不跟踪成本** - 智能体工作流的成本可能很高
- **在没有监督的情况下完全自动化** - 对关键操作采用人在回路机制
