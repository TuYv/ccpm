...
创建高转化的对比页面和替代方案页面，针对竞品意图关键词，并提供准确、结构化的内容。
...
### CTA 放置位置
- **首屏上方**：简短的对比总结以及主要 CTA
- **对比表之后**："Try [Your Product] free" CTA
- **页面底部**：最终推荐以及 CTA
...
- **及时更新**：在竞品发布重大变更后进行审核和更新
- **披露关联关系**：明确说明哪个产品属于自己
- **平衡呈现**：如实承认竞品的优势
- **价格准确性**：所有价格数据都应添加 "as of [date]" 免责声明
...
| 场景 | 操作 |
|----------|--------|
| 竞争对手 URL 无法访问 | 报告哪些竞争对手 URL 访问失败。使用可用数据继续处理，并说明对比中的数据缺口。 |
| 竞争对手数据不足（无法获取定价或功能信息） | 清楚标记缺失的数据点。在对比表中使用“公开信息不可用”，不要进行猜测。 |
| 未发现产品/服务之间的重叠 | 报告这些产品面向不同的市场。建议选择功能存在重叠的其他竞争对手，或改为类别汇总格式。 |