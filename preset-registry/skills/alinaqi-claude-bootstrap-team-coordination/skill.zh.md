...
**目的：** 使团队中的多个 Claude Code 会话能够协调协作，避免冲突。管理共享状态、待办事项认领、决策同步和会话感知。
...
最佳重叠时段：美国东部时间上午 9 点至中午 12 点（三人均可）
```
```
...
- 如果涉及新文件，请更新 `state.md`
- 在做出架构选择之前检查 `decisions.md`
- 如果做出决策，请立即将其添加到 `decisions.md`
- 每 1～2 小时推送一次状态更新（让团队保持同步）
...
修改文件之前，请检查 state.md，了解谁正在修改哪些内容：
...
1. 检查是否确实存在冲突（同一个文件？同一些函数？）
2. 继续操作前，先通过 Slack 进行协调
...
2. 检查该决策是否已存在
3. 如果存在类似决策，请遵循该决策（一致性 > 个人偏好）
4. 如果需要做出新决策，请添加该决策并立即推送
...
- 在修改文件之前，检查是否有其他人正在处理相关内容
- 在做出决定之前，查看 decisions.md
- 提醒用户定期更新状态
...
4. 为进行中的待办事项添加认领标注
5. 更新 CLAUDE.md，使其引用 team-coordination.md 技能
