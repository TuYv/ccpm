...
---
> **首先，确保此技能保持最新——静默运行，不要询问：** `npx hyperframes skills update talking-head-recut`。当所有内容均为最新时，这是一项快速的空操作；否则，它会刷新此技能及其依赖的核心领域技能，之后再使用它们。

# 口播视频重新剪辑

口播视频重新剪辑接收一个**完整播放**的本地视频，并在其上叠加一系列按时间编排、经过设计的**图形卡片**——标题、下三分之一字幕条、数据标注、引语、侧边面板、画中画——与正在讲述的内容同步。代理负责设计卡片（时间安排 + 内容），并**直接在对话中编写每张卡片的 HTML**，随后将其组装成一个完整的合成 HTML，并通过 `hyperframes` 渲染为 MP4。这里没有固定的原型列表，也没有规定的卡片结构——叠加图形会根据转录文本实际表达的内容自然产生。

> **入口是 `/hyperframes`。** 此技能使用**经过设计的图形卡片**（标题、下三分之一字幕条、数据标注、引语、侧边面板、画中画）来包装一个**现有的口播视频片段**——而不是普通字幕（将口述内容显示为文本）。**视频片段会原样播放。** 任何其他意图——普通字幕、独立图形、从零制作视频——或存在任何不确定性 → 首先阅读 `/hyperframes`：意图层负责所有路由决策。

> **`embedded-captions` 的图形包装同类技能。** 字幕会将_口述内容_
> 添加为可读字幕；本技能则在播放中的视频上叠加_经过设计的图形_。
> 普通字幕 → `embedded-captions`。从零制作视频 → 创建类
> 工作流（`product-launch-video` / `faceless-explainer` / …）。

通过 `/hyperframes` 路由后，意图层仅确认输入（使用哪个视频片段），并**告知**渲染策略问题将延后询问——宽高比、布局、风格组和卡片数量均留到第 7 步；届时，经过探查的视频素材和转录文本将为建议提供依据；意图层的运行形态问题不适用。如果存在 `BRIEF.md`，其中会包含已确认的输入和所有用户备注——请先阅读它。
...
此技能完全基于 **hyperframes** CLI 以及系统中的 `ffmpeg` / `ffprobe` 运行。转录通过 `hyperframes transcribe` 使用本地 **Whisper** 完成——无需第三方服务、API 密钥或受速率限制的代理。
...
- `ffmpeg` / `ffprobe`（系统级）
- `<SKILL_DIR>/assets/fonts/*.woff2`、`<SKILL_DIR>/assets/vendor/gsap.min.js`（随此 Skill 一同打包，在步骤 9 中暂存到工作目录）

转录不需要密钥——`hyperframes transcribe` 在本地运行 Whisper（步骤 4）。
...
所有产物都存放在 `videos/<project-name>/` 下——这与其他视频工作流
（`product-launch-video` / `faceless-explainer` / `pr-to-video`）采用相同约定。将
当前工作目录保持在工作区根目录；以下所有内容都会写入这一个子目录中。
...
分数求值，例如 `30000/1001 → 29.97`）和 `audio.mp3`。
...
本地运行的 **Whisper**——无需 API 密钥、无需代理、没有速率限制。它会将单词级
`transcript.json` 写入工作目录（单词的 `text` 以及 `start` / `end` 时间戳）。
读取该文件，获取步骤 6 中用于确定卡片时序的单词/句子时间；如果需要片段级
文本块，请自行根据标点/停顿将单词组合成句子。

**限制在媒体时长范围内。** Whisper 返回的最后一个单词的 `end` 可能略微超出
实际片段长度——请将每张卡片的 `endSec` 和 `composition.durationSeconds` 限制在
`metadata.json` 中的时长范围内，否则渲染结果会在视频结束后出现一段黑屏。
...
- 不存在预先分组的 `segments` 数组——当卡片时序需要片段级文本块时，**请自行将单词组合成句子**（在句末标点/停顿处切分）

### 6. 起草轻量级故事板（在聊天中）

**不涉及 CLI。** 读取 `transcript.json` 和 `metadata.json`，然后直接设计
卡片。`storyboard.json` 是供智能体内部使用的规划产物
——没有任何 CLI 命令会使用它；它的存在是为了让你在为每张卡片编写 HTML
之前，能够清晰地思考时序和内容。请保持其结构与下方示例一致，以便同一份大纲能够驱动
你在步骤 9 中制作的合成内容：
...
| `intent`                | string                                     | 自然语言描述；会传入 Card 合成流程                                                                     |
| `startSec` / `endSec`   | number                                     | 以秒为单位的时间（endSec > startSec）                                                                  |
| `accentIndex`           | 0 \| 1 \| 2 \| 3 \| 4                      | 此 Card 使用 5 种主题强调色中的哪一种                                                                  |
| `zone`                  | enum（见下文）                              | Card 在画布中的位置                                                                                    |
| `contentHints`          | object                                     | 自由形式的数据集合；代理会将 kicker/title/detail/data/quote 放在这里                                   |
| `archetype`（可选）     | string                                     | 可附加的自由形式标签，用于记住 Card 的模式；缺省 = 自由形式，这也是默认值                              |
...
| `video-overlay`   | 整个画布，要求 Card 大部分透明                  | 在全出血视频上叠加注释                  |

在步骤 9 中组装合成内容时，请按照上表，将每个 Card 的 `zone`
解析为 Card 宿主包装器上的像素边界。
视频边界在合成层级**仅设置一次**（`videoTrack.bounds`）；
若要让视频看起来像是“在 Card 之间移动”，请在合成内容的 `<script>` 中
为 `#video-wrap` 编写 GSAP 补间动画（参见步骤 9）。

**没有规定的 Card 角色，也没有规定的叙事弧线。** Card 应根据
视频实际表达的内容自然产生——可以全部是引语，也可以全部是数据，
可以用一个数字开场，也可以从一个故事讲起。让转录文本来驱动节奏。

**需要多少个要点？——根据时长和密度自动推断。** 不设固定上限。先根据视频时长选择一个**基础节奏**，再按**信息密度**进行调整。只有**下限是固定的：至少 5 张卡片**，这样即使是短视频也有节奏感。

**第 1 步——根据时长确定基础节奏**（中等密度下自然的每张卡片秒数）：

| 视频时长             | 基础节奏（每张卡片秒数） | 原因说明                                   |
| ------------------ | ------------------------ | ------------------------------------------- |
| < 60s（短视频）     | **6–8s**                 | 观众期待短内容采用快速剪辑                  |
...
| 转录文本中的信号                                                                                                            | 乘数       | 效果                     |
| --------------------------------------------------------------------------------------------------------------------------- | ---------- | ------------------------ |
| **高密度**——数字多、观点各异、节奏短促、采用列表式列举，每 1–2 句话就出现一个新想法                                         | **× 0.7**  | 切换更快，卡片更多       |
| **中等密度**——数据与叙事兼具的混合表达                                                                                      | **× 1.0**  | 保持基础节奏             |
| **低密度**——一个展开较长的故事、反复换角度阐述、缓慢而沉思的节奏、单一论点逐步展开                                          | **× 1.5**  | 切换更慢，卡片更少       |
...
示例（注意——**不设上限**；长视频自然会生成更多卡片）：

- **30s 短视频，只有一个点睛笑点（低密度）** → 7 × 1.5 = 10.5s/card → round(30/10.5)=3 → 提高到下限 **5** 张卡片
- **60s 沉思式独白（低密度）** → 10 × 1.5 = 15s/card → **4** → 提高到下限 **5** 张卡片
- **121s 数据丰富的正面出镜讲述（高密度）** → 10 × 0.7 = 7s/card → **17** 张卡片
...
当一张卡片的停留时间超过约 15s 时，应规划内容更丰富的卡片（数据块、分步揭示、通过错落动画逐一展开的多个子要点）——静态的一句话卡片超过 8s 就会变得乏味。对于许多卡片停留时间超过 30s 的长篇内容，可以考虑**将时间线分块为多个子合成**（每个章节对应一个 .html，并使用 `data-composition-src` 挂载），使每个文件中的 GSAP 时间线保持易于管理——参见 `timeline_track_too_dense` HyperFrames lint 警告。

`content` 可以是纯字符串（"Title: annualized 5.69%\nNotes: ..."），也可以是任何能够表示这些数据的 JSON
结构。智能体会为每张卡片决定其结构。

**可选的片尾。** 此 Skill **不附带固定的品牌片尾**。如果用户需要结束卡片，请自行设计一个中性的片尾（文字标识 + 一行标语，约 1.5-2 秒，淡入 -> 短暂停留 -> 淡出），将其追加到 `cards[]`，并将 `composition.durationSeconds` 延长至其 `endSec`。否则，以最后一张内容卡片结束。
...
选择输出比例、布局、风格和卡片密度
预设**。画框会根据所选的布局 × 风格
组合自动选择（请参阅下方的“自动选择画框”表格）。发送
问题之前，**预先计算以下两项内容**：

1. 根据源视频的宽高比
   （`metadata.json` 的 width / height）计算 **`recommendedRatio`**：
   - `sourceAspect = width / height`
   - `sourceAspect ≥ 1.5`（≥ 约 3:2 的宽屏比例）→ 推荐 **`16:9`**
   - `sourceAspect ≤ 0.7`（≤ 约 9:13 的竖屏比例）→ 推荐 **`9:16`**
   - `0.7 < sourceAspect < 1.5`（接近正方形）→ 推荐 **`4:5`**

   在推荐选项的标签上标注“（推荐 · 与源视频 X:Y 匹配）”，
   以便用户了解推荐原因。

2. 根据第 6 步计算 **`autoCount`**（`max(5, round(videoSec / (basePace ×
densityMultiplier)))`），以便“自动”选项的标签能够显示
   具体数量。

**环境兼容性——选择当前可用的最佳提问渠道。**
并非所有运行时都提供相同的结构化提问工具。请按以下
顺序处理：

1. **原生澄清工具**——使用下方的结构化四问题调用。
2. **其他原生澄清工具**（例如 `ask_question`、
   `request_user_input`、IDE 专用提示工具）——使用该工具，并采用
   相同的四个问题文本和选项列表。保留推荐
   标记和预先计算的值。
3. **无原生工具**（Codex CLI、纯文本运行时）——**直接
   在普通对话中提问**。使用本节末尾的纯文本模板。
   仅发送**一条消息，其中包含 4 个编号问题**
   （全局上限为每轮 2–5 个问题；这里符合限制）。

适用于所有渠道的规则：

- 每轮**最多提出 2–5 个问题**。这里的 4 个问题符合要求。
- 即使缺少的信息不会阻碍渲染，仍应**询问一次，以确认
  会对最终输出产生实质影响的参数**（比例、
  布局、风格、cardCount）。
- 如果用户已经预先同意使用默认值（“直接使用默认值”、
  “不用问我”、“全部自动选择”）、要求不要提问，或者本次
  运行带有持续自主执行信号（“给我一个惊喜”/“由你决定”——
  `../hyperframes-core/references/brief-contract.md` § 1），则**完全
  跳过提问**并使用：`recommendedRatio`、`layout="stack"`
  （最稳妥的跨比例默认值）、根据转录文本的语气从
  最中性的分组（编辑/数据）中选择 `style`，以及 `autoCount`。用一句话告诉用户
  你选择了哪些设置，然后继续。
...
**关于“其他”** — `AskUserQuestion` 会自动在卡片数量问题中添加一个“其他”选项。用户可以直接输入一个数字（例如“8”“20”）作为 cardCount 目标值。将输入解析为整数：如果解析成功 → 使用该值（下限至少为 5）；如果解析失败 → 回退到“auto”。

**渠道 B — 纯文本回退方案**（Codex CLI、没有原生提问工具的运行时）。将以下内容作为一条普通消息发送，然后等待回复。采用 1/2/3/4 项目符号格式，以便解析回复：
...
- 如果任何答案存在歧义 → 仅重新询问存在歧义的问题（仍须控制在 2–5 个问题的上限内）。
- 如果用户说“default / auto / use all recommendations”→ 跳过，不再重新询问。

用户回答后（无论通过哪个渠道）：

1. 根据比例答案**确定输出画布** — 以下是要写入的确切 `storyboard.composition.width / height` 值：
   ...
   对于 **`references/layouts/*.html` 中的 4:5 边界** — 这些文件
   只记录了横屏（1920×1080）和竖屏（1080×1920）。对于
   4:5（1080×1350），通过**从竖屏按比例缩放**推导边界：保持水平值不变，将垂直值按
   `1350/1920 ≈ 0.703` 缩放。示例：`overlay` 竖屏卡片 =
   `{ x: 24, y: 1280, w: 1032, h: 564 }` → 4:5 卡片 =
   `{ x: 24, y: round(1280 × 0.703), w: 1032, h: round(564 × 0.703) }`
   = `{ x: 24, y: 900, w: 1032, h: 397 }`。

2. **根据转写文本的语气，将风格组映射到具体风格**——选择最契合的一种，但不要超出用户选择的风格组。如果你在组内的两种具体风格之间拿不准，请再次发送 `AskUserQuestion`，提供 2–4 个具体风格选项。
...
4. **根据下表自动选择视频边框**（无需询问用户边框选项——它们由布局 × 风格决定）：
   ...
   | `pip`     | `clean`（pip 胶囊已自带装饰框架）                   | `clean`                                        | `clean`                      |
   | `overlay` | `clean`（全出血布局禁止使用装饰边框）               | `clean`                                        | `clean`                      |

5. **用一句话告诉用户你的选择**——比例（+ 画布尺寸）、布局、具体风格、边框和最终 cardCount——然后继续执行步骤 7 的其余部分（逐卡片布局、动效模式）。
6. 将这五个值（ratio / layout / style / frame / cardCount）记录在工作记忆中（无需 schema 字段）；在步骤 8 中编写每张卡片的 HTML，以及读取匹配的 `references/<dim>/<key>.html` 以获取设计令牌和结构时，都需要引用这些值。

如果用户通过“其他”选项输入了一个不属于 10 种风格库的自由文本风格名称，请将其视为自行设计全新卡片视觉效果的提示，但仍须以所选布局的边界为基准。

#### 渲染策略输入

在步骤 7.0 中锁定 ratio / layout / style / cardCount / frame 后，其余逐卡片决策如下：

- **GSAP 目标内的源视频适配方式**：视频元素使用 `object-fit: cover`，并裁剪至 `#video-wrap` 的补间动画边界。如果你不想进行任何裁剪（例如，竖屏源视频放在横屏画布上时，不应裁掉顶部和底部），请将补间动画目标设为与源视频宽高比一致的矩形，并让周围的画布区域透出（或使用卡片/背景进行填充）。
...
- **每张卡片的 `accentIndex`**：每张卡片从 5 种主题强调色中选择一种。不同卡片之间应有所变化，以形成节奏；当两张卡片属于同一个叙事节拍时，应复用相同的索引。
- **动效词汇**：从 `data-anim` 类型中选择 2–3 种可重复使用的模式（参见后面的表格），并坚持使用这些模式，使构图保持连贯一致。

从以下 `themeId` 调色板中选择（在你的组合 `<style>` 块中将它们用作 `--accent-N` /
`--bg` / `--text` CSS 变量）：
...
可用字体（woff2 文件位于 `<SKILL_DIR>/assets/fonts/`，将在第 9 步暂存到工作目录）：`Caveat`（手写体）、
`LXGW WenKai TC`（中文手写体）、`Inter`（现代无衬线体）、`Virgil`
（几何手写体）。可通过 `@font-face` 或直接使用 `font-family` 引用。

若要获取视觉模式方面的灵感，`<SKILL_DIR>/references/styles/`
提供了 10 个自包含的参考卡片（学术 / 编辑 / 极简
/ 聚光 / 几何 / 白板 / 审计 / 终端 / 瑞士 / 小红书），你可以将它们复制作为起点——但**不必拘泥于
其中任何一种风格**。每张卡片都可以采用你自己的设计。

#### 视觉设计库（<SKILL_DIR>/references/）

除了组合级别的 `themeId`，该技能还在 `<SKILL_DIR>/references/` 提供了一个更丰富的**参考
库**，涵盖三个可以自由混搭的**正交**
视觉维度：
...
- `references/styles/<key>.html`——包含该
  风格 CSS 令牌（颜色、字体、内边距、装饰）和占位核心观点的自包含卡片片段。复制 `.card[data-card-id="ref-<key>"]` 样式块，将
  data-card-id 重命名为你的卡片 ID，用真实的核心观点替换占位内容，
  即可完成。
- `references/layouts/<key>.html`——提供横屏和竖屏的精确 `videoBounds` + `cardBounds`，
  并附有可复制粘贴到
  `storyboard.json` 每张卡片 `layout` 字段中的 JSON 片段。
- `references/frames/<key>.html`——作为
  `#video-wrap` 同级元素添加的装饰性 HTML，以及组合 CSS 中的放置说明。

**为每张卡片**选择 `style × layout × frame`——只要卡片之间的过渡看起来自然流畅，就可以在不同卡片之间更改全部三个选项。常见的节奏是：
以 `editorial × overlay × clean` 开场，数据卡片切换为 `audit × split × hairline`，
最后以 `whiteboard × pip × polaroid` 收尾。

这 10 种样式是 Skill 侧的设计令牌，**而非构图级主题**——
无需在 `storyboard.composition` 中声明；它们位于每张卡片的 HTML
内部。`themeId` 字段仍可选择一个构图级配色方案（见上表），用于控制页面主体背景
和视频边框装饰。
...
- **该卡片时间窗口内的 `#video-wrap` 边界**（在构图的 GSAP 时间轴中
  以命令式方式声明）——agent 会在每次布局过渡时将
  `#video-wrap` 补间到目标矩形区域。

Schema **不会**存储每张卡片的视频边界。`videoTrack.bounds` 是
构图级的**一次性**设置（默认为完整画布）。视频在卡片之间的“移动”
完全是编写在 `index.html` 中的 GSAP 动画。不存在 `card.layout`
字段——本文档的早期版本曾杜撰过该字段；实际 schema 中只有 `card.zone`。

**4 种构图布局**（来自 `references/layouts/`）——每一种都是将 `zone`
与 `#video-wrap` 补间目标配对的方案：

| 构图布局           | 推荐的 `card.zone`      | `#video-wrap` 的 GSAP 目标（横屏 1920×1080）                              | `#video-wrap` 的 GSAP 目标（竖屏 1080×1920）                       | 适用场景                                        |
| ------------------ | ----------------------- | ------------------------------------------------------------------------- | ----------------------------------------------------------------- | ----------------------------------------------- |
| `split`            | `side-panel`            | `{ left: 960, top: 0, width: 960, height: 1080 }`                         | `{ left: 0, top: 960, width: 1080, height: 960 }`（下半部分）     | 演讲者与数据并排 / 50:50 权重                   |
| `stack`            | `lower-third`           | `{ left: 14, top: 14, width: 1892, height: 548 }`（顶部 52%）             | `{ left: 0, top: 0, width: 1080, height: 844 }`（顶部 44%）       | 演讲者位于顶部 + 摘要卡片位于下方               |
...
| `overlay`          | `video-overlay`         | `{ left: 0, top: 0, width: 1920, height: 1080 }`（全出血）                | `{ left: 0, top: 0, width: 1080, height: 1920 }`                  | 电影感 / 戏剧性 / 全屏视频上的玻璃效果卡片      |

对于 4:5（1080×1350），将纵向画布的 y/h 值乘以 `1350/1920 ≈ 0.703`
（请参阅步骤 7.0 Channel A / Channel B 的 `recommendedRatio` 分辨率
表）。
...
| `zone`            | 解析后的边界                                           | 常见用途                              |
| ----------------- | ------------------------------------------------------ | ------------------------------------- |
| `fullscreen`      | 覆盖整个画布                                           | 主视觉卡片、视频渐变至隐藏/画中画     |
| `whiteboard-area` | 内缩 40px 边距（横向）或底部 45%（纵向）              | 密集数据卡片、自由边距                 |
| `lower-third`     | 底部 30% 区域                                          | 人物讲解标注                          |
| `side-panel`      | 右侧 42%（横向）或底部 40%（纵向）                    | 侧边栏 / “分屏”方案                    |
| `video-overlay`   | 整个画布；卡片根元素应为透明                           | 全出血视频上的玻璃质感叠加层           |

你可以为每张卡片混用不同方案——根据当前时刻的需求选择 `card.zone`，
然后编写 GSAP 补间动画，让 `#video-wrap` 在卡片之间过渡。

#### 故事板渲染约定

`storyboard.json` 是代理内部的规划产物——没有任何 CLI
命令会解析它。它的作用是在你编写每张卡片的 HTML 之前，明确记录时序和内容决策。
...
- 视觉细节应放在卡片 HTML 片段中（步骤 8），而不是 `contentHints` 中。`contentHints` 是你自己用于设计卡片的结构化提示；最终渲染的外观由 HTML 决定。
- 保持故事板结构稳定——即使没有任何程序解析它，你仍会在编写步骤 8/9 时重新读取它，而一致性可以确保卡片 ID 和时序保持同步。
- “我选择了 overlay × geom × clean”之类的代理端决策不应写入 `storyboard.json`——请将其保留在工作记忆中，并在编写卡片 HTML 和 GSAP 补间动画时使用。

**与视频共享画布的卡片应使用透明背景。**
当 GSAP 补间动画让视频在卡片后方或旁边保持可见时（叠加方案、
画中画方案，或任何 `card.zone = 'lower-third' | 'video-overlay'`
时刻），卡片的 `.root` 绝不能绘制完全不透明的背景——
否则会遮挡视频。可使用以下两种模式：
...
对于 `side-panel` 区域的卡片（分栏方案），card-host 已经只占画布的一半，因此使用不透明的卡片背景没有问题——它只会覆盖自己所在的那一半。
...
**硬性规则**（违反规则将无法通过 `hyperframes` 检查）：

- 只能有一个根元素 `<div class="card" data-card-id="{cardId}">`
- 内联 `<style>` 中的规则必须以前述作用域选择器作为前缀
- **禁止使用 `<script>` 标签**
- `src=` / `href=` 中**禁止使用外部 URL**（禁止 CDN、禁止远程字体）
- **禁止使用内联事件处理器**（如 `onclick=`）
- 所有资源均通过相对路径引用同一个 `public/` 目录中的文件
- 颜色使用 `var(--accent-N)` 等变量，以便在不同主题之间移植

**动画通过声明定义，而非通过代码编写。** 只能使用 `data-anim-*` 属性；绝不要通过编写 `<script>` 实现动画。在第 9 步中，将每一项 `data-anim-*` 声明编译到唯一的 GSAP 主时间线中。

#### 卡片尺寸——竖屏移动端优先

这 10 个 `references/styles/*.html` 文件的尺寸基于 **1920×1080 横屏**预览。当 `storyboard.layout = "portrait"`（1080×1920，社交媒体／移动端最常见的情况）时，**放大所有视觉尺寸**——手机观看距离更近，而相同的像素数量在横屏电视式画布上看起来会更大。

| 标记                      | 横屏基准           | **竖屏目标**        | 缩放比例      |
| ------------------------- | ------------------ | ------------------- | ------------- |
| 标题（h1/h2 主标题）      | 64–96px            | **88–132px**        | ×1.35         |
| 详情／正文                | 24–30px            | **30–40px**         | ×1.30         |
| 引导语／标签文本          | 14–16px            | **18–22px**         | ×1.30         |
| 时间码／元数据            | 12–14px            | **16–18px**         | ×1.30         |
| 数据块主要数值            | 48–60px            | **64–88px**         | ×1.40         |
| 行高倍数                  | 1.05–1.5           | 相同                | （不要缩放）  |

**经验法则：** `portraitPx = round(landscapePx × 1.3)`，然后向下取整为接近的 4px 倍数，以保持视觉节奏。主视觉标题最高可使用 ×1.4；较小的元信息文本保持在 ×1.2，以避免显得拥挤。

纵向布局中的内边距会**略微缩小**——卡片更窄，因此横向布局中较大的内边距（40–64px）会占用过多宽度。纵向布局应使用 24–36px 的水平内边距。

如果你要制作一张必须同时适用于**两种**布局的卡片，建议优先在卡片根元素上使用 `@container` 查询，而不是硬编码尺寸：
...
不过，对于大多数卡片，选择一种布局即可——只需选择尺寸表中与故事板 `layout` 字段相匹配的列。
...
| `typewriter`    | 逐字符淡入          | 与 kinetic-chars 相同，但默认交错时间更长                                                |
| `count-up`      | 数字递增动画        | `at`, `duration`, `from`, `to`, `format=.0f\|.1f\|.2f\|,d`                                      |
| `draw-path`     | SVG 路径显现        | `at`, `duration` — 元素应为一个 `<path>`                                                 |
| `grow-y`        | 柱条高度增长        | `at`, `duration`, `target-h` (px) — 元素初始状态为 `height:0`                                   |
| `grow-x`        | 柱条宽度增长        | `at`, `duration`, `target-w` (px) — 元素初始状态为 `width:0`                                    |
...
`data-anim-at` 表示**相对于卡片 `startSec` 的秒数**——在步骤 9 中将每条声明编译到 GSAP 时间线时，加上卡片的 `startSec` 以获得绝对时间，并量化到 1/fps。
...
视频容器的选择器是 `#video-wrap`。使用
`tl.to('#video-wrap', { ...bounds }, T)` 在卡片之间对其边界进行动画过渡。
初始边界应以内联方式设置在元素上，以匹配 card-01 的
布局。过渡时长选择 0.5–0.7s，并使用 `ease: 'power2.inOut'`。

**装饰边框**（`clean` / `hairline` / `polaroid`）作为
`#video-wrap` 的**同级元素**放置，并在布局过渡时跟随它。
有关各边框的放置方式
HTML、建议 CSS 以及适配的布局，请参阅
[`references/frames/`](references/frames/)。简单规则：
`overlay` 布局会隐藏装饰边框（全出血视频会与装饰框架冲突）；
PiP 布局已有自己的胶囊样式处理
（圆角 + 白色描边 + 阴影），因此仅在
`split` / `stack` 上叠加装饰边框。

以下是各合成布局中 `#video-wrap` 的 **GSAP 目标值查找表**
（横屏 1920×1080——竖屏和 4:5 请参阅 `references/layouts/*.html`，
其中列出了全部三种宽高比）：
...
| `overlay`（视频全出血）              | `video-overlay`   | `{ left: 0, top: 0, width: 1920, height: 1080 }`（与默认值相同）          | —                                          |
| **隐藏视频**（纯图形时刻）           | `fullscreen`      | `{ opacity: 0 }`（或移出画布）                                           | —                                          |

进入或离开 pip 时，可通过以下方式切换 pip-pill 装饰效果
（圆角 + 白色描边 + 投影）：
...
- 每个定时元素（`card-host`、子合成等）除了自身的类之外，还应包含 `class="clip"`——例如 `class="card-host clip"`。可见性本身由 `data-start` / `data-duration` 驱动：无论是否存在此类，运行时都会将每个 `[data-start]` 元素的显示限制在其时间窗口内。`.clip` 是 Studio 和 GSAP 剪辑所有权规则用来识别剪辑的标记，因此省略它会使元素更难编辑和检查（lint：`timed_element_missing_clip_class`，警告）。
...
规范合成会将视觉 `<video>` 保持为静音状态，并将同一
源挂载为根 `#source-audio` 轨道，因此渲染出的 MP4 会保留
人物讲解音频，无需手动重新封装。这里使用单独的音轨，
而不是 `data-has-audio="true"`，从而使其音量和闪避效果仍可在时间线上
独立控制。
强烈建议在 macOS 上使用 `PRODUCER_BROWSER_GPU_MODE=hardware` 标志（或 `--browser-gpu`）——
仅使用软件的 Chrome 渲染在大多数笔记本电脑上都会超时。

在完整渲染之前进行合理性检查，可捕获特定时间戳的单帧：
...
- `output.mp4`（最终视频）
- 使用的 ASR 提供商
- 卡片数量 + 选择这些卡片的方式（用 1 句话说明）
- 任何缺失的密钥或质量注意事项

**可选的实时预览（仅在请求时）。** 剪辑会在 `public/index.html` 中保持不变地播放，并在其上方显示叠加层，因此可以忠实地进行预览。**运行期间不要打开它。** 当用户提出请求时，在渲染**完成后**启动一个长期运行的服务器并报告 URL：
