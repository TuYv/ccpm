...
根据分析对象，智能选择并应用最合适的改善分析技术。

## 描述
分析上下文并选择最佳方法：现场走访（代码探索）、价值流图（工作流/流程）或 Muda 分析（浪费识别）。引导你完成所选分析方法。
...
- METHOD：覆盖自动选择结果（gemba、vsm、muda）
...
## 注意事项
- 方法的选择取决于具体情境——选择最合适的方法
- 可以组合使用多种方法（Gemba Walk → Muda Analysis）
- 不熟悉相关领域时，从 Gemba Walk 开始
- 使用 VSM 进行流程优化
- 使用 Muda 提升效率并进行清理
- 所有方法都应促成可执行的改进措施
- 记录发现，以促进组织学习
- 考虑使用 `/analyse-problem` (A3) 对发现进行全面记录