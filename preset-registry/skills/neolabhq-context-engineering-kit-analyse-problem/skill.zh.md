...
采用 A3 问题解决格式，进行全面的单页问题记录和解决方案规划。

## 描述
结构化的单页分析格式，涵盖：背景、当前状况、目标、根本原因分析、对策、实施计划和后续跟进。因采用 A3 纸张尺寸而得名；强调文档简洁而完整。
...
- OUTPUT_FORMAT：markdown 或 text（默认：markdown）

## 步骤
1. **背景**：为什么这个问题很重要（情境、业务影响）
2. **当前状况**：目前正在发生什么（数据、指标、示例）
3. **目标/指标**：成功是什么样的（具体、可衡量）
4. **根本原因分析**：问题为什么存在（使用 5 Whys 或 Fishbone）
5. **对策**：针对根本原因提出的解决方案
6. **实施计划**：由谁、做什么、何时做、如何做
7. **后续跟进**：如何验证成功并防止问题再次发生
...
## 注意事项
- A3 强制进行简洁而完整的思考（内容可控制在一页内）
- 使用数据和事实，而不是观点或指责
- 根本原因分析至关重要——使用 `/why` 或 `/cause-and-effect`
- 对策必须针对根本原因，而不是表面症状
- 实施计划需要明确负责人和时间表
- 后续跟进可确保改进持续有效
- A3 会成为组织学习的历史记录
- 随着情况变化更新 A3（在关闭前始终作为动态文档）
- 以下情况可考虑使用 A3：事故、反复出现的问题、重大改进
- 以下情况使用 A3 属于过度处理：小缺陷、单行修复、琐碎问题