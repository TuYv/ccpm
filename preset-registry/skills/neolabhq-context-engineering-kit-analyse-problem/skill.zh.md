...
应用 A3 问题解决格式，进行全面的单页问题记录与解决规划。

## 描述
涵盖以下内容的结构化单页分析格式：背景、当前状况、目标、根因分析、对策、实施计划和跟进。名称源于 A3 纸张尺寸；强调简洁、完整的文档记录。
...
## 步骤
1. **背景**：为什么这个问题很重要（背景、业务影响）
2. **当前状况**：现在正在发生什么（数据、指标、示例）
3. **目标/指标**：成功的标准是什么（具体、可衡量）
4. **根因分析**：问题为什么存在（使用 5 Whys 或 Fishbone）
5. **对策**：针对根因提出的解决方案
6. **实施计划**：谁、做什么、何时、如何实施
7. **跟进**：如何验证成功并防止问题再次发生
...
- 后续跟进可确保改进具有可持续性
- A3 会成为组织学习的历史记录
- 随着情况的发展更新 A3（在关闭前将其作为持续更新的文档）
- 以下情况可考虑使用 A3：事故、反复出现的问题、重大改进
- 以下情况使用 A3 可能过度：小型 bug、一行修复、琐碎问题