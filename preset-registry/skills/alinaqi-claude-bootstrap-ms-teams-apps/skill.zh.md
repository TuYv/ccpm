...
**用途：** 为 Microsoft Teams 构建由 AI 驱动的智能体和应用。创建可与 OpenAI 和 Claude 等 LLM 集成的对话机器人、消息扩展和智能助手。
...
| 技巧 | 说明 |
|-----|-------------|
| 缓存 Graph 令牌 | 刷新令牌的开销很大 |
| 流式传输长响应 | 使用输入指示器和分块响应 |
| 为知识库建立索引 | 为 RAG 预先嵌入文档 |
| 使用连接池 | 复用 HTTP 连接 |
| 压缩有效载荷 | 使用 Gzip 压缩大型卡片响应 |
...
| 问题 | 原因 | 解决方法 |
|-------|-------|-----|
| 机器人无响应 | 端点不可访问 | 检查清单中的 ngrok/Azure URL |
| 身份验证失败 | 令牌已过期/无效 | 刷新 OAuth 连接 |
| 卡片未呈现 | 架构无效 | 在 adaptivecards.io/designer 上验证 |
...
- [Teams SDK 文档](https://microsoft.github.io/teams-sdk/)
- [Teams 平台文档](https://learn.microsoft.com/en-us/microsoftteams/platform/)
- [Adaptive Cards 设计器](https://adaptivecards.io/designer/)
- [Microsoft Graph 浏览器](https://developer.microsoft.com/en-us/graph/graph-explorer)
