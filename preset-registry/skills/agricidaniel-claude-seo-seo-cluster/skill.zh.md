...
由 SERP 重叠驱动的关键词聚类，用于内容架构设计。根据 Google 的实际排名结果（共享的前 10 名结果）对关键词进行分组，而不是依据文本相似度。设计中心辐射式内容集群，并通过内部链接矩阵生成交互式集群地图可视化。
...
1. **相关搜索**：搜索种子关键词，提取“相关搜索”和“用户也会搜索”
...
**去重：**规范化变体（转为小写、去除冠词），删除完全重复项。目标：得到 30-50 个唯一关键词变体。如果少于 30 个，则以排名靠前的 PAA 问题作为种子再次进行扩展。

### 步骤 2：SERP 重叠聚类

这是核心差异化能力。加载 `references/serp-overlap-methodology.md` 以获取完整算法。

**流程：**
1. 按初始意图猜测对关键词分组（减少成对比较次数）
2. 对每组中的每个候选关键词对分别使用 WebSearch 搜索
3. 统计前 10 个自然搜索结果中的共享 URL 数量（忽略广告、精选摘要、PAA）
4. 应用以下阈值：
...
**优化：**对于 40 个关键词，完整的两两比较需要进行 780 次比较。可以改为：
- 按意图预先分组（4 组，每组约 10 个关键词 = 4 x 45 = 180 次比较）
- 仅对跨组边界的关键词进行交叉检查
- 如果两个关键词都是同一核心词的长尾变体，则跳过比较（假设它们属于同一集群）

**DataForSEO 集成：**如果 DataForSEO MCP 可用，则使用 `serp_organic_live_advanced` 代替 WebSearch 获取 SERP 数据。在每个批次之前运行 `"${CLAUDE_PLUGIN_ROOT}/scripts/claude-seo" run dataforseo_costs.py check serp_organic_live_advanced --count N`。如果 `"status": "needs_approval"`，则显示费用估算并询问用户。如果 `"status": "blocked"`，则回退到 WebSearch。
...
从聚类中移除导航型关键词。标记边界案例，以便人工审核。关键词可能具有混合意图（例如，“best CRM software”同时具有商业型和信息型意图），应根据主导意图进行分类。
...
1. **选择中心关键词**：搜索量最高、意图最广泛、与其他关键词的 SERP 重叠程度最高
2. **将分支页分组到集群中**：每个集群是一个子主题领域（每个中心页包含 2-5 个集群）
3. **将文章分配到集群中**：每个集群包含 2-4 篇分支文章
4. **为每篇文章选择模板**：根据意图分类：
...
6. **关键词蚕食检查**：任何两篇文章都不能共享相同的主关键词。如果 SERP 重叠度达到 7+，则将这些关键词合并到一篇文章中，同时针对这两个关键词。
...
| 链接类型 | 方向 | 要求 |
|-----------|-----------|-------------|
| 分支页指向中心页 | spoke -> pillar | 必须（每个分支页都需要） |
| 中心页指向分支页 | pillar -> spoke | 必须（每个分支页都需要） |
| 集群内分支页互链 | spoke <-> spoke | 每篇文章 2-3 个链接 |
| 跨集群链接 | spoke -> spoke（其他集群） | 每篇文章 0-1 个链接 |

**规则：**
- 每篇文章必须至少有 3 个指向它的内部链接
- 不得存在孤立页面（每篇文章都必须能从中心页通过 2 次点击到达）
...
5. 告知用户：“在浏览器中打开 `cluster-map.html`，探索交互式集群地图。”
...
1. 在当前目录中查找最近的 `/seo plan` 输出（搜索匹配 `*SEO*Plan*`、`*strategy*`、`*content-strategy*` 的文件）
2. 解析 Markdown 表格中的：关键词、页面类型、内容支柱、URL 结构
3. 验证提取的数据：检查重复项、缺失关键词和不完整条目
4. 使用 SERP 数据进行丰富：对提取的关键词运行 SERP 重叠分析
5. 使用导入的关键词作为起始集合构建集群计划（跳过步骤 1）

如果未找到策略文件，向用户提示："当前目录中未找到现有的 SEO 计划。请先运行 `/seo plan`，或提供一个种子关键词以进行全新聚类。"
...
4. 按优先级顺序执行：先处理支柱文章，然后按搜索量处理分支文章（从最高开始）
5. 对每篇文章，调用 `blog-write` 技能并提供集群上下文：
   - 集群角色（支柱或分支）
   - 集群中的位置（集群索引、文章索引）
   - 目标关键词和次要关键词
   - 模板类型和目标字数
   - 要包含的内部链接（带锚文本）
   - 将由后续文章指向的链接（占位标记）
6. 每写完一篇文章，扫描之前的文章以查找反向链接占位符，并注入新文章的 URL
7. 所有文章写入完成后，生成集群评分卡
...
| 链接密度 | 每篇 3 个以上 | 统计每篇文章中的内部链接数 |
| 孤立页面 | 0 | 入站链接少于 1 个的文章数 |
| 关键词蚕食 | 0 个冲突 | 检查重复的主要关键词 |
| 图片数量 | 每篇至少 1 张 | 统计至少包含一张图片的文章数 |
| 支柱链接 | 100% | 所有分支文章都链接到支柱文章，反之亦然 |
| 交叉链接 | 80% 以上 | 已实现的建议分支到分支链接 |
| 内容缺口 | 0 | 被跳过或不完整的计划文章数 |
...
| "Insufficient keyword variants" | 扩展结果少于 15 个关键词 | 使用 PAA 问题运行第二轮扩展 |
...
| "cluster-plan.json not found" | 未进行规划便尝试执行 | 提示用户先运行 `/seo cluster plan` |
| "claude-blog not installed" | 未安装博客 Skill 便尝试执行 | 改为生成内容简报；建议安装该 Skill |
