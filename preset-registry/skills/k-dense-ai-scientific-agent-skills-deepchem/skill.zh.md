...
DeepChem 是一个综合性的 Python 库，用于将机器学习应用于化学、材料科学和生物学。通过专门的神经网络、分子特征化方法和预训练模型，实现分子性质预测、药物发现、材料设计以及生物分子分析。

**版本说明：** 示例面向 **deepchem 2.8.0**（PyPI 稳定版，2024 年 4 月）。需要 **Python 3.7–3.11**（PyPI 上为 `<3.12`）。核心工具（加载器、特征化器、MoleculeNet）无需深度学习后端即可运行；GNN 和 transformer 模型需要匹配的额外依赖（`torch`、`tensorflow` 或 `jax`）。使用 GPU 构建版本时，请先安装后端框架。
...
2. **分子特征化** — 环状指纹、图卷积和描述符。
3. **数据划分** — 随机划分、骨架划分、分层划分和 butina 划分，以及为什么
   对于分子而言，骨架划分是诚实的默认选择。
4. **模型选择与训练** — 模型系列以及如何拟合它们。
5. **MoleculeNet 基准测试** — 加载标准数据集及其已发布的划分。
6. **迁移学习** — 预训练与微调。
...
训练并评估溶解度预测模型。适用于 Delaney 基准数据或自定义 CSV 数据。
...
### 模式 3：先从简单方案开始，然后逐步扩展
1. 从 Random Forest + CircularFingerprint 开始（快速基线）
2. 如果 RF 效果良好，尝试 XGBoost/LightGBM
3. 如果你有超过 5K 个样本，转向深度学习（MultitaskRegressor）
4. 如果你有超过 10K 个样本，尝试 GNN
5. 对于小型数据集或新型骨架，使用迁移学习
...
**解决方案**：DeepChem 会延迟加载——安装与模型匹配的后端，然后添加对应的 extra：
```bash
...
```
使用 GPU 时，请先安装具有正确 CUDA 构建版本的 PyTorch 或 TensorFlow，**然后**再安装对应的 extra。在 zsh 中请为 extra 加引号：`'deepchem[torch]'`。

**Conda + PyTorch 用户：**如果 `import deepchem` 因 `undefined symbol: iJIT_NotifyEvent` 而失败，请将 MKL 固定在 2025 以下（`conda install "mkl<2025"`）——PyTorch wheels 可能与 MKL 2025.0.0 不兼容。
...
- 所有数据加载器及其使用场景
- 数据集类以及各自的适用场景
- 完整的 featurizer 目录及选择指南
- 按类别组织的模型目录（50+ 个模型）
- MoleculeNet 数据集说明
- 指标和评估函数
- 常见代码模式

**需要参考时：**当你需要具体的 API 细节、参数名称，或希望了解可用选项时，搜索此文件。
...
**需要参考时：**使用这些工作流作为实现完整解决方案的模板。

## 安装

核心软件包（数据加载器、featurizer、MoleculeNet、scikit-learn 封装器）：
...
Nightly 构建：`uv pip install --pre deepchem`（使用 `--pre` 时同样适用于这些 extras）。

请参阅[安装指南](https://deepchem.readthedocs.io/en/latest/get_started/installation.html)和[软性依赖要求](https://deepchem.readthedocs.io/en/latest/requirements.html)，了解每个模型类的可选依赖。
