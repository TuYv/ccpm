...
- **Python 原生**：使用 Python 代码定义基础设施，无需 YAML
- **自动扩缩**：可缩容至零，也可立即扩容至 100 多个 GPU
- **亚秒级冷启动**：基于 Rust 的基础设施可快速启动容器
- **容器缓存**：缓存镜像层，以便快速迭代
- **Web 端点**：将函数部署为 REST API，并支持零停机更新

**以下情况改用其他方案：**
- **RunPod**：适用于具有持久化状态且运行时间较长的 Pod
...
| `modal serve script.py` | 支持实时重新加载的开发模式 |
...
| `A10G` | 24GB | 训练和推理，速度比 T4 快 3.3 倍 |
| `L40S` | 48GB | 推荐用于推理（性价比最佳） |
| `A100-40GB` | 40GB | 大型模型训练 |
| `A100-80GB` | 80GB | 超大型模型 |
| `H100` | 80GB | 速度最快，支持 FP8 + Transformer Engine |
| `H200` | 141GB | 从 H100 自动升级，带宽为 4.8TB/s |
...
| GPU 内存不足 | 使用显存更大的 GPU（`A100-80GB`），启用梯度检查点 |
| 镜像构建失败 | 固定依赖项版本，检查 CUDA 兼容性 |
