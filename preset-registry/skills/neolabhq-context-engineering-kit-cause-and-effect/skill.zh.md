...
应用鱼骨图（石川图）分析，跨多个类别系统地探索问题的所有潜在原因。

## 描述
系统地检查六个类别中的潜在原因：人员、流程、技术、环境、方法和材料。创建结构化的“鱼骨图”视图，以识别促成问题的因素。
...
- 鱼骨图可以揭示各个领域中的系统性问题
- 多种原因往往会共同导致问题
- 不要停留在每个类别中找到的第一个原因上——要继续深入挖掘
- 有些原因横跨多个类别（请标记出来）
- 根本原因通常存在于流程或方法中（而不只是在技术层面）
- 使用 `/why` 命令对特定原因进行更深入的分析
- 按照以下公式确定解决方案的优先级：影响力 × 可行性 ÷ 工作量
- 解决根本原因，而不只是处理表面症状