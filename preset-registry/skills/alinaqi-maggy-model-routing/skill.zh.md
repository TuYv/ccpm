---
name: model-routing
description: 9-tier model routing system with cascading classifier fallback and result auto-evaluation
when-to-use: When configuring or debugging how prompts are classified and routed to the cheapest capable model
user-invocable: false
effort: medium
---
# 模型路由系统

## 路由决策如何做出

每个用户提示都会先经过一个 9 层分类流水线，然后才会有任何 AI 模型处理。系统回答三个问题：

1. **应该由哪个模型处理？** —— 9 层成本/复杂度分类
2. **分类器本身是否正常工作？** —— 级联回退（qwen3 → kimi → deepseek → cache）
3. **我们能验证结果吗？** —— 工具级回退 + 自动评估

### 流水线

```
User types prompt
    ↓
UserPromptSubmit hook fires (~/.claude/hooks/route-task-hook)
    ↓
Classifier: qwen3 (local, free) classifies into tier
    ↓  (fails?)
Classifier: kimi (local, free) retries
    ↓  (fails?)
Classifier: deepseek-flash (~$0.0001) retries
    ↓  (fails?)
Classifier: cached tier from last success
    ↓
Hook injects routing decision into Claude's context
    ↓
Claude delegates to the right model or handles directly
```

## 9 层路由表

| 层级 | 模型 | 输入（每百万） | 输出（每百万） | 处理内容 |
|------|------|----------------|----------------|---------|
| 0 | **Qwen3** (local) | $0 | $0 | grep、find、shell、语法、日志读取 |
| 1 | **Gemini 2.5 Flash-Lite** | $0.10 | $0.40 | 批量提取、分类、CIG 流水线 |
| 2 | **DeepSeek V4 Flash** | $0.14 | $0.28 | 简单代码、CRUD、测试编写、小修复 |
| 3 | **DeepSeek V4 Pro** | $0.44 | $0.87 | 多文件功能、重构、调试（约 80% 的工作） |
| 4 | **Gemini 2.5 Flash** | $0.15 | $0.60 | 多模态（图像、视频、音频）、品牌分析 |
| 5 | **Kimi K2.6** | $0.60 | $2.50 | 代码审查、提交信息、diff 摘要 |
| 6 | **Gemini 3.1 Pro + Search** | $1.25 | $10.00 | 深度研究、Google grounding、200 万上下文 |
| 7 | **Codex** | varies | varies | 批量生成、代码审查 |
| 8 | **Claude Sonnet/Opus** | $3-5 | $15-25 | 架构、安全、质量关键型工作 |

## 委派命令

当 hook 说“delegate to X”时，运行对应命令并返回其输出：

```bash
# Tier 0 — Qwen3
~/bin/qwen3 "prompt"

# Tier 1 — Gemini Flash-Lite
~/bin/gemini --flash-lite "prompt"

# Tier 2 — DeepSeek Flash
~/bin/deepseek --flash "prompt"

# Tier 3 — DeepSeek Pro
~/bin/deepseek --pro "prompt"

# Tier 4 — Gemini Flash
~/bin/gemini --flash "prompt"

# Tier 5 — Kimi
~/bin/kimi --quiet -p "prompt"

# Tier 6 — Gemini Pro Search
~/bin/gemini --pro-search "prompt"

# Tier 7 — Codex
codex exec "prompt"

# Tier 8 — Claude
# 直接处理（不委派）
```

## 委派脚本约定

每个 `~/bin/` 脚本都遵循相同模式：

1. **以参数形式接收 prompt**：`script "what is 2+2"`
2. **模型标志**：`--flash`、`--pro`、`--flash-lite`、`--pro-search`
3. **静默模式**：`--quiet`（如适用）
4. **输出**：将响应写到 stdout，错误写到 stderr
5. **退出码**：成功时为 0，失败时为非零

### 可用脚本

```
~/bin/
├── qwen3       # Shell: curl 到本地 Ollama API
├── kimi        # Shell: exec Kimi CLI binary
├── deepseek    # Python: httpx 到 DeepSeek Anthropic-compat API
├── gemini      # Python: httpx 到 Gemini OpenAI-compat API
├── research    # Python: multi-backend research with auto-evaluation
└── route-task  # Shell: qwen3-powered task classification
```

## 分类器回退链

分类器本身可能失败。发生这种情况时，会触发级联回退：

| 级别 | 分类器 | 成本 | 阈值 |
|-------|-----------|------|-----------|
| 1 | **qwen3** (Ollama) | $0 | 2s connect, 8s classify |
| 2 | **kimi** CLI | $0 | Local process |
| 3 | **deepseek-flash** | ~$0.0001 | API call |
| 4 | **Cached tier** | $0 | From `~/.claude/routing-cache.json` |

缓存（`~/.claude/routing-cache.json`）会保存上一次成功的层级和时间戳。在压缩后，当 Ollama 可能会短暂不可达时，缓存可确保路由继续进行，而不会默认降级到 CLAUDE。

## 工具回退协议

当 Claude 的内置工具失败时，外部后端会接管：

| 失败的工具 | 回退 1 | 回退 2 |
|-------------|------------|------------|
| **WebSearch** / **WebFetch** | `~/bin/research "query"` | `~/bin/deepseek --pro "query"` |
| **Read** / 文件访问 | 通过 Bash 使用 `cat` | — |
| **Grep** | 通过 Bash 使用 `grep -r` | — |

### 研究工具（`~/bin/research`）

带自动评估的多后端研究：
- 依次尝试 **deepseek-flash → deepseek-pro**
- 按内容质量、结构、长度对结果打 0-10 分
- 根据评估分数自动调整首选后端
- 查看统计信息：`~/bin/research --eval`
- 分数日志：`~/.claude/research-eval.jsonl`

## Maggy 集成

Maggy 的 `model_router.py` 在 `DEFAULT_TIERS` 中镜像了相同的 9 层结构。`PiAdapter` 使用相同的委派脚本执行。`routing_rules_defaults.py` 中的任务类型覆盖确保：

- `research`, `competitor` → **Gemini Pro Search**（Google grounding）
- `bulk` → **Gemini Flash-Lite**（最低成本）
- `security`, `architecture`, `planning` → **Claude**（质量关键）
- `docs`, `tests` → **DeepSeek Pro**（成本高效）
- `review` → **Claude**（安全 + 架构深度）

## 环境

```bash
# Required for delegation scripts (in ~/.zshrc)
export DEEPSEEK_API_KEY="sk-..."
export GEMINI_API_KEY="..."       # For gemini delegator
export OPENAI_API_KEY="sk-..."    # For codex CLI

# Ollama must be running locally for qwen3
ollama serve  # or launch at startup
```

## 可观测性

- **路由日志**：`~/.claude/routing-log.jsonl` — 每次分类的层级、使用的分类器、节省的 token
- **路由缓存**：`~/.claude/routing-cache.json` — 供压缩后恢复使用的上一次层级
- **研究评估**：`~/.claude/research-eval.jsonl` — 每个查询的后端评分
- **Maggy 路由热力图**：Dashboard → Models tab → 每个模型的奖励分数