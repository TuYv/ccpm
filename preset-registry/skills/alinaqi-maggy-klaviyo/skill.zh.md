...
用于集成 Klaviyo 电子邮件/SMS 营销——客户档案、事件跟踪、营销活动、流程和细分。
...
| 类型 | URL |
|------|-----|
| 服务端（Private） | `https://a.klaviyo.com/api` |
| 客户端（Public） | `https://a.klaviyo.com/client` |
...
| 主题 | 触发条件 |
|-------|---------|
| `profile.created` | 创建新用户档案 |
| `profile.updated` | 用户档案属性发生更改 |
| `profile.merged` | 合并用户档案 |
| `event.created` | 跟踪新事件 |
| `list.member.added` | 将用户档案添加到列表 |
| `list.member.removed` | 从列表中移除用户档案 |
...
| 时间窗口 | 限制 |
|--------|-------|
| 突发 | 75 requests/second |
| 稳定 | 700 requests/minute |
...
- **缺少邮箱/电话** - 每个用户资料至少需要一个标识符
- **重复事件** - 对于订单/交易使用 unique_id
- **缺少 Items 数组** - 产品推荐所必需
- **仅客户端** - 服务端跟踪更可靠
- **忽略速率限制** - 实现指数退避
- **硬编码 API 密钥** - 使用环境变量
