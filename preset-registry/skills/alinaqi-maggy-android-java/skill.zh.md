...
- ❌ **在后台线程更新 UI** - UI 更改始终应发布到主线程
- ❌ **硬编码字符串** - 所有面向用户显示的文本都应使用字符串资源
- ❌ **上帝 Activity** - 将 Activity 控制在 200 行以内，并将逻辑提取到 ViewModels 中
- ❌ **NetworkOnMainThreadException** - 切勿在主线程上执行网络调用
- ❌ **忽略生命周期** - 始终遵守 Activity/Fragment 的生命周期状态
- ❌ **阻塞主线程** - 将主线程操作控制在 16ms 以内
- ❌ **不处理配置更改** - 使用 ViewModel 来应对屏幕旋转
- ❌ **硬编码尺寸** - 使用 dp/sp 单位和尺寸资源
- ❌ **视图层级过深** - 将布局深度控制在 10 层以内，并使用 ConstraintLayout
- ❌ **不关闭资源** - 始终关闭 Cursor、InputStream 和数据库连接
