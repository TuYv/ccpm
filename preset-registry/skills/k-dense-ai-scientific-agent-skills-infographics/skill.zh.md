...
信息图表是信息、数据或知识的可视化表示形式，旨在快速、清晰地呈现复杂内容。**此技能使用 Nano Banana Pro AI 生成信息图表，并使用 Gemini 3.6 Flash 进行质量审核，使用 Perplexity Sonar 进行研究。**
...
**只需描述您想要的内容，Nano Banana Pro 就能为您创建。**
...
2. **第 1 次生成**：Nano Banana Pro 遵循最佳设计实践创建初始信息图表
3. **第 1 次审核**：**Gemini 3.6 Flash** 根据文档类型的质量阈值评估质量
4. **决策**：如果质量 >= 阈值 → **完成**（无需进一步迭代！）
5. **如果低于阈值**：根据评审意见改进提示词并重新生成
6. **重复执行**：直到质量达到阈值，或达到最大迭代次数

**智能迭代的优势：**
- ✅ 如果首次生成的质量已经足够，可节省 API 调用
- ✅ 为营销材料提供更高的质量标准
- ✅ 加快草稿和内部使用内容的处理速度
- ✅ 为每种使用场景提供适当的质量水平

**输出**：带版本的图像以及详细的审查日志，其中包含质量评分、批评意见和提前停止信息。
...
创建需要准确、最新数据的信息图时，使用 `--research` 标志通过 **Perplexity Sonar Pro** 自动收集事实和统计数据。
...
1. **收集关键事实**：围绕主题收集 5-8 条相关事实和统计数据
2. **提供背景信息**：提供背景信息，以确保准确呈现
3. **查找数据点**：查找具体数字、百分比和日期
4. **引用来源**：提及主要研究或来源
...
| `technology` | 科技蓝、石板色、紫罗兰色 | 软件、数据、AI |
| `nature` | 森林绿、薄荷绿、大地棕 | 环境、有机 |
...
| 调色板 | 颜色 | 描述 |
|---------|--------|-------------|
| `wong` | 橙色、天蓝色、绿色、蓝色、朱红色 | 目前最广泛推荐 |
| `ibm` | 群青色、靛蓝色、洋红色、橙色、金色 | IBM 的无障碍调色板 |
| `tol` | 12 色扩展调色板 | 适用于多个类别 |
...
- **`references/infographic_types.md`**：所有 10 种以上类型的扩展模板
...
- **scientific-schematics**：用于技术示意图和流程图
...
使用此技能，借助 Nano Banana Pro AI 以及智能质量评审功能，创建专业、易于访问且视觉效果出色的信息图表。
...
始终引用当前版本。DOI 和 https://arxiv.org/abs/2609.00065 会解析到最新的 arXiv 版本，因此切勿添加类似 `v1` 的版本后缀。当网络访问可用时，在撰写参考文献前获取 https://arxiv.org/abs/2609.00065（或
