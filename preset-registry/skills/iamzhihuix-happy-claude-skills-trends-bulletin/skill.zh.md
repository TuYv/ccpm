...
| 变量 | 必须 | 说明 |
|------|------|------|
| `TELEGRAM_BOT_TOKEN` | 是 | Telegram 机器人令牌 |
| `TELEGRAM_CHAT_ID` | 是 | Telegram 目标聊天 ID |
| `GROK_API_KEY` | 否 | xAI Grok API 密钥（为空则跳过 AI 趋势分析） |
| `PRODUCT_HUNT_TOKEN` | 否 | Product Hunt API 令牌（为空则跳过 Product Hunt） |
| `YOUTUBE_API_KEY` | 否 | YouTube Data API 密钥（为空则跳过 YouTube） |

## 执行

**智能体执行说明**：
1. 确定此 SKILL.md 文件的目录路径，将其作为 `{baseDir}`
...
- `REDDIT_SUBS` - Reddit 子版块列表，逗号分隔（默认：`MachineLearning,LocalLLaMA,artificial,ChatGPT,singularity`）
