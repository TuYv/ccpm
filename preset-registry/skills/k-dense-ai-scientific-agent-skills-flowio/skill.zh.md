...
使用 FlowIO 作为 Flow Cytometry Standard 文件的轻量级底层读取器和写入器。本技能中的示例针对 **FlowIO 1.4.0**，这是截至 2026-07-23 经验证的当前稳定版本。

FlowIO 适用于：

- 读取 FCS 2.0、3.0 和 3.1 文件
- 检查 HEADER、TEXT、ANALYSIS 以及通道元数据
- 将事件数据作为二维 NumPy 数组获取
- 读取包含多个数据集的旧版文件
- 写入 list-mode、单精度 FCS 3.1 文件
- 为 pandas、机器学习或下游细胞分析工具准备数据

FlowIO **不会**执行补偿、logicle/双指数变换、门控、聚类或 FlowJo 工作区处理。对于这些任务，请使用 FlowKit 或其他分析包。
...
2. **在加载事件前先进行检查。** 对于仅处理元数据的任务，尤其是处理大型或不熟悉的文件时，使用 `only_text=True`。
3. **明确选择事件语义。** 使用 `as_array(preprocess=True)` 根据 FCS 元数据应用增益、对数和时间缩放，或使用 `preprocess=False` 获取 DATA 段中编码的值。记录所作选择。
4. **默认保持严格解析。** 不要自动抑制偏移量错误。只有在已知供应商格式缺陷时才放宽检查，并检查由此产生的事件数据。
5. **将元数据视为潜在敏感信息。** FCS TEXT 值可能包含样本、受试者、操作员和仪器标识符。仅导出任务所需的字段。
6. **通过重新打开文件验证写入结果。** 任何 FCS 导出后，都应检查事件/通道数量、标签、元数据和具有代表性的值。
...
不要查找 `"$DATE"`、`"$CYT"` 或其他带大写字母和美元符号前缀的键。TEXT 值仍为字符串。FlowIO 1.4.0 还会从解码后的 TEXT 段中移除每一个 `$` 字符，包括值内部的 `$` 字符；当精确的元数据保真度很重要时，请保留原始文件。

### 事件有两种表示形式

- `flow.events` 是未经处理的一维扁平事件数组。
- `flow.as_array()` 返回形状为 `(event_count, channel_count)` 的 NumPy `float64` 数组。
- `flow.as_array(preprocess=True)` 会应用 FCS 增益、对数和时间缩放。它不会应用补偿或 logicle/双指数显示变换。
- `flow.as_array(preprocess=False)` 会在不执行上述缩放步骤的情况下重塑编码后的事件值。

`as_array()` 会创建另一个内存中的数组。FlowIO 不提供分块式或内存映射的事件访问。
...
- 一个已经打开的二进制文件句柄
- 按行优先的事件/通道顺序排列的一维扁平事件数据
- 每个通道一个 PnN 名称
- 可选的 PnS 名称，以及通过 `metadata_dict` 提供的字符串值元数据

它写入 FCS 3.1 list-mode（`$MODE=L`）单精度浮点（`$DATATYPE=F`）数据。必需的解释关键字由 FlowIO 生成，无法通过元数据覆盖。
...
不要对仅包含元数据的实例调用 `as_array()`，因为其事件数据并未加载。

相比调用方所拥有的文件句柄，优先使用路径或 `Path`。`FlowData` 在解析后会关闭所提供的句柄。在 FlowIO 1.4.0 中，`read_multiple_data_sets(handle)` 可能会因句柄已关闭而在第一个数据集之后失败；对于包含多个数据集的文件，请传入文件系统路径。
...
FCS 3.1 规范已弃用在一个文件中包含多个数据集的做法，但 FlowIO 可以读取使用该功能的旧式文件。
...
元数据键可以使用混合大小写或带 `$` 的形式提供，但不带 `$` 的小写键与 FlowIO 的规范化表示一致，出错风险更低。  
元数据值必须是字符串。

## 复制或重写现有文件

当事件数据无需更改时，请使用 `write_fcs()`：
...
传入 `metadata=None` 会保留 FlowIO 选定的默认值。传入任何字典（包括 `{}`）都会替换这些默认值，而不是与其合并。`write_fcs()` 始终生成 FCS 3.1 浮点输出；非浮点源事件会在写入前进行预处理。它会以覆盖模式打开目标文件，因此除非确实要替换，否则应在调用它之前拒绝已存在的输出路径。对于浮点源，它可以保留编码后的事件，同时删除 PnG 或 `timestep`，这会改变之后 `as_array(preprocess=True)` 的结果。请同时验证原始数据和预处理数据的往返结果。

当事件值、事件数量或通道布局发生变化时，请改用 `create_fcs()`。

## 内置检查器

`scripts/inspect_fcs.py` 无需网络访问即可盘点一个或多个数据集。默认情况下，它仅读取元数据，输出结构字段和通道标签，而不会输出完整的 TEXT/ANALYSIS 值，并且会拒绝处理超过可配置大小限制的文件。

将 `FLOWIO_SKILL_DIR` 设置为已安装的 skill 目录。在此代码库的根目录下，使用 `skills/flowio`：
...
- `references/api_reference.md` — FlowIO 1.4.0 的确切公共 API 和签名
- `references/workflows.md` — 盘点、DataFrame/CSV、批处理、写入和往返模式
- `references/fcs_semantics.md` — FCS 结构、元数据规范化、预处理方程、索引和写入器行为
...
- 绝不能直接向 `create_fcs()` 传入二维数组或路径。
- 绝不能假设 TEXT 键会保留 `$` 或大写拼写。
- 绝不能在未说明原因并验证数据的情况下忽略偏移错误。
