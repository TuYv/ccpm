...
使用 Google 的 Gemini CLI 搭配 Gemini 2.5 Pro 进行代码审查——其拥有高达 100 万 token 的超大上下文窗口，可一次性分析整个代码仓库。
...
| 特性 | 优势 |
|---------|---------|
| **Gemini 2.5 Pro** | 面向代码的业界领先推理能力 |
| **100 万 token 上下文** | 可容纳整个代码仓库，无需分块 |
| **免费套餐** | 使用 Google 账号每天可发起 1,000 次请求 |
| **一致的输出** | 格式清晰、结构可预测 |
| **GitHub 原生集成** | 通过 Gemini Code Assist 应用自动审查 PR |

### 基准测试表现

| 基准测试 | 得分 | 说明 |
|-----------|-------|-------|
| SWE-Bench Verified | 63.8% | 智能体编程基准测试 |
...
这将允许你使用 Gemini 2.5 Pro，并获得完整的 100 万 token 上下文窗口。
...
在项目根目录中创建一个 `GEMINI.md` 文件，用于提供项目特定的上下文：
...
## 对比：Claude、Codex 与 Gemini

| 方面 | Claude | Codex CLI | Gemini CLI |
|--------|--------|-----------|------------|
| **设置** | 无（内置） | npm + OpenAI API | npm + Google 账号 |
| **模型** | Claude | GPT-5.2-Codex | Gemini 2.5 Pro |
| **上下文** | 对话 | 每次审查均为全新上下文 | 100 万个 token（非常大！） |
| **免费套餐** | 不适用 | 有限 | 每天 1,000 次 |
| **最适合** | 快速审查 | 高准确率 | 大型代码库 |
| **GitHub 原生支持** | 无 | @codex | Gemini Code Assist |
...
| 工作流中的快速审查 | Claude |
| 关键安全审查 | Codex（88% 检出率） |
| 大型代码库（100+ 个文件） | Gemini（100 万上下文） |
| 免费自动化审查 | Gemini |
| 获取多种视角 | 三者全部使用（双引擎/三引擎） |
...
- **跳过身份验证设置** - 务必在配置 CI/CD 之前完成设置
- **在日志中使用 API 密钥** - 使用密钥管理
- **忽略上下文限制** - 即使是 100 万 token，对超大型单体仓库而言也存在限制
