...
| `get_agent(agent_id)` | 通过 ID 检索现有代理 |
| `as_agent(sdk_agent)` | 封装 SDK Agent 对象（无 HTTP 调用） |
...
| `HostedMCPTool` | `from agent_framework import HostedMCPTool` | 由服务管理的 MCP |
| `MCPStreamableHTTPTool` | `from agent_framework import MCPStreamableHTTPTool` | 由客户端管理的 MCP |
...
- 多轮对话使用 `get_new_thread()`
- 对于服务托管型 MCP，优先使用 `HostedMCPTool`，对于客户端托管型 MCP，使用 `MCPStreamableHTTPTool`

## 参考文件

- references/tools.md：详细的托管工具模式
- references/mcp.md：MCP 集成（托管 + 本地）
- references/threads.md：线程和会话管理
