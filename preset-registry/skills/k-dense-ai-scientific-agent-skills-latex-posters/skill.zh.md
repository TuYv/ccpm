...
科研海报是会议、专题研讨会和学术活动中进行科学传播的重要媒介。本技能提供了使用 LaTeX 宏包创建专业且视觉吸引力强的科研海报的全面指导。生成具有适当布局、排版、配色方案和视觉层次的出版质量海报。

## 使用此技能的场景

在以下情况下应使用此技能：
- 为会议、专题研讨会或海报展示会创建科研海报
- 为大学活动或论文答辩设计学术海报
- 准备用于公众参与的研究可视化摘要
- 将科学论文转换为海报格式
- 为研究小组或院系创建海报模板
...
这些是限制，而不是指导原则。违反这些限制是海报失败最常见的原因。完整的推理说明、按图形类型分类的表格以及实际示例请参见
...
| 内容分区（A0） | **最多 5-6 个** |
| 海报总词数 | **300-800** |
| 图形宽度 | `0.85\linewidth`，绝不能为 `1.0` |

每个图形提示词都必须包含：`POSTER FORMAT for A0`、明确的元素或词数（`ONLY 3 icons`、`3 words total`）、字体大小（`GIANT (120pt+)`）、`60% white space`，以及观看距离（`readable from 10-12 feet`）。

**两个强制性审查关卡。** 跳过其中任何一个，都会导致海报难以阅读：

- **生成前** — 对每个计划中的图形进行确认：包含 3-4 个项目、传达一个信息、少于 10 个词，并且不是包含 5 个或更多阶段的工作流。如果不符合，则将其拆分为多个图形。
- **生成后、组装前** — 以 25% 的缩放比例打开每个图形。所有文本都应清晰可读，元素数量不超过 4 个，留白达到 50% 以上，并且能在 2 秒内理解。任何一项不符合，都意味着必须重新生成或拆分。不要使用未通过审查的图形组装海报。
...
支持三种海报包——**beamerposter**（Beamer 语法、机构主题）、**tikzposter**（现代、彩色、灵活）和 **baposter**（结构化多栏布局）。关于包的比较、布局和网格系统、设计原则、标准尺寸、各包模板、图形和图像集成、配色方案、排版以及 QR 码的内容，均记录在  
[references/latex_poster_reference.md](references/latex_poster_reference.md) 中。

各部分可复用的内容模式、可访问性要求以及展示日指导，详见  
...
**重要：生成包含最少内容的简单图形。每个图形 = 一条信息。**

**内容限制：**
- 每个图形最多 4-5 个元素
- 每个图形总计最多 15 个词
- 至少 50% 的留白
- 超大字号（标签使用 80pt+，关键数字使用 120pt+）
...
3. **检查生成的图示——检查是否溢出：**
   - **以 25% 缩放查看**：所有文字仍然清晰可读吗？
   - **统计元素数量**：超过 5 个？→ 重新生成更简单的图示
   - **检查留白**：少于 40%？→ 在提示词中添加 "60% white space"
   - **字体太小？**：添加 "EVEN LARGER" 或增大 pt 大小
   - **仍然溢出？**：将元素数量从 4-5 个减少到 3 个
...
2. **设计布局结构**：
   - 规划列结构（2 列、3 列或 4 列）
   - 安排内容流向（通常为从左到右、从上到下）
   - 分配标题（10-15%）、内容（70-80%）和页脚（5-10%）的空间
...
2. **整合 AI 生成的图示**：
   - 将阶段 2 中的所有图示添加到相应部分
   - 使用 `\includegraphics` 并设置适当的大小
   - 确保图示主导每个部分（先展示视觉内容，再呈现文字）
   - 将图示置于区块中央，以增强视觉效果
...
2. **测试可读性**：
   - 按 25% 比例打印，并从 2-3 英尺处阅读（模拟从 8-12 英尺处观看海报）
   - 在不同显示器上检查颜色
   - 验证二维码是否正常工作
   - 请同事进行审阅
...
- **Generate Image / Nano Banana Pro**：用于风格化图形、概念插图和摘要视觉内容
- **Scientific Writing**：用于根据论文编写海报内容
- **Literature Review**：用于提供研究背景
- **Data Analysis**：用于创建结果图和图表

**推荐工作流程**：在创建 LaTeX 海报之前，始终先使用 scientific-schematics 和 generate-image 技能生成所有视觉元素。

## 需要避免的常见问题

**AI 生成图形时的错误（最常见）：**
- ❌ 一个图形中包含过多元素（10+ 项）→ 最多保持 3-5 项
- ❌ AI 图形中的文字过小 → 指定 "GIANT (100pt+)" 或 "HUGE (150pt+)"
- ❌ 提示词包含过多细节 → 使用 "SIMPLE" 和 "ONLY X elements"
- ❌ 未指定留白 → 在每个提示词中添加 "50% white space"
- ❌ 包含 8+ 步骤的复杂流程图 → 最多限制为 4-5 步
- ❌ 包含 6+ 项的对比图 → 最多限制为 3 项
- ❌ 包含 5+ 个指标的关键发现 → 仅展示最重要的 3 个

**修复 AI 图形中的溢出问题：**
如果 AI 生成的图形出现溢出或文字过小：
1. 在提示词中添加 "SIMPLER" 或 "ONLY 3 elements"
2. 增大字体："150pt+" 替代 "80pt+"
3. 添加 "60% white space" 替代 "50%"
4. 移除次要细节："NO sub-steps"、"NO axis labels"、"NO legend"
5. 使用更少的元素重新生成
...
- [references/ai_graphics_for_posters.md](references/ai_graphics_for_posters.md)：完整的 AI
  图形规则、各类型限制、经过实践验证的提示词示例以及审查关卡。
- [references/latex_poster_reference.md](references/latex_poster_reference.md)：宏包、
  布局、设计、尺寸、模板、图形、颜色、排版和二维码。
- [references/compilation_and_quality_control.md](references/compilation_and_quality_control.md)：
  编译引擎和完整的印刷前质量控制流程。
- [references/poster_patterns_and_presentation.md](references/poster_patterns_and_presentation.md)：
  内容模式、无障碍设计和演示技巧。
- [references/latex_poster_packages.md](references/latex_poster_packages.md)：对
  beamerposter、tikzposter 和 baposter 的详细比较及示例。
