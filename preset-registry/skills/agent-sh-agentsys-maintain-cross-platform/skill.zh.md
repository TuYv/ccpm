...
**范围：** 仅适用于此仓库的本地技能。包含为维持 Claude Code + OpenCode + Codex CLI 兼容性所需的具体文件位置、转换规则和自动化模式。

## 关键规则

1. **3 个平台必须正常工作** - Claude Code、OpenCode、Codex CLI。无一例外。
2. **每次推送前都必须验证** - 推送前钩子会自动运行 6 个验证器。
3. **所有版本字段必须一致** - 共 11 个文件（package.json + 10 个 plugin.json 文件）。
4. **文档必须准确** - CI 会验证数量、路径和平台引用。
5. **更新此技能** - 如果发现不一致之处或自动化机会，请更新此文件。
...
|-----------|-------------|----------|-----------|
| **命令** | 插件 `commands/` | `~/.config/opencode/commands/` | 不适用（使用技能） |
| **智能体** | 插件 `agents/` | `~/.config/opencode/agents/` | 不适用（使用 MCP） |
| **技能** | 插件 `skills/` | `.opencode/skills/`（单数形式） | `~/.codex/skills/` |
| **钩子** | 插件 `hooks/` | 插件 `hooks/` | 插件 `hooks/` |

### 安装位置（此仓库）

| 平台 | 软件包副本 | 命令 | 智能体 | 技能 | 配置 |
|----------|--------------|----------|--------|--------|--------|
| Claude Code | 通过市场安装 | 随插件捆绑 | 随插件捆绑 | 随插件捆绑 | 不适用 |
...
| `model: sonnet/opus/haiku` | **已移除**（OpenCode 使用用户默认模型） |

**关键：** `bin/cli.js` 中的 `--strip-models` 标志会移除模型配置，以便无法使用全部三个模型层级的 OpenCode 用户正常使用。
...
| 文件 | 用途 |
|------|---------|
| `bin/cli.js` | 主安装程序（811 行）——处理所有 3 个平台 |
| `scripts/setup-hooks.js` | Git hooks 安装程序（pre-commit、pre-push） |
| `adapters/opencode-plugin/` | 原生 OpenCode TypeScript 插件 |
| `adapters/opencode/` | OpenCode 安装脚本（旧版） |
| `adapters/codex/` | Codex 安装脚本（旧版） |
| `mcp-server/index.js` | 跨平台 MCP 服务器 |

### 验证脚本（全部用于 CI + Pre-Push）

| 脚本 | 验证内容 | 以下情况返回 Exit 1 |
|--------|-------------------|-----------|
...
| `scripts/check-hardcoded-paths.js` | 不存在硬编码的 `.claude/` 路径 | 发现硬编码路径 |
| `scripts/validate-counts.js` | 文档准确性（agents、plugins、skills、versions） | 数量不匹配 |
...
- `OPENCODE_COMMAND_MAPPINGS` - 第 242 行 - 要复制给 OpenCode 的命令
- `CODEX_SKILL_MAPPINGS` - 第约 280 行 - 要为 Codex 创建的技能
...
1. **无需更改** - 安装程序会自动将 Agent 复制到 OpenCode 的 `~/.config/opencode/agents/`
2. **Codex 使用 MCP** - 不直接支持 Agent，请改用 MCP 工具
3. **验证会发现问题** - 如果基于文件的 Agent 数量发生变化
...
4. **两者都要提交** - lib/ 中的源文件以及 plugins/*/lib/ 中的副本
...
**原因：** 并非所有 OpenCode 用户都能使用全部三个模型层级。改为使用用户的默认模型。
...
**2. Agent 逻辑 → 内联到 Skill 中**
- Codex 不直接支持 Agent
- Agent 工作流嵌入 Skill 指令中
- 或使用 MCP 工具调用 Agent 逻辑
...
- plugin.json 结构有效性
- 必填字段是否存在
- 已声明的命令、Agent 和 Skill 是否存在

**以下情况以状态码 1 退出：** plugin.json 无效
...
**检查项：**
- Agent、命令和 Skill 中不存在硬编码的 `.claude/` 路径
...
- 文档表格（`| State Dir |`）
- 平台比较示例
- Skill 文档（enhance/* SKILL.md）
- 检查清单引用

**以下情况以状态码 1 退出：** 在安全上下文之外发现硬编码路径

### validate:counts
**文件：** `scripts/validate-counts.js`
**检查项：**
- README、CLAUDE.md、AGENTS.md、package.json 和文档中的插件数量（9）
- 所有文档中的 Agent 数量（共 39 个 = 29 个基于文件的 Agent + 10 个基于角色的 Agent）
- 所有文档中的 Skill 数量（23）
- 版本一致性（package.json 与全部 10 个 plugin.json 文件匹配）
- CLAUDE.md ↔ AGENTS.md 关键规则一致性（相似度 >90%）

**实际数量（文件系统）：**
- 插件：`plugins/` 中有 9 个目录
- 基于文件的 Agent：`plugins/*/agents/` 中有 29 个 .md 文件
- 基于角色的 Agent：10 个（来自 audit-project，内联定义）
- Skill：`plugins/*/skills/*/SKILL.md` 中有 23 个 SKILL.md 文件

**智能验证：**
- 接受“39 个 Agent”或“39 个 Agent（29 个基于文件的 Agent + 10 个基于角色的 Agent）”
- 跳过特定于插件的数量，例如“next-task（12 个 Agent）”
- 仅验证顶层总数

**以下情况退出码为 1：** 计数不匹配或版本未对齐
...
- 跳过对比表格
- 跳过检查清单引用
- 跳过技能名称提及（如 `enhance-claude-memory`）
- 跳过文档示例
...
提示：“你是否已对这些文件运行 /enhance？(y/N)”

若回答为“N”，则阻止推送（依据 CLAUDE.md 关键规则 #7）。
...
**installForCodex(installDir)** - 第 330 行及以后
- 创建目录：`~/.codex/skills/`
...
**CODEX_SKILL_MAPPINGS** - 约第 280 行及之后：
```javascript
const skillMappings = [
...
4. ✅ **模型移除** - 用于 OpenCode 的 `--strip-models` 标志
5. ✅ **版本检查** - validate:counts 可发现版本不一致
6. ✅ **路径检查** - validate:paths 可发现硬编码路径
7. ✅ **/enhance 强制执行** - 预推送钩子会进行提示
...
**基于角色（10）：** 在 audit-project 命令中以内联方式定义
...
- **绝不破坏现有功能**——这是一个拥有真实用户的生产项目
- **始终在所有 3 个平台上进行测试**——至少进行冒烟测试
- **始终更新此技能**——如果发现缺漏或可改进之处
