...
3. **批量提问**：如果工具支持单次调用多个问题，则将所有适用问题合并到一次调用中；如果仅支持单个问题，则按优先级顺序逐个提问。

以下具体的 `AskUserQuestion` 引用仅作为示例——在其他运行时中，请替换为本地等效工具。

## 快速开始

只需运行 `/release-skills` —— 系统会自动检测项目配置。
...
1. 检查 `.releaserc.yml`（可选配置覆盖文件）
   - 如果存在，检查其中是否定义了发布钩子
2. 按以下优先级顺序扫描并自动检测版本文件：
   - `package.json`（Node.js）
   - `pyproject.toml`（Python）
   - `Cargo.toml`（Rust）
   - `marketplace.json` 或 `.claude-plugin/marketplace.json`（Claude 插件）
   - `VERSION` 或 `version.txt`（通用项目）
3. 使用 glob 模式扫描变更日志文件：
   - `CHANGELOG*.md`
   - `HISTORY*.md`
   - `CHANGES*.md`
4. 根据文件名后缀识别每个变更日志的语言
5. 检测 GitHub 发布支持：
...
| 钩子 | 用途 | 预期职责 |
|---------|---------|---------|
| `prepare_artifact` | 使一个目标可发布 | 验证目标是否自包含，同步/嵌入本地依赖项，可选暂存额外文件 |
...
执行规则：
- 保持技能的通用性：不要将注册表/包管理器/项目布局的具体细节硬编码到此 SKILL 中。
- 如果存在 `prepare_artifact`，则在需要最终可发布目标状态的发布相关检查之前，为每个目标运行一次。
- 将发布说明写入临时文件，并将该文件路径传递给 `publish_artifact`；不要将多行变更日志文本直接嵌入 shell 命令。
- 如果不存在钩子，则回退到默认的与项目无关的发布工作流。

**语言检测规则**：

变更日志文件遵循 `CHANGELOG_{LANG}.md` 或 `CHANGELOG.{lang}.md` 格式，其中 `{lang}` / `{LANG}` 是语言或地区代码。
...
- 移除公共 API、重命名导出项、更改接口

如果检测到破坏性变更，则向用户发出警告："检测到破坏性变更。请考虑进行主版本升级（`--major` 标志）。"

### 步骤 3：确定版本升级类型

规则（按优先级排序）：
1. 用户标志 `--major/--minor/--patch` → 使用指定类型
2. 检测到 BREAKING CHANGE → 主版本升级（1.x.x → 2.0.0）
3. 存在 `feat:` 提交 → 次版本升级（1.2.x → 1.3.0）
4. 否则 → 修订版本升级（1.2.3 → 1.2.4）
...
2. **检测第三方贡献者**：
   ...
   - 对于每个已合并的 PR，使用 `gh pr view <number> --json author --jq '.author.login'` 识别 PR 作者
   - 使用 `gh repo view --json owner --jq '.owner.login'` 与仓库所有者进行比较
   - 如果 PR 作者 ≠ 仓库所有者 → 第三方贡献者
3. **以该语言生成内容**：
   - 使用目标语言编写章节标题
   - 使用目标语言自然地撰写变更描述（而不是进行翻译）
   - 日期格式：YYYY-MM-DD（通用）
   - **第三方贡献**：在变更日志条目中追加贡献者署名 `(by @username)`
4. **插入到文件开头**（保留现有内容）
...
- 使用带 `@` 前缀的 GitHub 用户名
- 放在变更日志条目行末
- 所有语言统一适用（始终使用 `(by @username)` 格式，不进行翻译）
...
分析上一个标签以来的提交，并按受影响的技能/模块分组：

1. **识别**每个提交中发生变更的文件
2. **按技能/模块分组**：
   - `skills/<skill-name>/*` → 归入对应技能
   - 根目录文件（CLAUDE.md 等）→ 归入“项目”
   - 一个提交涉及多个技能 → 拆分为多个分组
...
3. **创建发布说明文件**：
   ...
   - 如果不存在英文/默认变更日志，则使用检测到的第一个变更日志
   - 仅提取从精确的 `## {VERSION} - {YYYY-MM-DD}` 部分开始到下一个 `##` 之前的内容
   - 必要时同时匹配普通版本号和带标签前缀的标题，例如 `1.2.3` 和 `v1.2.3`
   - 将破坏性变更置于靠前位置；如有需要，在其他部分之前添加简短的重点摘要
   - 将说明写入 UTF-8 临时文件，并将其复用于带注释的标签消息、GitHub Releases 和 `publish_artifact`
   - 在正常模式下，如果找不到说明，则停止操作，不创建空标签或 GitHub Release
...
3. **发布 GitHub Release**（单选）：
   ...
   - 如果用户同时选择推送，则默认选择 "Yes, publish after tag push"
...
3. **创建带注释的标签**：
   ...
   ```
   如果 `.releaserc.yml` 将 `tag.sign: true` 设为启用，则使用 `git tag -s` 和同一个说明文件。
...
### 步骤 10：发布构建产物和 GitHub Release

项目构建产物发布和 GitHub Releases 是两个独立的输出：

1. **项目构建产物**：
   - 如果存在 `release.hooks.publish_artifact`，则针对每个已准备的目标运行一次
   - 传入与标签和 GitHub Release 所使用的相同 `{release_notes_file}`
   - 在 dry-run 模式下，传入 `{dry_run}=true`，并报告将要发布的内容

2. **GitHub Release**：
   - 仅当用户已确认远程发布且 GitHub 支持可用时运行
   - 创建 Release 前，确保远程已存在该标签
   - 使用提取出的说明创建或更新：
     ```bash
...
当用户要求回填历史版本，或传入 `--backfill-releases` 时，使用此模式。

1. 不要递增版本、编辑变更日志或创建发布提交。
2. 按版本顺序列出现有标签，并检测缺失的 Releases：
   ```bash
...
3. 对于没有 GitHub Release 的每个标签：
   ...
   - 使用以下命令创建 Release：
     ...
     ```
4. 使用 `git cat-file -t <tag>` 检测轻量标签（`commit` 表示轻量标签，`tag` 表示带注释的标签）。
5. 默认不要重写公开的轻量标签。将现有远程标签转换为带注释的标签需要用户明确确认，因为这会重写已发布的引用。
...
**重要**：如果用户说“just push”或“直接 push”，且存在未提交的更改，仍然要先执行上述所有步骤。