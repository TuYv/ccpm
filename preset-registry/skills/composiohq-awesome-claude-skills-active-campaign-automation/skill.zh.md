...
- 通过 `RUBE_MANAGE_CONNECTIONS` 使用工具包 `active_campaign` 建立 ActiveCampaign 连接
- 始终先调用 `RUBE_SEARCH_TOOLS` 以获取当前工具 schema

## 设置

**获取 Rube MCP**：在你的客户端配置中将 `https://rube.app/mcp` 添加为 MCP server。无需 API key——只需添加端点即可。

1. 通过确认 `RUBE_SEARCH_TOOLS` 响应来验证 Rube MCP 是否可用
2. 使用工具包 `active_campaign` 调用 `RUBE_MANAGE_CONNECTIONS`
3. 如果连接未处于 ACTIVE，请使用返回的授权链接完成设置
...
- **检查连接**：在执行工具前，请确认 `RUBE_MANAGE_CONNECTIONS` 显示 ACTIVE 状态
- **Schema 合规**：使用与搜索结果完全一致的字段名和类型
- **Memory 参数**：在 `RUBE_MULTI_EXECUTE_TOOL` 调用中始终包含 `memory`，即使为空也要传 `{}`
- **会话复用**：在同一工作流内复用会话 ID。对新工作流生成新的 ID
...
| 操作 | 方法 |
|-----------|----------|
| 查找工具 | 使用 ActiveCampaign 特定用例调用 `RUBE_SEARCH_TOOLS` |
| 连接 | 使用工具包 `active_campaign` 调用 `RUBE_MANAGE_CONNECTIONS` |
