...
### 应用程序
- [ ] 所有测试均通过（单元测试、集成测试、E2E 测试）
- [ ] 代码或配置文件中不存在硬编码密钥
- [ ] 错误处理覆盖所有边界情况
- [ ] 日志采用结构化格式（JSON），且不包含 PII
- [ ] 健康检查端点返回有意义的状态

### 基础设施
- [ ] Docker 镜像可复现构建（版本已固定）
- [ ] 环境变量已记录在文档中，并在启动时进行验证
- [ ] 已设置资源限制（CPU、内存）
- [ ] 已配置水平扩缩容（最小/最大实例数）
- [ ] 所有端点均已启用 SSL/TLS

### 监控
- [ ] 已导出应用程序指标（请求速率、延迟、错误）
- [ ] 已配置错误率 > 阈值时触发的告警
- [ ] 已设置日志聚合（结构化日志、可搜索）
- [ ] 已对健康检查端点设置可用性监控
