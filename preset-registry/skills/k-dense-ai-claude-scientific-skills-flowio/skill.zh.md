...
使用 FlowIO 作为 Flow Cytometry Standard 文件的轻量级底层读取器和写入器。本技能中的示例针对 **FlowIO 1.4.0**，这是截至 2026-07-23 已验证的当前稳定版本。
...
- 为 pandas、机器学习或下游细胞术工具准备数据

FlowIO **不**执行补偿、logicle/双指数变换、门控、聚类或 FlowJo 工作区处理。对于这些任务，请使用 FlowKit 或其他分析包。
...
1. **明确操作。** 区分元数据清单、事件提取、文件修复、格式转换和下游生物学分析。
2. **在加载事件前进行检查。** 对于仅处理元数据的任务，尤其是处理大型或不熟悉的文件时，使用 `only_text=True`。
3. **明确选择事件语义。** 使用 `as_array(preprocess=True)` 根据 FCS 元数据应用增益/对数/时间缩放，或使用 `preprocess=False` 获取 DATA 段中编码的值。记录所作选择。
4. **默认保持解析严格。** 不要自动抑制偏移量错误。只有在已知供应商格式缺陷时才放宽检查，并检查由此产生的事件数据。
5. **将元数据视为潜在敏感信息。** FCS TEXT 值可能包含样本、受试者、操作员和仪器标识符。仅导出任务所需的字段。
6. **通过重新打开文件验证写入结果。** 进行任何 FCS 导出后，检查事件/通道数量、标签、元数据和具有代表性的值。

## 关键语义

### TEXT 键会被规范化

`FlowData.text` 会将键存储为小写，并从标准 FCS 关键字中去除开头的 `$`：
...
不要查找 `"$DATE"`、`"$CYT"` 或其他带有美元符号前缀的大写键。TEXT 值仍为字符串。FlowIO 1.4.0 还会从解码后的 TEXT 段中移除所有 `$` 字符，包括值内部的 `$` 字符；当精确的元数据保真度很重要时，请保留原始文件。

### 事件有两种表示形式

- `flow.events` 是未经处理的一维扁平事件数组。
- `flow.as_array()` 返回形状为 `(event_count, channel_count)` 的 NumPy `float64` 数组。
- `flow.as_array(preprocess=True)` 应用 FCS 增益、对数和时间缩放。它不会应用补偿或 logicle/双指数显示变换。
...
- `null_channels` 包含通过 `null_channel_list` 提供的 PnN 标签字符串，也包括所提供但未找到的标签。
...
优先使用路径或 `Path`，而不是调用方自行管理的文件句柄。`FlowData` 会在解析后关闭所提供的句柄。在 FlowIO 1.4.0 中，`read_multiple_data_sets(handle)` 可能会在第一个数据集之后失败，因为句柄已经关闭；对于包含多个数据集的文件，请传入文件系统路径。

## 快速开始：读取多个数据集

使用独立的辅助函数，而不要手动解释 `$NEXTDATA` 偏移量：
...
元数据键可以使用混合大小写或带 `$` 的形式提供，但不带 `$` 的小写键与 FlowIO 的规范化表示一致，更不易出错。  
元数据值必须是字符串。
...
传入 `metadata=None` 会保留 FlowIO 选定的默认值。传入任何字典（包括 `{}`）都会替换这些默认值，而不是与其合并。`write_fcs()` 始终生成 FCS 3.1 浮点输出；非浮点源事件会在写入前进行预处理。它会以覆盖模式打开目标文件，因此除非确实有意替换，否则应在调用它之前拒绝已存在的输出路径。对于浮点源文件，它可以保留编码后的事件，但会删除 PnG 或 `timestep`，从而改变之后 `as_array(preprocess=True)` 的结果。请同时验证原始数据和预处理数据的往返结果。

当事件值、事件数量或通道布局发生变化时，改用 `create_fcs()`。

## 内置检查器

`scripts/inspect_fcs.py` 可以在无需网络访问的情况下盘点一个或多个数据集。默认情况下，它只读取元数据，输出结构字段和通道标签，而不输出完整的 TEXT/ANALYSIS 值，并拒绝处理超过可配置大小限制的文件。
...
- `references/troubleshooting.md` — 偏移量失败、多数据集文件、内存限制、验证、安全性和隐私
...
- 绝不能直接将二维数组或路径传递给 `create_fcs()`。
- 绝不能假设 TEXT 键会保留 `$` 或大写拼写。
- 绝不能在未说明原因并验证数据的情况下静默忽略偏移错误。
