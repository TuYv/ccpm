...
使用 LoRA、QLoRA 和 25 种以上的适配器方法，仅训练不到 1% 的参数即可微调 LLM。

## 何时使用 PEFT

**在以下情况下使用 PEFT/LoRA：**
- 在消费级 GPU（RTX 4090、A100）上微调 7B-70B 模型
- 需要训练不到 1% 的参数（6MB 适配器，相比之下完整模型为 14GB）
- 希望通过多个任务专用适配器进行快速迭代
- 基于一个基础模型部署多个微调变体
...
| 方法 | 可训练参数占比 | 内存 | 速度 | 最适合 |
|--------|------------|--------|-------|----------|
| **LoRA** | 0.1-1% | 低 | 快 | 通用微调 |
| **QLoRA** | 0.1-1% | 极低 | 中等 | 内存受限场景 |
| AdaLoRA | 0.1-1% | 低 | 中等 | 自动秩选择 |
| IA3 | 0.01% | 极少 | 最快 | 少样本适配 |
| 前缀微调 | 0.1% | 低 | 中等 | 生成控制 |
| 提示微调 | 0.001% | 极少 | 快 | 简单任务适配 |
...
1. **从 r=8-16 开始**，如果质量不足则增大
2. **使用 alpha = 2 * rank** 作为起点
3. **以注意力层 + MLP 层为目标模块**，以获得最佳质量和效率
4. **启用梯度检查点**以节省内存
5. **频繁保存适配器**（文件小，易于回滚）
6. **合并前在留出数据上进行评估**
