...
---
Vue.js Options API 最佳实践、TypeScript 集成以及常见陷阱。
...
- 事件处理函数的参数需要适当的类型安全 → 参见 [ts-options-api-type-event-handlers](reference/ts-options-api-type-event-handlers.md)
- 需要使用接口为对象或数组类型的 prop 标注类型 → 参见 [ts-options-api-proptype-complex-types](reference/ts-options-api-proptype-complex-types.md)
...
### 方法与生命周期
- 方法未能绑定到组件实例上下文 → 参见 [no-arrow-functions-in-methods](reference/no-arrow-functions-in-methods.md)
