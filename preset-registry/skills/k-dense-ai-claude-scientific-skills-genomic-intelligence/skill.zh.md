...
Genomic Intelligence（GI）在托管 GPU 上为六项序列分析任务提供 Transformer DNA 语言模型服务。提供**基因符号**、**基因组区域**或 **DNA/FASTA 序列**；它会返回结构化预测结果——启动子区域、剪接位点、增强子活性、染色质状态、表达量（log TPM）以及从头基因注释。所有操作均不在本地运行：无需模型权重、GPU 或庞大的 Python 技术栈。它只是托管式、有版本控制的推理 API 的轻量客户端。
...
当用户拥有 DNA 并希望获得模型预测时，使用 GI：

- **查找启动子**：在基因组区域中（`promoter`）
- **预测剪接**供体/受体位点（`splice`）
- **评估增强子活性**——发育相关与管家型（`enhancer`）
- **注释染色质状态**：跨数百个轨道（`chromatin`）
- **预测表达量**：根据序列 + 细胞类型上下文预测 log(TPM+1)（`expression`）
- **从头注释基因/转录本**：无需参考序列（`annotation`）
- **查找区域中的基因并预测每个基因的表达量**（复合任务）
...
### 托管 MCP 服务器（最适合 AI agent——无需密钥）

GI 在 `https://mcp.genomicintelligence.ai/mcp` 托管 MCP 服务器（可流式传输的
HTTP）。当你的 agent 宿主支持 MCP 时，优先使用它：它可以**无需密钥**使用
有限额的公共演示配额（零配置），而可选的 `gi_` bearer 密钥可以提高配额。它提供获取工具，返回一个**序列句柄**
（`sequence_ref`），以及接受该句柄的 `predict_*` 工具——因此大序列不会使上下文膨胀。请参见下方的 [MCP 工作流](#mcp-workflow-handle-based) 和
`references/mcp.md`。

### REST API（通用）

使用 `requests` 通过 `https://api.genomicintelligence.ai/v1` 发送普通 HTTP 请求。REST 路径**需要**`GI_API_KEY`（一个 `gi_` bearer 密钥）。可在任何主机、脚本中使用，或在需要原始信封时使用。请参见[核心 REST 工作流](#core-rest-workflow)。

## 访问与身份验证

1. **托管 MCP 演示服务无需密钥**——无需设置任何内容即可试用。
2. **REST `/v1` API 需要密钥**，通过 `Authorization: Bearer <key>` 发送。
   可通过 [contact@genomicintelligence.ai](mailto:contact@genomicintelligence.ai) 申请。
3. **绝不要将密钥硬编码。** 从 `GI_API_KEY` 环境变量中读取
   （或通过 `python-dotenv` 读取 `.env`）。绝不要提交密钥。
...
密钥按合作伙伴层级设定并发数和每分钟上限。`429`
表示你已达到某项上限——请退避并重试，或请求 GI 提升你的层级。

## 六项任务

所有 REST 任务都采用相同的形式：`POST /v1/tasks/{task}/predict`，请求体为
`{sequence, sequence_name, model?, options?}`，返回 `{data, meta}`
封装结构。各任务的不同之处如下：

| Task | Mode | Length bound | Notes |
|---|---|---|---|
| `promoter` | sync | 1–500,000 bp | 滑动窗口启动子区域 |
| `splice` | sync | 1–500,000 bp | 供体/受体位点（长上下文 BigBird） |
| `enhancer` | sync | 1–500,000 bp | 发育 + 管家基因评分（DeepSTARR，*Drosophila*） |
| `chromatin` | sync | 1–500,000 bp | 数百条轨道（DeepSEA） |
| `expression` | sync | **exactly 9,198 bp** | log(TPM+1)；需要细胞类型 `description` |
| `annotation` | **async** | 1–500,000 bp | 从头转录本；提交 + 轮询 |

**省略 `model`，API 就会使用该任务的默认模型**——这是推荐的调用方式。默认模型 ID
在此处特意**不予记录**：默认值会发生变化，而已退役的 ID 会直接失败，因此绝不要硬编码模型
ID。若要固定某个模型，或选择非人类模型（多个任务提供果蝇、酵母和拟南芥模型），请在调用时使用
`GET /v1/tasks/{task}/models`（REST）或 `list_models`（MCP）发现 ID——并且**绝不要凭空编造 ID**。每项任务的完整输出结构见
`references/tasks.md`。

模型强制执行以下两条规则：

- **`expression` 需要正好 9,198 bp**，且窗口必须**以 TSS 为中心**
  （上游 4,599 bp + TSS + 下游 4,598 bp）。任何其他长度都会被拒绝。请使用下面的序列获取辅助工具
  来构建它——不要手动截断。
- **`expression` 需要 `description`**——一个细胞类型 / assay 字符串（例如
...
- **从基因符号获取** → MCP `fetch_ensembl_sequence(gene=...)`；**从坐标获取** → `fetch_region(region=...)`。两者都会获取公开的 Ensembl 参考
  序列（无需密钥）。REST 用户可以直接查询 Ensembl REST。（`find_genes` 是注释任务，而不是获取工具。）
- **对于 `expression`** → 使用以 TSS 为中心的获取方式，使窗口长度恰好为
  9,198 bp。MCP：`fetch_gene_for_expression`（负责居中）。不要手动构建窗口。
- **从本地 FASTA 获取** → MCP `store_inline_sequence`，或 REST 用户自行读取文件。（`load_local_fasta` 仅存在于本地部署中，托管服务器上不可用。）
- **获取演示序列** → MCP `load_demo_sequence(name=...)` 会返回一个可直接使用的句柄
  （非常适合进行无需密钥的冒烟测试）；`name` 为必需参数。

有关确切的 Ensembl 调用和表达窗口计算方式，请参阅
...
`annotation` 采用提交后轮询。发送 `Prefer: respond-async`，获取 `job_id`，
轮询直到终止状态：
...
要回答“该区域中有哪些基因，以及它们如何表达？”，请使用以下组合流程：

- **MCP：** `find_genes_and_predict_expression(sequence_ref=..., description=...)`
  — 接受的是**句柄，而不是区域**（先使用 `fetch_region` 获取句柄）；
  `description` 是必需的。查找序列中的基因，并返回每个基因的
  表达预测结果。
- **REST：** 调用基因发现接口，然后对每个基因循环调用 `expression`（通过序列获取辅助工具为每个基因构建以 TSS 为中心、长度为 9,198 bp 的窗口）。
...
| 401 | 缺少/无效密钥（REST） | 设置 `GI_API_KEY`；或使用无密钥 MCP demo |
| 413 | 序列过长 | 保持在任务的长度限制内（≤500,000 bp） |
| 429 | 速率 / 并发上限 | 降低请求频率并重试；请求 GI 提高你的层级 |
| 422 | 验证失败（`validation_failed`） | 最常见的失败原因：expression 长度不是恰好 9,198 bp，或序列短于模型的最小长度要求 |
| 5xx | 服务器错误 | 重试；如果问题持续存在，请联系支持团队 |

## 参考文件

- `references/tasks.md` — 各任务的输出格式、模型注册表以及异步
  注解契约。
- `references/api-and-auth.md` — REST 端点、`{data, meta}` 信封格式、
  身份验证、基础 URL 覆盖方式和层级。
