...
| List Event Schema Sets | [SKILL.md § List Event Schema Sets](#list-event-schema-sets) | |
| Get Event Schema Set (Properties) | [SKILL.md § Get Event Schema Set (Properties)](#get-event-schema-set-properties) | |
| Get Event Schema Set Definition | [SKILL.md § Get Event Schema Set Definition](#get-event-schema-set-definition) | Decode base64 definition → summarize eventTypes & schemas |
| Validate Event Schema Set Configuration | [SKILL.md § Validate Event Schema Set Configuration](#validate-event-schema-set-configuration) | |
| Must / Prefer / Avoid | [SKILL.md § Must / Prefer / Avoid](#must--prefer--avoid) | **MUST DO / AVOID / PREFER** checklists |
| Examples | [SKILL.md § Examples](#examples) | Prompt → command-flow pairs (list, inspect, decode definition, get schema version, list business events) |
...
- **Refuse write requests up front, before any listing or inspection** — if the user asks
...
- Do NOT modify the event schema set nor event schema with this consumption skill — a dedicated authoring skill is not yet available, so use the Fabric Event Schema Set authoring REST APIs for writes
...
- For authoring operations, a dedicated authoring skill is not yet available — use the Fabric Event Schema Set authoring REST APIs directly.
