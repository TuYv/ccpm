...
Claude Code 负责协调将任务路由给 Kimi 和 Codex。用户只与 Claude 交互——委派过程在后台进行。
...
## Codex 自动审查（Stop Hook — 自动执行）

当安装了 Codex 时，测试通过后，Stop hook 会审查代码：

1. TDD 循环运行测试
2. `codex-auto-review.sh` 使用 Codex 对差异进行审查
3. Critical/High 级别的问题反馈给 Claude（退出码 2）
4. 审查通过时直接继续（退出码 0）

**完全自动执行。** 用户或 Claude 无需执行任何操作。

---

## Kimi 委派（由 Claude 协调）

当安装了 Kimi 且任务复杂度受控时，Claude 会直接进行委派——用户无需运行任何命令。

### 第 1 步：评估复杂度，而不是文件数量

文件数量并不能很好地代表委派风险。对授权路径进行的 1 个文件修改，可能比涉及 12 个文件的重命名更难。根据来自 iCPG 信号以及 Claude 语义推理的结果，从以下五个维度对任务评分，每个维度 0-2 分：

| 维度 | 0（低） | 1（中） | 2（高） | 来源 |
|---|---|---|---|---|
| **圈复杂度 / 表面深度** | <10 LOC，无分支 | 10-50 LOC，≤3 个分支 | 50+ LOC 或嵌套控制流 | 对函数体执行 iCPG `query_graph` |
| **扇出（消费者影响范围）** | 0-2 个调用方 | 3-10 个调用方 | 11+ 个调用方 | iCPG `trace_path(<symbol>, mode=callers)` |
| **跨越安全边界**（SEC-006、auth、PII、RLS、org-scope、billing、payments） | 无 | 间接相关 | 直接读取或写入 | iCPG SEC-* / R-063 标签 + grep 查找 `org_id`、`user_id`、`auth`、`pii` |
| **并发 / 事务性** | 纯函数 / 同步 | 仅异步 | 锁、事务、原子认领、`FOR UPDATE`、`asyncio.Lock`、`session.begin` | iCPG 并发标记 + grep |
| **所需领域不变量** | 无 / 已在内联文档中充分说明 | 存在一些隐式要求（需要阅读 1-2 个文件） | 很重（跨文档、受 ADR 约束、受 RFC 约束） | Claude 推理 + iCPG ADR 关联 |
...
| 总分 | 路由 | 理由 |
|---|---|---|
| **0-3** | Kimi 单独处理 | 影响范围受控，不涉及安全、并发或跨文档问题 |
| **4-6** | Kimi → Codex 自动审查（无需询问用户） | 存在实际风险，但尚未高到需要 Claude 的完整上下文——Codex 可以捕获 Kimi 可能遗漏的问题 |
| **7-10** | Claude 直接处理 | 涉及跨领域、安全关键或并发密集型问题——需要完整上下文 |

### 第 3 步：下限——简单任务捷径

对于真正简单的工作，可以跳过 iCPG 查询成本：
...
这可以处理简单的重命名/拼写错误修复场景，而无需承担 iCPG 往返的开销。

### 不要委派的情况（覆盖评分结果）

- 用户明确要求 Claude 来完成
- 跨服务变更（API + 前端 + 数据库）——无论评分如何，都需要完整上下文
- 发布分支上的生产环境热修复——跨工具审查延迟过高
- 任一单项维度评分达到 7+（只要有一个关键维度达到该分数，就应让 Claude 保持参与）

### 第 4 步：通过 Bash 委派

Claude 写入一个 mnemos 检查点，然后以无头模式运行 Kimi：
...
在进行任何代码变更之前，Claude 都必须运行以下命令（委派时也要包含结果）：
...
检查点就是桥梁。Claude 写入检查点，Kimi 读取检查点：
