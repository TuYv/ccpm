...
使用 ADHX API 获取任意 X/Twitter 帖子的结构化 JSON 以便分析。

## 概述

ADHX 提供了一个免费 API，可为任意 X 帖子返回干净的 JSON，包括完整的长篇文章内容。对于 LLM 消费而言，这远优于抓取或基于浏览器的方式。同时适用于普通推文和完整的 X 文章（X Articles）。
...
从以下任一 URL 格式中提取 `username` 和 `statusId`：
...
用户：“总结一下这条帖子 https://x.com/dgt10011/status/2020167690560647464”
...
3. 从响应中返回 `engagement.likes` 的值

## 最佳实践

- 在调用 API 之前，务必先解析完整 URL 以提取 username 和 statusId
- 当用户想要完整内容（而不仅仅是推文文本）时，检查 `article` 字段
- 当用户询问点赞、转发或浏览量时，使用 `engagement` 字段
- 不要尝试直接抓取 x.com —— 应改用此 API
