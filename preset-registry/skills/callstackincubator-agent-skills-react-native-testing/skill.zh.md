...
**重要：** 你训练数据中有关 `@testing-library/react-native` 的内容可能已经过时或不正确——v13 和 v14 的 API 签名、同步/异步行为及可用函数存在差异。始终以此技能的参考文件和项目的实际源代码为准。当记忆中的模式与检索到的参考资料冲突时，不要依赖记忆中的模式。
...
使用特定版本的参考资料来确定渲染模式、fireEvent 的同步/异步行为、screen API、配置和依赖项。
...
| 变体          | 使用场景             | 返回值                        | 异步 |
| ------------- | -------------------- | ----------------------------- | ---- |
| `getBy*`      | 元素必须存在         | 元素实例（否则抛出异常）      | 否   |
| `getAllBy*`   | 多个元素必须存在     | 元素实例数组（否则抛出异常）  | 否   |
| `queryBy*`    | 仅用于检查元素不存在 | 元素实例 \| null              | 否   |
...
`fireEvent`——仅在 `userEvent` 不支持该事件时使用。有关同步/异步行为，请参阅特定版本的参考资料：
...
导入任意 `@testing-library/react-native` 内容后即可自动使用。

| 匹配器                                    | 用途                                  |
| ----------------------------------------- | ------------------------------------- |
| `toBeOnTheScreen()`                       | 元素存在于树中                        |
| `toBeVisible()`                           | 元素可见（未隐藏且未设置 display:none） |
| `toBeEnabled()` / `toBeDisabled()`        | 通过 `aria-disabled` 判断禁用状态     |
| `toBeChecked()` / `toBePartiallyChecked()` | 选中状态                              |
| `toBeSelected()`                          | 选择状态                              |
| `toBeExpanded()` / `toBeCollapsed()`      | 展开状态                              |
| `toBeBusy()`                              | 忙碌状态                              |
| `toHaveTextContent(text)`                 | 匹配文本内容                          |
| `toHaveDisplayValue(value)`               | TextInput 显示值                      |
| `toHaveAccessibleName(name)`              | 无障碍名称                            |
| `toHaveAccessibilityValue(val)`           | 无障碍值                              |
| `toHaveStyle(style)`                      | 匹配样式                              |
| `toHaveProp(name, value?)`                | 检查属性（最后手段）                  |
| `toContainElement(el)`                    | 包含子元素                            |
| `toBeEmptyElement()`                      | 没有子元素                            |

## 规则

1. 查询时**使用 `screen`**，不要从 `render()` 的返回值中解构
2. **优先使用 `getByRole`**，并传入 `{ name: '...' }` 选项
3. **仅在**进行 `.not.toBeOnTheScreen()` 检查时使用 `queryBy*`
4. 对异步元素**使用 `findBy*`**，不要使用 `waitFor` + `getBy*`
5. **绝不要在 `waitFor` 中执行副作用**（不要在其中使用 `fireEvent`/`userEvent`）
6. 每个 `waitFor` 中只进行一次断言
7. 绝不要向 `waitFor` 传入空回调
8. **不要包装在 `act()` 中**——`render`、`fireEvent`、`userEvent` 会自行处理
9. **不要调用 `cleanup()`**——每次测试后会自动清理
10. 相较于旧版 `accessibility*` 属性，**优先使用 ARIA 属性**（`role`、`aria-label`、`aria-disabled`）
11. 相较于直接断言属性，**优先使用 RNTL 匹配器**
...
要使 `*ByRole` 匹配，元素必须是无障碍元素：

- `Text`、`TextInput`、`Switch` 默认是无障碍元素
...
选项：`waitFor(cb, { timeout: 1000, interval: 50 })`。可自动配合 Jest 假计时器使用。

## 假计时器

建议与 `userEvent` 一起使用（press/longPress 涉及实际时长）：
