...
此技能可创建 JSON-LD 格式的 Schema.org 结构化数据标记，帮助搜索引擎理解你的内容，并在 SERP 中启用富媒体搜索结果。

## 必须触发此技能的情况

当对话涉及以下任一情况时，请使用此技能——即使用户未使用 SEO 术语：

只要任务需要可交付的资产或转换结果，并且该结果应直接进入质量审核、部署或监控流程，就使用此技能。
...
**仅使用手动提供的数据时：**
要求用户提供：
1. 页面 URL 或完整 HTML 内容
2. 页面类型（文章、产品、FAQ、操作指南、本地商家等）
3. schema 所需的具体数据（价格、日期、作者信息、问答对等）
...
1. **识别内容类型和富媒体搜索结果机会**
   ...
   参考 CORE-EEAT 基准中的 **O05（Schema 标记）**，进行内容类型到 schema 的映射：
...
2. **生成 Schema 标记**
   ...
   > **参考资料**：有关所有 schema 类型（包含必需属性和可选属性）的完整、可直接复制使用的 JSON-LD 模板，请参阅 [references/schema-templates.md](references/schema-templates.md)。
...
### 输入验证
- [ ] 已提供页面 URL 或内容
- [ ] Schema 类型适合内容（例如，博客使用 Article，电子商务使用 Product）
...
- [ ] Schema 内容与页面上的可见内容完全匹配
- [ ] 通过 ~~Schema 验证器，且无错误
- [ ] 清楚注明每个数据点的来源（~~网络爬虫提取、用户提供或手动录入）
...
_实现：将上述 JSON-LD 包裹在 `<script type="application/ld+json">...</script>` 中，并放置于 `<head>` 内或 `</body>` 之前。使用 ~~Schema 验证器进行测试。_
...
1. **匹配可见内容** - Schema 必须反映用户看到的内容
2. **不要滥用** - 仅为相关内容添加 Schema
3. **及时更新** - 日期和价格发生变化时及时更新
4. **充分测试** - 部署前进行验证
5. **监控 Search Console** - 留意错误和警告

## Schema 类型决策树

> **参考**：完整的决策树（内容到 Schema 的映射）、特定行业的建议、实施优先级层级（P0-P4）以及验证快速参考，请参阅 [references/schema-decision-tree.md](references/schema-decision-tree.md)。
