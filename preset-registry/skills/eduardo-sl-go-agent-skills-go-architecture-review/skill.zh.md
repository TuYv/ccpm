...
- **布局审查**（默认）——根据下文章节评估现有代码库，并报告违规项及其严重程度。
- **重构计划**——进行相同的评估，但交付物是按顺序排列的迁移计划（优先执行最小且安全的步骤），而不仅仅是审查结果。
- **新服务咨询**——当被问及“我应该如何组织 X”时：将第 1～3 节作为规范性指导，而不是审查检查项。

## 审查大型代码库

对于包含许多包的代码仓库，请先构建依赖关系图，再进行判断：

1. 梳理模块：使用 `go list ./...` 获取包列表，然后通过 import 语句追踪依赖方向。
2. 执行相互独立的检查：（a）依据第 1 节检查布局；（b）依据第 2 节检查依赖方向；（c）依据第 3 节和第 5 节检查装配与配置；（d）依据第 4 节检查包设计。
3. 如果你的环境支持将工作委派给并行子代理或任务，请为每项检查各分配一个；最后再汇总结果——依赖关系方面的发现通常能够解释布局方面的问题。
4. 每项发现都必须引用包路径和 `file.go:line`。
...
- 每个二进制程序使用一个 `main.go`，其中只包含最少的逻辑。

## 2. 依赖方向

依赖关系必须向内流动。领域核心不得包含任何外部依赖：
...
- `service/` 依赖 `domain/` 中的类型和接口，而不是具体的 store。
- `handler/` 依赖 `service/` 中的接口。
- `store/` 实现在 `service/` 或 `domain/` 中定义的接口。
- 循环依赖属于 🔴 阻断性问题。编译器会捕获它们，但设计本身就应防止它们出现。
...
包含 20 个相关文件的包，优于每个只包含 1 个文件的 20 个包。
应在职责不同时拆分包，而不是在包变大时拆分。
...
规则：
- 所有配置均来自环境变量（十二要素应用）。
- 在启动时验证，遇到问题立即失败并给出清晰的消息。
- 不要将配置分散在各个包中——统一集中在 `internal/config`。
- 绝不要硬编码值。即使“只是暂时如此”也不行。
