...
SimPO 是一种无需参考模型、性能优于 DPO 的无参考偏好优化方法。
...
- **DPO**：需要参考模型基线，更为保守
- **PPO**：控制能力最强，需要奖励模型，设置复杂
- **GRPO**：节省内存的强化学习方法，无需评论家模型
...
**损失函数**：请参阅 [references/loss-functions.md](references/loss-functions.md)，了解 sigmoid 与 hinge 损失、数学公式以及各自的适用场景。

**超参数调优**：请参阅 [references/hyperparameters.md](references/hyperparameters.md)，了解 beta、gamma、学习率选择指南以及针对不同模型规模的建议。

**数据集准备**：请参阅 [references/datasets.md](references/datasets.md)，了解偏好数据格式、质量筛选以及自定义数据集创建。
