...
**明智地选择分区键，围绕访问模式进行设计，了解一致性方面的权衡。**

Cosmos DB 会跨分区分发数据。分区键的选择决定了可扩展性、性能和成本。请围绕均匀分布和高效查询进行设计。
...
| **Logical Partition** | 具有相同分区键的项 |
| **Physical Partition** | 存储单元（最大 50GB、10K RU/s） |
| **RU (Request Unit)** | 吞吐量计量单位 |
...
| 级别 | 保证 | 延迟 | 使用场景 |
|-------|-----------|---------|----------|
| **强一致性** | 线性一致性读取 | 最高 | 财务、库存 |
| **有界过时** | 在界限内保持一致 | 高 | 排行榜、计数器 |
| **会话** | 读取到自己的写入 | 中 | 用户会话（默认） |
| **一致前缀** | 有序读取 | 低 | 社交动态 |
...
| **仅为查询的内容建立索引** | 降低写入 RU 成本 |
| **使用点读取** | 1 RU，而查询需要 3+ RU |
| **开发/测试使用无服务器** | 按请求付费 |
| **生产环境使用自动缩放** | 在低流量期间缩减规模 |
| **为临时数据设置 TTL** | 自动删除旧项目 |
...
- **索引过多** - 增加写入成本；仅为查询的路径建立索引
- **过大的项目** - 最大为 2MB；将 blob 存储在 Azure Blob Storage 中
- **忽略 RU 成本** - 监控并优化成本高昂的查询
- **处处使用强一致性** - 除非有必要，否则使用 Session（默认值）
