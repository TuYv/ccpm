...
引导用户将其 CMS 连接到 notfair 的 SEO 分析工具。

配置完成后，`/seo-analysis` 会自动从 CMS 拉取已发布的内容，并与 Google Search Console 数据进行交叉比对，从而找出不可见页面、内容缺口、过时文章以及缺失的 SEO 字段。

---

## 步骤 0 — 设置

阅读并遵循 `../shared/preamble.md`——该文件会定位 SEO 脚本目录。调用下方所有脚本时，使用前言中的 `$SKILL_SCRIPTS`。
...
- `CMS_STATUS=0` → 已配置 CMS（`$CMS_TYPE` 是其名称）。
  ...
  等待用户回复。如果用户表示要重新配置或切换，则继续执行步骤 2。
  如果用户表示要测试或验证，则跳转到步骤 5（直接进行连接测试）。

- `CMS_STATUS=2` → 尚未配置任何 CMS。继续执行步骤 2。
...
> “你要连接哪种 CMS？我支持：
>
> 1. **WordPress** — 自托管或 WordPress.com（使用 REST API + 应用程序密码）
> 2. **Strapi** — v4 或 v5，自托管（使用 API 令牌）
> 3. **Contentful** — 云端无头 CMS（使用 Delivery API 密钥）
> 4. **Ghost** — Ghost.org 或自托管（使用 Content API 密钥）
>
> 请回复名称或编号。”
...
WordPress 使用内置的**应用程序密码**功能（在 WP 5.6 中引入）。
...
> 2. **你的 WordPress 用户名**（你用于登录的用户名）
> 3. **一个应用程序密码**——请在以下位置创建：
>    WordPress 管理后台 → 用户 → 个人资料 → 向下滚动到**应用程序密码**
>    → 输入类似 "notfair" 的名称 → 点击**新增** → 复制生成的密码
>
> 准备好后，请逐项粘贴这些值。”

逐一收集值：
1. 询问 `WP_URL` → 验证它是否以 `http://` 或 `https://` 开头
2. 询问 `WP_USERNAME`
3. 询问 `WP_APP_PASSWORD`
4. 询问 `WP_CONTENT_TYPE`：
   > “我应该分析哪种内容类型？常用值：`posts`、`pages`。
   > 按 Enter 使用 `posts`（默认值），或输入自定义文章类型的别名。”

收集完全部四个值后，继续执行步骤 4（测试连接）。
...
> 2. **具有完整访问权限的 API 令牌**——请在以下位置创建：
>    Strapi 管理后台 → 设置 → 全局设置 → API 令牌 → 创建新的 API 令牌
>    → 类型：**完整访问权限** → 复制令牌
>
> 可选：
> - **内容类型**——内容集合的复数 API ID（默认值：`articles`）。
>   可在以下位置找到：内容类型构建器 → [你的类型] → API ID（复数）
> - **Strapi 版本**——`4` 或 `5`（如果省略，将自动检测）
>
> 准备好后，请逐项粘贴这些值。”
...
```
仅当用户指定了 `STRAPI_VERSION` 时，才包含 `STRAPI_VERSION=<value>`。
...
>
> 1. **Space ID** — 可在以下位置找到：Contentful → Settings → General Settings → Space ID
> 2. **Content Delivery API token** — 可在以下位置找到：
>    Settings → API Keys → [your key] → Content Delivery API - access token
>    （如果尚不存在密钥，请前往 Settings → API Keys → Add API Key 创建）
> 3. **Content type ID** — 内容类型的 API 标识符。
>    可在以下位置找到：Content model → [your type] → API Identifier
>
> 可选：
> - **Environment**（默认值：`master`）
>
> 准备好后，请逐一粘贴这些值。”
...
- `403 Forbidden` → 令牌权限不足 — 建议使用拥有完全访问权限/不受限制的令牌
- `404 Not Found` → URL 错误或内容类型别名错误
- 网络错误 → 无法访问 URL — 建议先在浏览器中检查该 URL

询问：“要修复凭据后重试吗（我会返回步骤 3），还是暂时跳过 CMS 设置？”
